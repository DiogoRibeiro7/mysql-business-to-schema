...
class StreamManager:
    ...
    def __init__(self, config_path: str = None):
        ...
        # Service URLs
        self.kafka_connect_url = self.config.get('kafka_connect_url', 'http://localhost:8083')
        self.schema_registry_url = self.config.get('schema_registry_url', 'http://localhost:8081')
    ...
    def get_connector_status(self, name: str) -> Dict:
        ...
        try:
            response = requests.get(f"{self.kafka_connect_url}/connectors/{name}/status")
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
    ...
    def resume_connector(self, name: str) -> bool:
        ...
        try:
            response = requests.put(f"{self.kafka_connect_url}/connectors/{name}/resume")
            response.raise_for_status()
    ...
    def restart_connector(self, name: str) -> bool:
        """Restart a connector"""
        try:
            response = requests.post(f"{self.kafka_connect_url}/connectors/{name}/restart")
            response.raise_for_status()
    ...
    def deploy_ksql_queries(self, query_file: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Deployed {success_count}/{len(statements)} queries successfully")
    ...
    def get_schema(self, subject: str, version: str = 'latest') -> Dict:
        ...
        try:
            response = requests.get(f"{self.schema_registry_url}/subjects/{subject}/versions/{version}")
            response.raise_for_status()
            return response.json()
        except requests.RequestException as e:
...
def main():
    ...
    parser.add_argument('command', choices=[
        'deploy', 'teardown', 'status', 'health', 'list-connectors',
        'deploy-connector', 'delete-connector', 'restart-connector',
        'list-streams', 'list-tables', 'metrics'
