...
CREATE INDEX idx_hours_special ON restaurant_hours(restaurant_id, special_hours_date)
    WHERE special_hours_date IS NOT NULL;
...
CREATE INDEX idx_orders_driver_active ON orders(driver_id, status)
    WHERE driver_id IS NOT NULL AND status IN ('picked_up', 'on_the_way');
...
CREATE INDEX idx_restaurants_nearby ON restaurants(status, delivery_enabled, latitude, longitude)
    WHERE status = 'active' AND delivery_enabled = TRUE;
...
CREATE INDEX idx_orders_active ON orders(status, placed_at, estimated_delivery_time)
    WHERE status IN ('confirmed', 'preparing', 'ready', 'picked_up', 'on_the_way');
...
ALTER TABLE restaurants
    ...
    ADD SPATIAL INDEX idx_restaurants_location (location);

-- Customer address locations
ALTER TABLE customer_addresses
    ADD COLUMN location POINT GENERATED ALWAYS AS (POINT(longitude, latitude)) STORED,
    ADD SPATIAL INDEX idx_addresses_location (location);
...
ALTER TABLE drivers
    ...
    ADD SPATIAL INDEX idx_drivers_current_location (current_location);
...
ALTER TABLE orders
    ...
    ADD SPATIAL INDEX idx_orders_delivery_location (delivery_location);
