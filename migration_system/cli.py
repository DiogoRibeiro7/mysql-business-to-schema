...
def init(host, port, user, password, database, migrations_path):
    ...
    click.echo("\nNext steps:")
    click.echo("  1. Create a migration: migrate generate --description 'Initial schema'")
...
@cli.command()
@click.option('--limit', '-n', type=int, default=10, help='Number of migrations to show')
...
def history(limit, all):
    ...
    for m in migrations:
        table_data.append([
            m['version'],
            m['description'][:40] + '...' if len(m['description']) > 40 else m['description'],
            m['applied_at'],
...
@click.option('--type', '-t', type=click.Choice(['auto', 'sql', 'python']), default='auto',
              help='Migration type')
...
'''
    ...
    click.echo(f"   Edit the file at: {config.get('migrations_path', 'migrations')}/{filename}")
...
def show(version):
    ...
    if history.is_applied(version):
        applied = next((m for m in history.get_applied_migrations() if m['version'] == version), None)
        if applied:
            click.echo(f"\n✅ Applied")
...
def diff(source, target, output):
    ...
    if output or click.confirm("\nGenerate migration file?"):
        ...
        version, filename = generator.generate_migration(
            differences,
            description
        )
        ...
        if output:
            ...
            import shutil
            shutil.move(
                Path(config.get('migrations_path', 'migrations')) / filename,
                output
            )
...
def repair(version, force):
    ...
    if migration_record['status'] != 'failed':
        click.echo(f"Migration {version} is not in failed state (status: {migration_record['status']})")
        return
