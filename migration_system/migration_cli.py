...
class MigrationCLI:
    ...
    def create_migration(self, args):
        ...
        try:
            ...
            self.print_colored(f"  Up script:   {output_dir}/up/{migration.id}.sql", 'success')
            self.print_colored(f"  Down script: {output_dir}/down/{migration.id}_rollback.sql", 'success')
            self.print_colored(f"  Metadata:    {output_dir}/{migration.id}.json", 'success')
    ...
    def list_migrations(self, args):
        ...
        try:
            ...
            # Display migrations
            self.print_colored(f"Found {len(migrations)} migration(s) in '{migrations_dir}':\n", 'info')
            ...
            for m in migrations:
                ...
                print(f"{migration_id:<35} {name:<25} {path:<20} {status_color}{status:<10}{self.colors['reset']}")
    ...
    def validate_migration(self, args):
        ...
        try:
            ...
            checks = {
                ...
                'No syntax errors (basic)': not ('ERROR' in content.upper() or 'FAIL' in content.upper()),
                'Has proper endings': content.strip().endswith(';') or content.strip().endswith('//'),
            ...
            if 'postgresql' in args.id.lower():
                checks.update({
                    'PostgreSQL types': any(t in content.upper() for t in ['SERIAL', 'JSONB', 'UUID']),
            ...
            print()
            if all_passed:
                self.print_colored("Validation PASSED - Migration appears valid", 'success')
            else:
                self.print_colored("Validation FAILED - Review the migration script", 'error')
    ...
    def batch_migrate(self, args):
        ...
        if not schema_files:
            self.print_colored(f"No schema files found matching '{schema_pattern}'", 'warning')
            return False

        self.print_colored(f"Found {len(schema_files)} schema file(s) to migrate\n", 'info')
        ...
        for schema_file in schema_files:
            example_name = schema_file.parts[0] if len(schema_file.parts) > 0 else 'unknown'
            ...
            try:
                ...
                self.print_colored(f"  [SUCCESS] Created migration: {migration.id}", 'success')
                success_count += 1
    ...
    def clean_migrations(self, args):
        ...
        if not migrations_dir.exists():
            self.print_colored(f"Migrations directory not found: {migrations_dir}", 'error')
            return False
        ...
        try:
            ...
            # Calculate size
            total_size = sum(f.stat().st_size for f in up_files + down_files + metadata_files)

            self.print_colored(f"Found {total_files} file(s) using {total_size:,} bytes", 'info')
...
def main():
    ...
    create_parser.add_argument('target', choices=['postgresql', 'mongodb'],
                              help='Target database type')
    create_parser.add_argument('--source-type', default='mysql',
                              choices=['mysql'], help='Source database type')
    create_parser.add_argument('-o', '--output', help='Output directory')
    create_parser.add_argument('-p', '--preview', action='store_true',
                              help='Show migration preview')
    ...
    show_parser.add_argument('-s', '--show-sql', action='store_true',
                           help='Show SQL content')

    # Validate command
    validate_parser = subparsers.add_parser('validate',
                                           help='Validate a migration')
    validate_parser.add_argument('id', help='Migration ID')
    validate_parser.add_argument('-d', '--dir', help='Migrations directory')

    # Batch command
    batch_parser = subparsers.add_parser('batch',
                                        help='Create migrations for multiple schemas')
    batch_parser.add_argument('target', choices=['postgresql', 'mongodb'],
                             help='Target database type')
    batch_parser.add_argument('-p', '--pattern',
                             help='Schema file pattern (glob)')
    batch_parser.add_argument('-o', '--output', help='Output directory')

    # Export command
    export_parser = subparsers.add_parser('export',
                                         help='Export migration to file')
