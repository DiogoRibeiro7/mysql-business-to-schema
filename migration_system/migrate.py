...
class MigrationCLI:
    ...
    def create_migration(self, args):
        ...
        if not os.path.exists(args.source):
            print(f"{Fore.RED}Error: Source file not found: {args.source}{Style.RESET_ALL}")
            return 1
    ...
    def execute_migration(self, args):
        ...
        if args.dry_run:
            ...
            up_script_file = os.path.join('migrations', 'up', f'{args.migration_id}.sql')
        ...
        try:
            ...
            result = self.executor.execute_migration(args.migration_id, connection_params)

            if result.success:
                print(f"{Fore.GREEN}✓ Migration executed successfully!{Style.RESET_ALL}")
    ...
    def rollback_migration(self, args):
        ...
        print(f"{Fore.YELLOW}Rolling back migration: {args.migration_id}{Style.RESET_ALL}")
        ...
        if not args.force:
            response = input(f"{Fore.YELLOW}Are you sure you want to rollback? (yes/no): {Style.RESET_ALL}")
        ...
        try:
            ...
            result = self.executor.rollback_migration(args.migration_id, connection_params)

            if result.success:
                print(f"{Fore.GREEN}✓ Migration rolled back successfully!{Style.RESET_ALL}")
            else:
    ...
    def show_migration(self, args):
        ...
        if 'error_message' in metadata:
            print(f"Error:        {Fore.RED}{metadata['error_message']}{Style.RESET_ALL}")
        ...
        if args.show_script:
            ...
            if args.show_rollback:
                down_script_file = os.path.join('migrations', 'down', f'{args.migration_id}_rollback.sql')
...
Examples:
  ...
  migrate.py show --migration-id 20240218_123456_user_tables --show-script
    ...
    create_parser.add_argument('--target-type', required=True,
                              choices=['postgresql', 'mongodb'],
                              help='Target database type')
    create_parser.add_argument('--preview', action='store_true',
                              help='Preview the generated migration script')
    ...
    show_parser.add_argument('--show-script', action='store_true',
                            help='Show the migration script')
    show_parser.add_argument('--show-rollback', action='store_true',
                            help='Show the rollback script')
