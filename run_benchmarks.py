...
class BenchmarkRunner:
    ...
    def _get_examples(self) -> List[Path]:
        ...
        if self.config['examples']:
            ...
            # All examples
            examples = sorted([d for d in self.project_root.glob('example_*')
                             if d.is_dir()])
    ...
    def _run_sequential(self, examples: List[Path]):
        ...
        for i, example in enumerate(examples, 1):
            ...
            if result.get('success'):
                print(f"  ✓ Complete - Generator: {result.get('generator_time', 0):.2f}s")
            else:
    ...
    def _run_parallel(self, examples: List[Path]):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            future_to_example = {
                executor.submit(self._benchmark_example, ex): ex
                for ex in examples
            }
    ...
    def _benchmark_generator(self, example_dir: Path) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'output': result.stdout[-1000:] if result.returncode == 0 else result.stderr[-1000:]
            }
    ...
    def _calculate_summary(self):
        ...
        successful = sum(1 for ex in self.results['examples'].values()
                        if ex.get('success'))
        failed = len(self.results['examples']) - successful

        # Generator statistics
        generator_times = [ex.get('generator_time', 0)
                          for ex in self.results['examples'].values()
                          if ex.get('success')]

        total_rows = sum(ex.get('benchmarks', {}).get('generator', {}).get('rows_generated', 0)
                        for ex in self.results['examples'].values())

        # Query statistics
        total_queries = sum(ex.get('benchmarks', {}).get('query', {}).get('total_queries', 0)
                           for ex in self.results['examples'].values())

        # Index statistics
        total_indexes = sum(ex.get('benchmarks', {}).get('index', {}).get('total_indexes', 0)
                           for ex in self.results['examples'].values())

        unused_indexes = sum(ex.get('benchmarks', {}).get('index', {}).get('unused_indexes', 0)
                            for ex in self.results['examples'].values())
        ...
        self.results['summary'] = {
            ...
            'avg_generator_time': sum(generator_times) / len(generator_times) if generator_times else 0,
    ...
    def _generate_report(self):
        ...
        if self.results['examples']:
            sorted_examples = sorted(
                [(k, v) for k, v in self.results['examples'].items()
                 if v.get('success')],
            ...
            if sorted_examples:
                ...
                for i, (name, data) in enumerate(sorted_examples[:5], 1):
                    ...
                    rows = data.get('benchmarks', {}).get('generator', {}).get('rows_generated', 0)
