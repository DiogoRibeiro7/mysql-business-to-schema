...
class IndexAdvisor:
    ...
    def analyze_slow_queries(self, queries: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        analysis_results['optimization_potential'] = self._calculate_optimization_potential()
    ...
    def _analyze_pattern(self, pattern: str, queries: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'max_time_ms': max((q.get('execution_time', 0) for q in queries), default=0),
            ...
            if ':' in pattern:  # Table:columns pattern
                ...
                if columns:  # Has WHERE clause columns
                    problem = {
                        ...
                        'avg_time': sum(q.get('execution_time', 0) for q in queries) / len(queries),
                        'severity': 'high' if len(queries) > 10 else 'medium'
                    }
                    problems.append(problem)

        # Sort by severity and frequency
        problems.sort(key=lambda x: (x['severity'] == 'high', x['frequency']), reverse=True)
    ...
    def _generate_analysis_summary(self) -> Dict[str, Any]:
        ...
        slow_patterns = sum(1 for p, q in self.query_patterns.items()
        ...
        return {
            ...
            'tables_affected': len(set(p.split(':')[0] for p in self.query_patterns if ':' in p)),
            'recommendation': 'Critical' if slow_patterns > 5 else 'Moderate' if slow_patterns > 2 else 'Minor'
        }

    def suggest_indexes(self, schema: Dict[str, Any], workload: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for table_name in tables_in_workload:
            ...
            table_queries = [q for q in workload if table_name in q.get('sql', '').upper()]
    ...
    def _suggest_indexes_for_table(self, table_name: str, table_info: Dict[str, Any],
        ...
        for cols in where_columns:
            if not self._index_exists(cols, existing_indexes):
                suggestion = self._create_index_suggestion(
                    table_name, cols, 'where', len([q for q in queries if all(c in q.get('sql', '') for c in cols)])
                )
        ...
        for cols in orderby_columns:
            if not self._index_exists(cols, existing_indexes):
                suggestion = self._create_index_suggestion(
                    table_name, cols, 'sort', len([q for q in queries if 'ORDER BY' in q.get('sql', '').upper()])
                )
                suggestions.append(suggestion)

        return suggestions

    def _analyze_where_clauses(self, table_name: str, queries: List[Dict[str, Any]]) -> List[List[str]]:
    ...
    def _analyze_join_conditions(self, table_name: str, queries: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        for query in queries:
            ...
            join_matches = re.findall(
                rf'JOIN\s+\w+\s+ON\s+.*?{table_name}\.(\w+)',
                sql,
                re.IGNORECASE
            )
    ...
    def _analyze_orderby_clauses(self, table_name: str, queries: List[Dict[str, Any]]) -> List[List[str]]:
        ...
        for query in queries:
            ...
            orderby_match = re.search(r'ORDER\s+BY\s+(.*?)(?:LIMIT|$)', sql, re.IGNORECASE)
            if orderby_match:
    ...
    def _index_exists(self, columns: List[str], existing_indexes: List[Dict[str, Any]]) -> bool:
    ...
    def _create_index_suggestion(self, table: str, columns: List[str],
                                reason: str, frequency: int) -> Dict[str, Any]:
        ...
        impact_score = self._calculate_suggestion_impact(reason, frequency, len(columns))
    ...
    def _calculate_suggestion_impact(self, reason: str, frequency: int, num_columns: int) -> float:
    ...
    def analyze_index_impact(self, table: str, index_columns: List[str],
        ...
        impact_analysis['before_metrics'] = {
            'avg_cost': sum(q['before_cost'] for q in impact_analysis['affected_queries']) / len(sample_queries),
            'total_scanned_rows': sum(q['before_rows'] for q in impact_analysis['affected_queries'])
        }

        impact_analysis['after_metrics'] = {
            'avg_cost': sum(q['after_cost'] for q in impact_analysis['affected_queries']) / len(sample_queries),
            'total_scanned_rows': sum(q['after_rows'] for q in impact_analysis['affected_queries'])
        }
        ...
        impact_analysis['improvement'] = {
            ...
            'percentage': ((before_cost - after_cost) / before_cost * 100) if before_cost > 0 else 0,
    ...
    def _analyze_query_impact(self, query: str, table: str, index_columns: List[str]) -> Dict[str, Any]:
    ...
    def get_index_recommendations(self, database: str, limit: int = 10) -> List[Dict[str, Any]]:
        """Get top index recommendations for a database"""
        # Compile all suggestions and sort by impact
        all_suggestions = sorted(self.index_suggestions, key=lambda x: x['impact_score'], reverse=True)

        # Filter by database if specified
        if database:
            all_suggestions = [s for s in all_suggestions if s.get('database') == database]

        return all_suggestions[:limit]

    def generate_index_report(self, analysis_results: Dict[str, Any],
        ...
        report = {
            ...
            'summary': {
                ...
                'critical_suggestions': sum(1 for s in suggestions if s['recommendation'] == 'critical'),
    ...
    def _create_implementation_plan(self, suggestions: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
