...
class QueryManager:
    ...
    def get_query(self, query_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        if (query['owner'] == user_id or
            query['is_public'] or
            user_id in query.get('shared_with', [])):
            return query

        return None

    def get_user_queries(self, user_id: str, include_shared: bool = True) -> List[Dict[str, Any]]:
        """Get all queries accessible to a user"""
        user_queries = []

        for query in self.queries.values():
            if (query['owner'] == user_id or
                (include_shared and (query['is_public'] or user_id in query.get('shared_with', [])))):
    ...
    def get_user_collections(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        for collection in self.collections.values():
            if (collection['owner'] == user_id or
                collection['is_public'] or
                user_id in collection.get('shared_with', [])):
                # Include query count
    ...
    def share_query(self, query_id: str, owner_id: str, share_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def restore_version(self, query_id: str, version_id: str, user_id: str) -> Dict[str, Any]:
    ...
    def record_execution(self, query_id: str, execution_data: Dict[str, Any]):
        ...
        if old_count > 0:
            query['avg_execution_time'] = (old_avg * old_count + execution_time) / query['execution_count']
        else:
    ...
    def get_performance_history(self, query_id: str, user_id: str) -> List[Dict[str, Any]]:
    ...
    def search_queries(self, user_id: str, search_term: str) -> List[Dict[str, Any]]:
        ...
        for query in user_queries:
            if (search_lower in query['name'].lower() or
                search_lower in query.get('description', '').lower() or
                search_lower in query.get('sql', '').lower() or
                any(search_lower in tag.lower() for tag in query.get('tags', []))):
