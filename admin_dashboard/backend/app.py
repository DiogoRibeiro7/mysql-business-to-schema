...
async def login(credentials: LoginRequest):
    """User login."""
    token = await auth_manager.login(credentials.username, credentials.password)
    if not token:
...
async def apply_migrations(
    ...
    try:
        ...
        result = await migration_handler.apply_migrations(
            target_version=request.target_version, dry_run=request.dry_run
        )

        # Send real-time update
        await ws_manager.broadcast({"type": "migration_update", "data": result})
...
async def rollback_migrations(
    ...
    try:
        ...
        result = await migration_handler.rollback_migrations(
            target_version=request.target_version, steps=request.steps
        )

        # Send real-time update
        await ws_manager.broadcast({"type": "migration_rollback", "data": result})
...
async def execute_query(
    ...
    try:
        ...
        if (
            request.query.strip()
            .upper()
            .startswith(("INSERT", "UPDATE", "DELETE", "DROP", "ALTER"))
        ):
            if current_user.role not in ["admin", "developer"]:
                raise HTTPException(status_code=403, detail="Write access required")

        result = await query_analyzer.execute_query(
            query=request.query, database=request.database, limit=request.limit
        )
...
async def explain_query(
    ...
    try:
        plan = await query_analyzer.explain_query(
            query=request.query, database=request.database
        )
...
async def get_query_history(
    ...
    try:
        history = await query_analyzer.get_query_history(
            user=current_user.username, limit=limit
        )
...
async def save_query(
    ...
    try:
        ...
        if query_data.get("execution_time"):
            query_manager.record_execution(
                result["query"]["id"],
                {
...
async def get_saved_query(
    ...
    try:
        ...
        if not query:
            raise HTTPException(
                status_code=404, detail="Query not found or access denied"
            )
        return {"query": query}
...
async def analyze_index_impact(
    ...
    try:
        ...
        if not table or not columns:
            raise HTTPException(
                status_code=400, detail="Table and columns are required"
            )
        ...
        return {"status": "success", "impact": impact_analysis}
    except HTTPException:
        raise
    except Exception as e:
...
async def get_existing_indexes(
    ...
    try:
        ...
        return {"database": database, "table": table, "indexes": existing_indexes}
    except Exception as e:
...
async def create_backup(
    ...
    try:
        ...
        result = await db_manager.create_backup(
            database=request.database, description=request.description
        )

        # Send real-time update
        await ws_manager.broadcast({"type": "backup_created", "data": result})
...
async def restore_backup(
    ...
    try:
        ...
        result = await db_manager.restore_backup(
            backup_id=request.backup_id, target_database=request.target_database
        )
