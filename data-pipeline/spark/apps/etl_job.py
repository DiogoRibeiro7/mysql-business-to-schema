...
from pyspark.sql.functions import (
    ...
    when, coalesce, struct, to_json, collect_list,
...
def process_patient_data(df: DataFrame) -> DataFrame:
    ...
    df = df.withColumn(
        "age",
        F.floor(F.datediff(F.current_date(), col("date_of_birth")) / 365.25)
    )
    ...
    df = df.withColumn("registration_year", year("created_at")) \
...
def process_iot_data(df: DataFrame) -> DataFrame:
    ...
    df = df.withColumn("moving_avg", avg("value").over(window_spec)) \
    ...
    # Add time features
    df = df.withColumn("reading_hour", hour("timestamp")) \
...
def write_to_clickhouse(df: DataFrame, table: str, mode: str = "append"):
    ...
    df.write \
        .mode(mode) \
        .format("jdbc") \
        .option("url", f"jdbc:clickhouse://{CLICKHOUSE_HOST}:{CLICKHOUSE_PORT}/analytics") \
...
def run_batch_etl():
    ...
    try:
        ...
        patient_df = spark \
            ...
            .load() \
            .select(from_json(col("value").cast("string"), get_patient_schema()).alias("data")) \
            .select("data.*")
        ...
        order_df = spark \
            ...
            .option("startingOffsets", "earliest") \
            .load() \
            .select(from_json(col("value").cast("string"), get_order_schema()).alias("data")) \
            .select("data.*")
        ...
        iot_df = spark \
            ...
            .option("startingOffsets", "earliest") \
            .load() \
            .select(from_json(col("value").cast("string"), get_iot_schema()).alias("data")) \
            .select("data.*")
...
def run_streaming_etl():
    ...
    try:
        # Read streaming data
        patient_stream = read_kafka_stream(spark, "clinic_db.patients", get_patient_schema())
        order_stream = read_kafka_stream(spark, "ecommerce_db.orders", get_order_schema())
        ...
        iot_query = write_stream_to_kafka(
            iot_stream,
            "processed.iot",
            "iot_checkpoint"
        )
