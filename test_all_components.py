...
class ComponentTester:
    ...
    def test_project_structure(self):
        ...
        for dir_name, description in required_dirs.items():
            if os.path.exists(dir_name):
                ...
                self.print_test(f"{description}", "FAIL", f"Directory {dir_name} not found")
    ...
    def test_schema_files(self):
        ...
        for i in range(1, 21):
            ...
            schema_dirs = [d for d in os.listdir('.') if d.startswith(f'example_{i:02d}_')]
            ...
            if schema_dirs:
                ...
                if os.path.exists(schema_path):
                    sql_files = [f for f in os.listdir(schema_path) if f.endswith('.sql')]
                    ...
                    if has_essentials:
                        self.print_test(f"Schema {i}: {schema_dir} ({len(sql_files)} SQL files)", "PASS")
                        schema_count += 1
                    else:
                        self.print_test(f"Schema {i}: {schema_dir}", "WARN", "Missing essential SQL files")
                else:
                    self.print_test(f"Schema {i}: {schema_dir}", "FAIL", "No schema directory")
        ...
        for gen_file in generator_files:
            if os.path.exists(gen_file):
                try:
                    ...
                    spec = importlib.util.spec_from_file_location("test_module", gen_file)
                    if spec:
                        self.print_test(f"Generator: {os.path.basename(gen_file)}", "PASS")
                    else:
                        self.print_test(f"Generator: {os.path.basename(gen_file)}", "FAIL", "Cannot load module")
                except Exception as e:
                    self.print_test(f"Generator: {os.path.basename(gen_file)}", "FAIL", str(e))
            else:
                self.print_test(f"Generator: {os.path.basename(gen_file)}", "SKIP", "File not found")
    ...
    def test_frontend_components(self):
                ...
                with open('admin_dashboard/frontend/package.json', 'r') as f:
                    ...
                    for dep in key_deps:
                        if dep in deps:
                            ...
                            self.print_test(f"Frontend dependency: {dep}", "FAIL", "Not in package.json")
            except Exception as e:
        ...
        for comp_file in component_files:
            if os.path.exists(comp_file):
                self.print_test(f"React component: {os.path.basename(comp_file)}", "PASS")
            else:
                self.print_test(f"React component: {os.path.basename(comp_file)}", "SKIP", "Not created")
                ...
                try:
                    with open(file_path, 'r') as f:
                        ...
                        if 'version:' in content and 'services:' in content:
                            ...
                            self.print_test(f"{description}", "WARN", "May be incomplete")
                except Exception as e:
    ...
    def test_configuration_files(self):
        ...
        config_files = [
            ...
            ('observability/grafana/dashboards/mysql-overview.json', 'Grafana dashboard'),
    ...
    def test_migration_system(self):
        ...
        if os.path.exists('migration_system'):
            ...
            if os.path.exists('migrations'):
                ...
                if migration_files:
                    self.print_test(f"Migration files ({len(migration_files)} found)", "PASS")
                else:
    ...
    def run_quick_validation(self):
        ...
        if python_version.major >= 3 and python_version.minor >= 8:
            self.print_test(f"Python version ({python_version.major}.{python_version.minor})", "PASS")
        else:
            self.print_test(f"Python version ({python_version.major}.{python_version.minor})", "WARN", "Python 3.8+ recommended")
    ...
    def print_summary(self):
        ...
        print(f"{Fore.GREEN}Passed:{Style.RESET_ALL} {len(self.results['passed'])}/{total}")
        print(f"{Fore.RED}Failed:{Style.RESET_ALL} {len(self.results['failed'])}/{total}")
        print(f"{Fore.YELLOW}Skipped:{Style.RESET_ALL} {len(self.results['skipped'])}/{total}")
        print(f"{Fore.YELLOW}Warnings:{Style.RESET_ALL} {len(self.results['warnings'])}")
        ...
        if not self.results['failed']:
            ...
            print(f"{Fore.GREEN}[OK] ALL CRITICAL COMPONENTS VALIDATED!{Style.RESET_ALL}")
