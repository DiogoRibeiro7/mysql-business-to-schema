...
class ExampleLoader:
            ...
            if readme_path.exists():
                with open(readme_path, "r", encoding="utf-8") as f:
                    ...
                    if "iot" in example_name.lower() or "sensor" in content.lower():
                        ...
                        "fintech" in example_name.lower()
                        or "banking" in content.lower()
                    ):
                        category = "Financial"
                        ...
                        "clinic" in example_name.lower()
                        or "healthcare" in example_name.lower()
                    ):
                        category = "Healthcare"
                    elif (
                        "event" in example_name.lower()
                        or "ticketing" in example_name.lower()
                    ):
                        category = "Entertainment"
                    elif (
                        "streaming" in example_name.lower()
                        or "ml" in example_name.lower()
                    ):
    ...
    def get_example_details(example_path):
        ...
        details = {
            ...
            "files": {},
            "readme_html": "",
            "stats": {"tables": 0, "queries": 0, "indexes": 0, "procedures": 0},
        }
        ...
        if schema_dir.exists():
            ...
            for sql_file in sorted(schema_dir.glob("*.sql")):
                with open(sql_file, "r", encoding="utf-8") as f:
                    ...
                    details["stats"]["indexes"] += content.lower().count(
                        "create unique index"
                    )
                    details["stats"]["procedures"] += content.lower().count(
                        "create procedure"
                    )
                    details["stats"]["procedures"] += content.lower().count(
                        "create function"
        ...
        if generator_dir.exists():
            details["generator"] = {"exists": True, "config": None, "script": None}
                ...
                with open(config_path, "r", encoding="utf-8") as f:
                    ...
                    details["generator"]["config"] = {
                        ...
                        "highlighted": highlight(
                            config_content, YamlLexer(), HtmlFormatter()
                        ),
            ...
            if readme_path.exists():
                with open(readme_path, "r", encoding="utf-8") as f:
                    ...
                    if query_lower in content:
                        ...
                        matches.append(
                            {"file": "README.md", "context": "..." + context + "..."}
                        )
                ...
                results.append(
                    {
                        ...
                        "name": "_".join(example_dir.name.split("_")[2:])
                        .replace("_", " ")
                        .title(),
...
def compare_examples():
    ...
    return render_template(
        "compare.html", examples=examples, comparison=comparison_data, selected=selected
    )
...
def documentation():
    ...
    for key in docs:
        docs[key]["html"] = markdown.markdown(
            docs[key]["content"], extensions=["fenced_code", "tables"]
        )
...
def er_diagram(example_path):
    ...
    return render_template(
        "er_diagram.html", example_path=example_path, svg_content=svg_content
    )
...
def api_er_diagram(example_path):
    ...
    return jsonify({"tables": tables, "relationships": relationships})
...
def performance_analysis(example_path):
    ...
    return render_template(
        "performance.html", example_path=example_path, report=report, charts=charts
    )
...
def api_optimize_query():
    ...
    optimizer = QueryOptimizer({"host": "localhost", "user": "root", "password": ""})
...
def export_schema(example_path, format):
    ...
    if format == "json":
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename={example_path}_schema.json"
        )
        return response
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename={example_path}_schema.md"
        )
        return response
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename={example_path}_schema.sql"
        )
        return response
