        ...
        if "tables" in metrics_data:
            ...
            for table, stats in metrics_data["tables"].items():
                if "size" in stats and stats["size"]:
                    ...
                    data_sizes.append(
                        stats["size"]["data_length"] / 1024 / 1024
                    )  # Convert to MB
...
class QueryOptimizer:
        ...
        try:
            ...
            try:
                ...
                if json_plan:
                    ...
                    if (
                        "query_block" in plan_data
                        and "cost_info" in plan_data["query_block"]
                    ):
    ...
    def suggest_indexes(self, query, database=None):
        ...
        where_match = re.search(
            r"WHERE\s+(.*?)(?:GROUP BY|ORDER BY|LIMIT|$)", query_upper, re.DOTALL
        )
        if where_match:
        ...
        join_matches = re.finditer(
            r"JOIN\s+\w+\s+ON\s+(.*?)(?:JOIN|WHERE|GROUP BY|ORDER BY|$)",
            query_upper,
            re.DOTALL,
        )
...
class DataExporter:
    ...
    @staticmethod
    def export_to_sql(
        database, tables=None, include_data=False, connection_params=None
    ):
    ...
    def export_to_markdown(schema_analysis):
        ...
        for table_name, columns in schema_analysis.items():
            ...
            for col in columns:
                ...
                output.append(
                    f"| {col['name']} | {col['type']} | {nullable} | {key_type} |"
                )
    ...
    def export_to_json(schema_analysis):
        ...
        export_data = {"timestamp": datetime.now().isoformat(), "tables": {}}

        for table_name, columns in schema_analysis.items():
            export_data["tables"][table_name] = {"columns": columns}
