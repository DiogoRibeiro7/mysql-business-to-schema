...
class RegressionDetector:
    ...
    def _compare_generators(self):
        ...
        for example in all_examples:
            ...
            if not baseline_data and current_data:
                # New generator added
                self.changes.append(PerformanceChange(
    ...
    def _compare_metric(self, metric_name: str, baseline: float,
                       current: float, direction: str, details: Dict = None):
    ...
    def _generate_report(self, health_score: float) -> Dict:
        ...
        regressions = [c for c in self.changes if c.change_type == ChangeType.REGRESSION]
        improvements = [c for c in self.changes if c.change_type == ChangeType.IMPROVEMENT]
        ...
        report = {
            ...
            'summary': {
                ...
                'critical_issues': len([c for c in regressions if c.severity == 'CRITICAL']),
    ...
    def _generate_recommendations(self) -> List[Dict]:
        ...
        generator_regressions = [c for c in self.changes
                                if c.change_type == ChangeType.REGRESSION
                                and c.details.get('type') == 'generator']

        memory_regressions = [c for c in self.changes
                             if c.change_type == ChangeType.REGRESSION
        ...
        # Critical issues
        critical = [c for c in self.changes
                   if c.change_type == ChangeType.REGRESSION
    ...
    def print_summary(self, report: Dict):
        ...
        if report['regressions']:
            ...
            for reg in report['regressions'][:5]:
                ...
                print(f"  {severity_icon} [{reg['severity']}] {reg['metric']}: "
...
def main():
    ...
    parser.add_argument('--latest', action='store_true',
                       help='Compare two most recent benchmarks')
