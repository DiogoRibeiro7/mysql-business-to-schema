...
class IoTBinsGenerator(BaseGenerator):
    ...
    def __init__(self, config_path: str = 'config.yaml', **db_params):
        ...
        self.start_date = datetime.now() - timedelta(days=self.config['days_of_history'])
        ...
        for district in self.districts:
            ...
            for _ in range(self.config['bins_per_district']):
                ...
                bin_data = {
                    ...
                    'installation_date': self.fake.date_between(start_date='-3y', end_date='-6m'),
                    'last_maintenance': self.fake.date_between(start_date='-3m', end_date='today'),
                    'is_active': 1 if random.random() > 0.05 else 0,  # 95% active
                    'created_at': self.fake.date_time_between(start_date='-3y', end_date='-2y'),
                    'updated_at': self.fake.date_time_between(start_date='-7d', end_date='now')
                }
        ...
        for bin_data in self.bins:
            for sensor_type in sensor_types[:self.config['sensors_per_bin']]:
                ...
                sensor = {
                    ...
                    'manufacturer': random.choice(['SensorTech', 'IoTDevices', 'SmartSense', 'TechMeasure']),
                    ...
                    'battery_capacity': 5000 if sensor_type == 'battery' else random.choice([3000, 4000, 5000]),
                    'installation_date': bin_data['installation_date'],
                    'last_calibration': self.fake.date_between(start_date='-2m', end_date='today'),
                    'is_active': bin_data['is_active'],
                    'created_at': bin_data['created_at'],
                    'updated_at': self.fake.date_time_between(start_date='-24h', end_date='now')
                }
    ...
    def generate_trucks(self):
        ...
        truck_types = ['compactor', 'recycling', 'side_loader', 'rear_loader', 'front_loader']
        ...
        for i in range(self.config['trucks']):
            truck = {
                ...
                'last_service_date': self.fake.date_between(start_date='-3m', end_date='today'),
                'next_service_date': self.fake.date_between(start_date='today', end_date='+3m'),
    ...
    def generate_drivers(self):
        ...
        for i in range(self.config['drivers']):
            driver = {
                ...
                'license_expiry': self.fake.date_between(start_date='+1y', end_date='+5y'),
                'hire_date': self.fake.date_between(start_date='-10y', end_date='-1m'),
                'is_active': 1 if random.random() > 0.1 else 0,  # 90% active
                'created_at': self.fake.date_time_between(start_date='-10y', end_date='-1m'),
                'updated_at': self.fake.date_time_between(start_date='-30d', end_date='now')
            }
    ...
    def generate_routes(self):
        ...
        for district in self.districts:
            ...
            for route_num in range(self.config['routes_per_district']):
                ...
                bins_per_route = len(district_bins) // self.config['routes_per_district']
                route_bins = random.sample(district_bins, min(bins_per_route, len(district_bins)))
    ...
    def insert_data_to_database(self):
        ...
        try:
            ...
            if self.districts:
                ...
                self.bulk_insert('districts', district_data, list(self.districts[0].keys()))
                print(f"Inserted {len(self.districts)} districts")

            # Insert bins
            if self.bins:
                bin_data = [tuple(b.values()) for b in self.bins]
                self.bulk_insert('bins', bin_data, list(self.bins[0].keys()),
            ...
            if self.sensors:
                ...
                self.bulk_insert('sensors', sensor_data, list(self.sensors[0].keys()),
                               batch_size=self.config.get('batch_size', 1000))
            ...
            if self.route_assignments:
                ...
                self.bulk_insert('route_bin_assignments', assignment_data,
            ...
            columns = ['reading_id', 'sensor_id', 'timestamp', 'value', 'is_valid', 'created_at']

            # Process one day at a time to manage memory
            for day_offset in range(min(7, self.config['days_of_history'])):  # Limit to 7 days for demo
                ...
                for i in range(0, len(self.sensors), batch_size):
                    ...
                    readings = self.generate_sensor_readings_batch(batch_sensors, current_date)

                    if readings:
                        self.bulk_insert('sensor_readings', readings, columns,
                                       batch_size=self.config.get('batch_size', 5000))

                print(f"    Generated {len(self.sensors) * 6} readings for {current_date}")
    ...
    def generate_data(self, scale: str = 'small'):
        ...
        print(f"  Total bins: {self.config['districts'] * self.config['bins_per_district']}")
...
def main():
    ...
    parser.add_argument('--format', choices=['database', 'json', 'csv'], default='database',
                       help='Output format')
    ...
    parser.add_argument('--database', default='iot_waste_management', help='Database name')
