...
class IoTBinsGenerator:
    ...
    def generate_all(self):
        ...
        print(f"  Total sensors: {CONFIG['districts'] * CONFIG['bins_per_district'] * CONFIG['sensors_per_bin']}")
    ...
    def generate_districts(self):
        ...
        district_names = [
            "Downtown", "North End", "South Side", "East Borough", "West Village",
            "Central Business", "Industrial Zone", "Riverside", "University Area",
            "Tech Park", "Old Town", "Harbor District"
        ]
        ...
        for i in range(CONFIG['districts']):
            district = {
                ...
                'name': district_names[i] if i < len(district_names) else f"District {i+1}",
    ...
    def generate_bins(self):
        ...
        location_types = ['residential', 'commercial', 'industrial', 'park', 'school', 'hospital', 'public']
        ...
        for district in self.districts:
            ...
            for _ in range(CONFIG['bins_per_district']):
                ...
                bin_data = {
                    ...
                    'location_type': random.choices(location_types, weights=location_weights)[0],
                    ...
                    'status': random.choices(['active', 'maintenance', 'damaged'], weights=[95, 4, 1])[0],
    ...
    def generate_sensors(self):
        ...
        for bin_data in self.bins:
            ...
            sensor_configs.append({
                'type': 'fill_level',
                'manufacturer': random.choice(['UltraSonic Inc', 'SmartSense', 'IoTech']),
            ...
            if bin_data['location_type'] in ['commercial', 'industrial'] and random.random() > 0.5:
                sensor_configs.append({
                    'type': 'weight',
            ...
            for config in sensor_configs:
                ...
                sensor = {
                    ...
                    'battery_level': round(random.uniform(20, 100), 2) if config['type'] != 'battery' else 100,
                    'last_reading_time': fake.date_time_between(start_date='-1h', end_date='now'),
                    'status': random.choices(['active', 'offline', 'maintenance', 'faulty'],
                                           weights=[90, 5, 3, 2])[0],
    ...
    def generate_trucks(self):
        ...
        for i in range(CONFIG['trucks']):
            truck = {
                ...
                'last_maintenance_date': fake.date_between(start_date='-2m', end_date='today'),
                'next_maintenance_date': fake.date_between(start_date='today', end_date='+3m'),
                'odometer_km': random.randint(10000, 200000),
                'status': random.choices(['available', 'in_use', 'maintenance', 'repair'],
                                       weights=[40, 45, 10, 5])[0],
    ...
    def generate_drivers(self):
        ...
        for i in range(CONFIG['drivers']):
            driver = {
                ...
                'status': random.choices(['active', 'on_leave', 'sick', 'terminated'],
                                       weights=[85, 10, 3, 2])[0],
    ...
    def generate_routes(self):
        ...
        route_id = 0
        for district in self.districts:
            district_bins = [b for b in self.bins if b['district_id'] == district['district_id']]
            ...
            for route_num in range(CONFIG['routes_per_district']):
                ...
                num_bins = random.randint(min_bins, max_bins) if min_bins <= max_bins else len(district_bins)
    ...
    def generate_schedules_and_collections(self):
        ...
        active_trucks = [t for t in self.trucks if t['status'] in ['available', 'in_use']]
        ...
        while current_date < end_date:
            ...
            for route in self.routes:
                ...
                schedule = {
                    ...
                    'status': 'completed' if current_date < end_date - timedelta(days=1) else 'scheduled',
                ...
                if schedule['status'] == 'completed':
                    ...
                    actual_start = current_date.replace(hour=start_hour, minute=random.randint(0, 30))
                    actual_duration = route['estimated_duration_minutes'] + random.randint(-30, 60)
                    ...
                    route_bins = [a for a in self.route_assignments if a['route_id'] == route['route_id']]
                    ...
                    for assignment in route_bins:
                        ...
                        bin_data = next(b for b in self.bins if b['bin_id'] == assignment['bin_id'])

                        # Calculate fill level based on bin type and days since last collection
                        base_fill = random.uniform(40, 95)  # Most bins are fairly full when collected

                        # Adjust based on bin type
                        if bin_data['bin_type'] == 'organic':
                            base_fill = min(100, base_fill * 1.2)  # Organic fills faster
                        ...
                        event = {
                            ...
                            'truck_id': truck['truck_id'],
                            'driver_id': driver['driver_id'],
                            'collected_at': actual_start + timedelta(minutes=assignment['collection_order'] * 3),
                            'fill_level_before': round(base_fill, 2),
                            'weight_kg': round(base_fill * bin_data['capacity_liters'] * 0.3, 2),  # Rough weight estimate
                            'collection_duration_seconds': random.randint(30, 180),
                            'notes': None if random.random() > 0.02 else fake.sentence(),
    ...
    def generate_alert_thresholds(self):
        ...
        threshold_configs = [
            ...
            {'name': 'General Bin Fill Warning', 'bin_type': 'general', 'sensor_type': 'fill_level',
             'warning_value': 75, 'critical_value': 90},
            {'name': 'Recycling Bin Fill Warning', 'bin_type': 'recycling', 'sensor_type': 'fill_level',
             'warning_value': 80, 'critical_value': 95},
            {'name': 'Organic Bin Fill Warning', 'bin_type': 'organic', 'sensor_type': 'fill_level',
             'warning_value': 70, 'critical_value': 85},

            # Temperature thresholds
            {'name': 'High Temperature Alert', 'bin_type': 'all', 'sensor_type': 'temperature',
             'warning_value': 35, 'critical_value': 45},
            {'name': 'Low Temperature Alert', 'bin_type': 'all', 'sensor_type': 'temperature',
             'warning_value': 0, 'critical_value': -10},

            # Battery thresholds
            {'name': 'Low Battery Warning', 'bin_type': 'all', 'sensor_type': 'battery',
             'warning_value': 30, 'critical_value': 15},

            # Odor thresholds
            {'name': 'High Odor Level', 'bin_type': 'organic', 'sensor_type': 'odor',
             'warning_value': 60, 'critical_value': 80},

            # Tilt detection
            {'name': 'Tilt Detection', 'bin_type': 'all', 'sensor_type': 'tilt',
             'warning_value': 15, 'critical_value': 45},  # Degrees from vertical
        ]
    ...
    def generate_sensor_data(self):
        ...
        for sensor in random.sample(self.sensors, min(100, len(self.sensors))):  # Sample of sensors
            bin_data = next(b for b in self.bins if b['bin_id'] == sensor['bin_id'])

            # Find last collection for this bin
            bin_collections = [e for e in self.collection_events if e['bin_id'] == sensor['bin_id']]
            ...
            while current_time < datetime.now():
                ...
                if reading_value is not None:
                    ...
                    reading = {
                        ...
                        'quality': self.determine_reading_quality(sensor, reading_value),
                        'created_at': current_time
                    }
                    self.sensor_readings.append(reading)

                    # Check for alerts
                    self.check_and_create_alerts(sensor, bin_data, reading_value, current_time)
            ...
            for day in range(sample_days):
                ...
                day_readings = [r for r in self.sensor_readings
                ...
                if day_readings:
                    ...
                    daily_agg = {
                        ...
                        'avg_value': round(sum(r['reading_value'] for r in day_readings) / len(day_readings), 3),
                        'peak_hour': max(day_readings, key=lambda r: r['reading_value'])['reading_time'].time(),
                        'peak_value': max(r['reading_value'] for r in day_readings),
                        'reading_count': len(day_readings),
                        'quality_good_pct': round(len([r for r in day_readings if r['quality'] == 'good']) * 100 / len(day_readings), 2),
                        'quality_warning_pct': round(len([r for r in day_readings if r['quality'] == 'warning']) * 100 / len(day_readings), 2),
                        'quality_error_pct': round(len([r for r in day_readings if r['quality'] == 'error']) * 100 / len(day_readings), 2),
                    ...
                    for hour in range(24):
                        ...
                        hour_readings = [r for r in day_readings
                                       if r['reading_time'].hour == hour]
                        ...
                        if hour_readings:
                            ...
                            hourly_agg = {
                                ...
                                'min_value': min(r['reading_value'] for r in hour_readings),
                                'max_value': max(r['reading_value'] for r in hour_readings),
                                'avg_value': round(sum(r['reading_value'] for r in hour_readings) / len(hour_readings), 3),
                                'reading_count': len(hour_readings),
                                'quality_good_count': len([r for r in hour_readings if r['quality'] == 'good']),
                                'quality_warning_count': len([r for r in hour_readings if r['quality'] == 'warning']),
                                'quality_error_count': len([r for r in hour_readings if r['quality'] == 'error']),
    ...
    def calculate_sensor_reading(self, sensor, bin_data, bin_collections, current_time):
        ...
        if sensor['sensor_type'] == 'fill_level':
            ...
            if bin_collections:
                ...
                hours_since_collection = (current_time - last_collection['collected_at']).total_seconds() / 3600
                ...
                if bin_data['location_type'] == 'residential' and current_time.weekday() >= 5:
                    fill_rate *= 1.3

                # Calculate fill level
                base_fill = 5  # Start at 5% after collection
                fill_level = base_fill + (hours_since_collection * fill_rate)
                fill_level = min(100, fill_level + random.uniform(-5, 5))  # Add noise and cap at 100
            ...
            days_since_install = (current_time - datetime.combine(sensor['installation_date'], datetime.min.time())).days
            ...
            fill_sensor = next((s for s in self.sensors
                              if s['bin_id'] == sensor['bin_id'] and s['sensor_type'] == 'fill_level'), None)
            if fill_sensor:
    ...
    def check_and_create_alerts(self, sensor, bin_data, reading_value, current_time):
        ...
        thresholds = [t for t in self.alert_thresholds
        ...
        for threshold in thresholds:
            ...
            if sensor['sensor_type'] == 'fill_level':
                ...
                if reading_value >= threshold['critical_value']:
                    ...
                    severity = 'emergency'
                    message = f"Bin {bin_data['bin_code']} has been tilted or vandalized"
            ...
            if alert_type and random.random() > 0.7:  # Don't create too many alerts
                ...
                alert = {
                    ...
                    'resolved_at': current_time + timedelta(hours=random.randint(1, 12)) if random.random() > 0.3 else None,
                    'alert_value': reading_value,
                    'message': message,
                    'acknowledged': random.random() > 0.2,
                    'acknowledged_by': fake.name() if random.random() > 0.2 else None,
                    'acknowledged_at': current_time + timedelta(minutes=random.randint(5, 60)) if random.random() > 0.3 else None,
                    'created_at': current_time
                }
            ...
            for bin_data in random.sample(self.bins, min(100, len(self.bins))):
                ...
                for hour in [6, 9, 12, 15, 18, 21]:  # Key hours of the day
                    ...
                    prediction = {
                        ...
                        'predicted_fill_rate': min(100, round(base_fill + random.uniform(-10, 10), 2)),
