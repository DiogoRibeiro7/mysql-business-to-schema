...
class IoTBinsGenerator:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        # Parse dates
        self.start_date = datetime.strptime(self.config['date_ranges']['historical_start'], '%Y-%m-%d')
        self.end_date = datetime.strptime(self.config['date_ranges']['historical_end'], '%Y-%m-%d')
    ...
    def generate_districts(self):
        ...
        district_names = [
            "Downtown", "Northside", "Southside", "East End", "West End",
            "Harbor District", "University District", "Industrial Zone",
            "Old Town", "Business Park"
        ]
    ...
    def generate_bins(self):
        ...
        location_type_weights = list(self.config['distributions']['location_types'].values())
        ...
        for i in range(self.config['counts']['bins']):
            ...
            # Installation date
            install_start = datetime.strptime(self.config['date_ranges']['installation_start'], '%Y-%m-%d')
            install_end = datetime.strptime(self.config['date_ranges']['installation_end'], '%Y-%m-%d')
            ...
            bin_data = {
                ...
                'last_maintenance_date': self.fake.date_between(install_date, self.end_date) if random.random() < 0.3 else None,
    ...
    def generate_sensors(self):
        ...
        for bin_data in self.bins:
            ...
            for sensor_type in sensor_types[:num_sensors]:
                ...
                battery_level = max(10, 100 - (days_since_install * 0.05) + random.uniform(-5, 5))
                ...
                sensor = {
                    ...
                    'installation_date': install_date,
                    'reading_frequency_seconds': self.config['sensor_parameters']['reading_frequency_seconds'],
                    'battery_level': round(battery_level, 2),
                    'last_reading_time': self.end_date.strftime('%Y-%m-%d %H:%M:%S'),
                    'status': random.choices(
                        ['active', 'offline', 'maintenance', 'faulty'],
                        weights=[0.92, 0.04, 0.03, 0.01]
                    )[0] if bin_data['status'] == 'active' else 'offline'
                }
    ...
    def generate_route_assignments(self):
        ...
        for route in self.routes:
            ...
            district_bins = [b for b in self.bins if b['district_id'] == route['district_id'] and b['status'] == 'active']
            ...
            for route in scheduled_routes:
                if available_trucks and available_drivers:
                    ...
                    schedule = {
                        ...
                        'status': 'completed' if current_date < self.end_date.date() else 'scheduled',
                        'actual_start_time': f"{current_date} {start_hour:02d}:{random.randint(0, 15):02d}:00"
                            if current_date < self.end_date.date() else None,
                        'actual_end_time': f"{current_date} {start_hour + duration_hours:02d}:{random.randint(0, 30):02d}:00"
                            if current_date < self.end_date.date() else None,
                        'notes': None
                    }
    ...
    def generate_collection_events(self):
        ...
        for schedule in self.schedules:
            ...
            route_bins = [a for a in self.route_assignments if a['route_id'] == schedule['route_id']]
            ...
            for assignment in route_bins:
                ...
                collection_time = datetime.strptime(schedule['actual_start_time'], '%Y-%m-%d %H:%M:%S')
                ...
                weight = (fill_level / 100) * bin_data['capacity_liters'] * weight_per_liter
    ...
    def generate_fill_pattern(self, bin_data: Dict, sensor: Dict, current_time: datetime) -> float:
        ...
        if location_type == 'residential':
            if day_of_week < 5:  # Weekday
                base_rate = 30 + 40 * math.sin((hour - 6) * math.pi / 12) if 6 <= hour <= 22 else 20
            else:  # Weekend
                base_rate = 35 + 45 * math.sin((hour - 8) * math.pi / 14) if 8 <= hour <= 22 else 25
            ...
            base_rate = 25 + 30 * math.sin((hour - 6) * math.pi / 16) if 6 <= hour <= 22 else 15
        ...
        for event in sorted(self.collection_events, key=lambda e: e['collected_at'], reverse=True):
            if event['bin_id'] == bin_data['bin_id'] and event['collected_at'] < current_time.strftime('%Y-%m-%d %H:%M:%S'):
                last_collection = datetime.strptime(event['collected_at'], '%Y-%m-%d %H:%M:%S')
                break

        # Calculate gradual increase since last collection
        if last_collection:
            hours_since_collection = (current_time - last_collection).total_seconds() / 3600
    ...
    def generate_sensor_readings(self):
        ...
        while current_time <= self.end_date:
            for sensor in self.sensors:
                ...
                if random.random() < self.config['sensor_parameters']['offline_probability']:
                    continue
                ...
                if sensor['sensor_type'] == 'fill_level':
                    reading_value = self.generate_fill_pattern(bin_data, sensor, current_time)
                    unit = 'percentage'
                elif sensor['sensor_type'] == 'temperature':
                    # Temperature varies by time and season
                    month = current_time.month
                    if month in [12, 1, 2]:  # Winter
                        base_temp = self.config['sensor_parameters']['temperature']['winter_mean']
                        std = self.config['sensor_parameters']['temperature']['winter_std']
                    else:  # Summer/Spring/Fall
                        base_temp = self.config['sensor_parameters']['temperature']['summer_mean']
                        std = self.config['sensor_parameters']['temperature']['summer_std']
                    reading_value = random.gauss(base_temp, std)
                    unit = 'celsius'
                elif sensor['sensor_type'] == 'odor':
                    # Odor correlates with fill level and temperature
                    fill_sensor = next((s for s in self.sensors if s['bin_id'] == sensor['bin_id'] and s['sensor_type'] == 'fill_level'), None)
                    if fill_sensor:
                        fill_level = self.generate_fill_pattern(bin_data, fill_sensor, current_time)
                    else:
                        fill_level = 50
                    base_odor = self.config['sensor_parameters']['odor']['base_level']
                    reading_value = base_odor + (fill_level / 100) * self.config['sensor_parameters']['odor']['filled_multiplier'] * 20
                    unit = 'ppm'
                elif sensor['sensor_type'] == 'battery':
                    # Battery degrades over time
                    days_elapsed = (current_time - self.start_date).days
                    reading_value = max(10, sensor['battery_level'] - days_elapsed * self.config['sensor_parameters']['battery_degradation_per_day'])
                    unit = 'percentage'
                else:
                    continue

                # Determine quality
                if random.random() < self.config['sensor_parameters']['error_reading_probability']:
                    quality = 'error'
            ...
            if len(self.sensor_readings) > 1000000:
                print(f"Generated {len(self.sensor_readings)} readings, stopping early for performance...")
                break
    ...
    def generate_aggregates(self):
        ...
        for reading in self.sensor_readings:
            ...
            reading_time = datetime.strptime(reading['reading_time'], '%Y-%m-%d %H:%M:%S')
            hour_key = (reading['sensor_id'], reading_time.strftime('%Y-%m-%d %H:00:00'))
        ...
        for (sensor_id, date), readings in daily_data.items():
            ...
            peak_hour = max(hourly_values.keys(), key=lambda h: sum(hourly_values[h]) / len(hourly_values[h]))
            ...
            daily = {
                ...
                'quality_good_pct': round(quality_counts['good'] / total_readings * 100, 2),
                'quality_warning_pct': round(quality_counts['warning'] / total_readings * 100, 2),
                'quality_error_pct': round(quality_counts['error'] / total_readings * 100, 2)
            }
    ...
    def generate_alert_thresholds(self):
        ...
        threshold_configs = [
            {'name': 'Fill Level Warning', 'bin_type': 'all', 'sensor_type': 'fill_level',
             'warning_value': 80, 'critical_value': 90},
            {'name': 'Battery Low', 'bin_type': 'all', 'sensor_type': 'battery',
             'warning_value': 20, 'critical_value': 10},
            {'name': 'Temperature High', 'bin_type': 'all', 'sensor_type': 'temperature',
             'warning_value': 35, 'critical_value': 40},
            {'name': 'Odor High', 'bin_type': 'all', 'sensor_type': 'odor',
             'warning_value': 60, 'critical_value': 80},
            {'name': 'Organic Bin Temperature', 'bin_type': 'organic', 'sensor_type': 'temperature',
             'warning_value': 30, 'critical_value': 35},
            {'name': 'Hazardous Fill Critical', 'bin_type': 'hazardous', 'sensor_type': 'fill_level',
    ...
    def generate_alerts(self):
        ...
        for aggregate in self.hourly_aggregates[:1000]:  # Limit for performance
            sensor = next((s for s in self.sensors if s['sensor_id'] == aggregate['sensor_id']), None)
            if not sensor:
                continue

            bin_data = next((b for b in self.bins if b['bin_id'] == sensor['bin_id']), None)
            if not bin_data:
                continue
            ...
            for threshold in self.alert_thresholds:
                if threshold['sensor_type'] != sensor['sensor_type']:
                    continue
                if threshold['bin_type'] != 'all' and threshold['bin_type'] != bin_data['bin_type']:
                    continue
                ...
                if alert_type and random.random() < 0.3:  # 30% chance to generate alert
                    triggered_at = datetime.strptime(aggregate['hour_start'], '%Y-%m-%d %H:%M:%S')
                    ...
                    resolved_at = None
                    if resolved:
                        resolved_at = triggered_at + timedelta(hours=random.randint(1, 24))
                    ...
                    alert = {
                        ...
                        'acknowledged_by': f"operator_{random.randint(1, 10)}" if resolved else None,
                        'acknowledged_at': (triggered_at + timedelta(minutes=random.randint(5, 60))).strftime('%Y-%m-%d %H:%M:%S') if resolved else None
                    }
