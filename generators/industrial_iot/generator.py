...
class IndustrialIoTGenerator:
    ...
    def generate_all(self):
        ...
        print(f"  Total production lines: {CONFIG['factories'] * CONFIG['lines_per_factory']}")
        print(f"  Total machines: {CONFIG['factories'] * CONFIG['lines_per_factory'] * CONFIG['machines_per_line']}")
    ...
    def generate_factories(self):
        ...
        factory_types = ['automotive', 'electronics', 'consumer_goods', 'pharmaceutical', 'food']
        locations = [
        ...
        for i in range(CONFIG['factories']):
            ...
            factory = {
                ...
                'country': location[1],
                'timezone': location[2],
                'established_date': fake.date_between(start_date='-20y', end_date='-5y'),
        ...
        for factory in self.factories:
            for line_num in range(CONFIG['lines_per_factory']):
                ...
                if factory['factory_type'] == 'automotive':
                    product_types = ['engine_parts', 'chassis', 'electronics', 'interior']
                ...
                line = {
                    ...
                    'product_types': json.dumps(random.sample(product_types, k=random.randint(1, 3))),
    ...
    def generate_machines(self):
        ...
        machine_types_by_line = {
            'assembly': ['assembly_robot', 'welding_robot', 'conveyor', 'quality_inspection'],
            ...
            'mixed': ['cnc_mill', 'assembly_robot', 'packaging', 'conveyor', 'quality_inspection']
        }

        manufacturers = ['Siemens', 'ABB', 'Fanuc', 'Kuka', 'Mitsubishi', 'Bosch', 'Schneider']
        ...
        for line in self.production_lines:
            ...
            for machine_num in range(CONFIG['machines_per_line']):
                ...
                capacity = random.randint(*capacity_ranges.get(machine_type, (100, 500)))
                ...
                machine = {
                    ...
                    'ideal_cycle_time_seconds': round(3600 / capacity, 2),  # Based on capacity
                    'max_capacity_per_hour': capacity,
                    'power_consumption_kw': round(random.uniform(5, 50), 2),
                    'status': random.choices(['running', 'idle', 'maintenance', 'error', 'offline'],
    ...
    def generate_sensors(self):
        ...
        sensor_configs = {
            'temperature': {'unit': '°C', 'min': -50, 'max': 200, 'normal_min': 20, 'normal_max': 80},
            'vibration': {'unit': 'mm/s', 'min': 0, 'max': 100, 'normal_min': 0, 'normal_max': 10},
            'pressure': {'unit': 'bar', 'min': 0, 'max': 100, 'normal_min': 1, 'normal_max': 10},
            'current': {'unit': 'A', 'min': 0, 'max': 1000, 'normal_min': 10, 'normal_max': 100},
            'voltage': {'unit': 'V', 'min': 0, 'max': 1000, 'normal_min': 200, 'normal_max': 240},
            'speed': {'unit': 'rpm', 'min': 0, 'max': 10000, 'normal_min': 500, 'normal_max': 3000},
            'flow': {'unit': 'l/min', 'min': 0, 'max': 1000, 'normal_min': 10, 'normal_max': 100},
            'force': {'unit': 'N', 'min': 0, 'max': 10000, 'normal_min': 100, 'normal_max': 1000},
        }
    ...
    def generate_products(self):
        ...
        categories_by_factory = {
            'automotive': ['engine_component', 'brake_system', 'suspension', 'electrical', 'body_part'],
            'electronics': ['circuit_board', 'display', 'sensor', 'connector', 'housing'],
        ...
        for i in range(CONFIG['products']):
            ...
            quality_specs = {
                ...
                'surface_finish': random.choice(['smooth', 'textured', 'polished', 'matte']),
            ...
            product = {
                ...
                'unit_of_measure': random.choice(['unit', 'piece', 'kg', 'meter', 'liter']),
    ...
    def generate_work_orders_and_runs(self):
        ...
        while current_date < datetime.now():
            ...
            for _ in range(daily_orders):
                ...
                line = random.choice([l for l in self.production_lines if l['status'] == 'operational'])

                # Plan production
                planned_quantity = random.randint(100, 1000)
                planned_hours = planned_quantity * product['standard_cycle_time_seconds'] / 3600
                ...
                order = {
                    ...
                    'actual_start_time': planned_start + timedelta(minutes=random.randint(-30, 30)),
                    'actual_end_time': planned_end + timedelta(minutes=random.randint(-60, 120)),
                    'produced_quantity': actual_quantity,
                    'good_quantity': good_quantity,
                    'rejected_quantity': rejected_quantity,
                    'status': 'completed' if current_date < datetime.now() - timedelta(days=1) else 'in_progress',
                ...
                if order['status'] == 'completed':
                    machines_on_line = [m for m in self.machines if m['line_id'] == line['line_id']]

                    for machine in random.sample(machines_on_line, k=min(3, len(machines_on_line))):
                        self.run_id += 1
                        ...
                        run = {
                            ...
                            'order_id': self.order_id,
                            'machine_id': machine['machine_id'],
                            'operator_id': random.randint(1, 100),  # Assuming 100 operators
                            'start_time': order['actual_start_time'],
                            'end_time': order['actual_end_time'],
                            'quantity_produced': actual_quantity // len(machines_on_line),
                            'quantity_good': good_quantity // len(machines_on_line),
                            'quantity_rejected': rejected_quantity // len(machines_on_line),
                            'cycle_time_actual': round(product['standard_cycle_time_seconds'] * random.uniform(0.9, 1.1), 2),
    ...
    def generate_quality_inspections(self):
        ...
        defect_types = {
            'dimensional': ['length_deviation', 'width_deviation', 'thickness_deviation', 'diameter_deviation'],
            'surface': ['scratch', 'dent', 'rough_finish', 'discoloration', 'contamination'],
            'functional': ['not_working', 'intermittent', 'out_of_spec', 'loose_connection'],
            'cosmetic': ['color_mismatch', 'texture_issue', 'label_defect', 'packaging_damage']
        }

        for run in random.sample(self.production_runs, min(len(self.production_runs) // 3, 1000)):
            self.inspection_id += 1

            sample_size = min(10, run['quantity_produced'])
            defects_found = min(run['quantity_rejected'], random.randint(0, sample_size // 5))

            # Determine defect types found
            found_defect_types = []
            if defects_found > 0:
                category = random.choice(list(defect_types.keys()))
                found_defect_types = random.sample(defect_types[category], k=min(defects_found, 3))
            ...
            inspection = {
                ...
                'run_id': run['run_id'],
                'product_id': next(wo['product_id'] for wo in self.work_orders if wo['order_id'] == run['order_id']),
                'inspection_time': run['end_time'] + timedelta(minutes=random.randint(5, 30)),
            ...
            for defect_type in found_defect_types:
                ...
                defect = {
                    ...
                    'quantity': random.randint(1, max(1, defects_found // len(found_defect_types))),
                    'root_cause': random.choice(['material_issue', 'process_deviation', 'equipment_malfunction', 'operator_error', 'unknown']),
                    'corrective_action': fake.sentence(),
                    'image_url': f"https://images.example.com/defect_{self.defect_id}.jpg" if random.random() > 0.5 else None,
        ...
        for machine in self.machines:
            ...
            if random.random() > 0.3:  # 70% of machines have maintenance history
                for _ in range(random.randint(1, 5)):
                    ...
                    maintenance_date = fake.date_time_between(start_date='-6m', end_date='now')
                    duration_hours = random.uniform(0.5, 8)

                    record = {
                        'record_id': self.record_id,
                        'machine_id': machine['machine_id'],
                        'schedule_id': self.schedule_id if random.random() > 0.3 else None,
                        'maintenance_type': random.choice(maintenance_types),
                        'start_time': maintenance_date,
                        'end_time': maintenance_date + timedelta(hours=duration_hours),
                        'technician_id': random.randint(1, 10),  # Assuming 10 technicians
                        'downtime_minutes': round(duration_hours * 60, 2),
                        'parts_replaced': json.dumps([
                            {'part': random.choice(['filter', 'belt', 'bearing', 'seal']),
                        ...
                        'next_action': fake.sentence() if random.random() > 0.7 else None,
                        'status': 'completed',
                        'created_at': maintenance_date,
                        'updated_at': maintenance_date + timedelta(hours=duration_hours)
                    }
        ...
        for day in range(sample_days):
            ...
            for machine in random.sample(self.machines, min(50, len(self.machines))):
                for hour in range(24):
                    ...
                    theoretical_output = (operating_time * 60) / ideal_cycle_time if ideal_cycle_time > 0 else 0
                    ...
                    quality = (good_pieces / actual_output * 100) if actual_output > 0 else 0
                    ...
                    machine_sensors = [s for s in self.sensors if s['machine_id'] == machine['machine_id']]
                    ...
                    for sensor in machine_sensors:
                        ...
                        for minute in range(0, 60, 5):
                            ...
                            value = self.calculate_sensor_value(sensor, machine, operating_time > 0)
                            ...
                            reading = {
                                ...
                                'value': round(value, 4),
                                'quality': 'good' if random.random() > 0.05 else random.choice(['uncertain', 'bad'])
                            }
    ...
    def calculate_sensor_value(self, sensor, machine, is_running):
        ...
        if sensor['sensor_type'] == 'temperature':
            ...
            if is_running:
                ...
                nominal_speed = machine['max_capacity_per_hour'] * 10  # Rough RPM estimate
    ...
    def generate_downtime_events(self):
        ...
        reason_categories = [
            'equipment_failure', 'changeover', 'material_shortage',
            'quality_issue', 'no_operator', 'planned_maintenance',
            'unplanned_maintenance', 'other'
        ]

        # Generate downtime events based on OEE data
        for oee in random.sample(self.oee_metrics, min(len(self.oee_metrics) // 10, 500)):
            if oee['downtime_min'] > 5:  # Only create event for significant downtime
                self.event_id += 1

                machine = next(m for m in self.machines if m['machine_id'] == oee['machine_id'])

                event = {
                    'event_id': self.event_id,
                    'machine_id': oee['machine_id'],
                    'line_id': oee['line_id'],
                    'start_time': oee['hour_start'] + timedelta(minutes=random.randint(0, 50)),
                    'end_time': oee['hour_start'] + timedelta(minutes=60 - oee['downtime_min']),
                    'duration_minutes': oee['downtime_min'],
                    'reason_category': random.choice(reason_categories),
                    'reason_detail': fake.sentence(),
                    'impact_level': random.choice(['line_stop', 'reduced_speed', 'quality_impact']),
                    'lost_production_units': int(oee['downtime_min'] / machine['ideal_cycle_time_seconds'] * 60),
    ...
    def generate_alerts(self):
        ...
        alert_configs = [
            {'source': 'sensor', 'type': 'temperature_high', 'severity': 'high', 'message': 'Temperature exceeds threshold'},
            {'source': 'sensor', 'type': 'vibration_high', 'severity': 'medium', 'message': 'Abnormal vibration detected'},
            {'source': 'machine', 'type': 'oee_low', 'severity': 'low', 'message': 'OEE below target'},
            {'source': 'line', 'type': 'production_delay', 'severity': 'medium', 'message': 'Production behind schedule'},
            {'source': 'quality', 'type': 'defect_rate_high', 'severity': 'high', 'message': 'Quality issues detected'},
            {'source': 'maintenance', 'type': 'maintenance_due', 'severity': 'low', 'message': 'Scheduled maintenance due'},
        ]
        ...
        for _ in range(random.randint(100, 500)):
            ...
            if config['source'] == 'sensor':
                ...
                source_id = random.choice(self.quality_inspections)['inspection_id'] if self.quality_inspections else 1
            else:  # maintenance
                source_id = random.choice(self.maintenance_schedules)['schedule_id'] if self.maintenance_schedules else 1

            triggered_time = fake.date_time_between(start_date=self.start_date, end_date='now')
            ...
            alert = {
                ...
                'acknowledged_at': triggered_time + timedelta(minutes=random.randint(5, 60)) if random.random() > 0.2 else None,
                'acknowledged_by': random.randint(1, 10) if random.random() > 0.2 else None,
                'resolved_at': triggered_time + timedelta(hours=random.randint(1, 24)) if random.random() > 0.3 else None,
    ...
    def generate_summary(self):
        ...
        avg_oee = sum(m['oee_percentage'] for m in self.oee_metrics) / len(self.oee_metrics) if self.oee_metrics else 0
