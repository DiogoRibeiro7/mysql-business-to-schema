...
class IndustrialIoTGenerator:
    ...
    def generate_all(self):
        ...
        print(
            f"[OK] Generated data for {len(self.factories)} factories, "
    ...
    def _generate_production_lines(self):
        ...
        for factory in self.factories:
            lines_per_factory = self.config["counts"]["production_lines"] // len(
                self.factories
            )
            ...
            for line_num in range(lines_per_factory):
                line = {
                    ...
                    "line_type": random.choice(
                        ["assembly", "machining", "packaging", "quality"]
                    ),
    ...
    def _generate_machines(self):
        ...
        for line in self.production_lines:
            machines_per_line = self.config["counts"]["machines"] // len(
            ...
            for _ in range(machines_per_line):
                ...
                machine = {
                    ...
                    "manufacturer": random.choice(
                        ["Siemens", "ABB", "Fanuc", "Kuka", "Mitsubishi"]
                    ),
    ...
    def _generate_sensors(self):
        ...
        for machine in self.machines:
            ...
            for sensor_type in list(self.config["sensor_types"].keys())[
                : self.config["counts"]["sensors_per_machine"]
            ]:
                ...
                sensor = {
                    ...
                    "status": random.choices(
                        ["active", "inactive", "faulty"], weights=[0.95, 0.03, 0.02]
                    )[0],
                }
    ...
    def _generate_operators(self):
        ...
        for _ in range(self.config["counts"]["operators"]):
            ...
            operator = {
                ...
                "safety_certified": random.random()
                < self.config["operator_skills"]["certifications"]["safety"],
                "quality_certified": random.random()
                < self.config["operator_skills"]["certifications"]["quality"],
                "maintenance_certified": random.random()
                < self.config["operator_skills"]["certifications"]["maintenance"],
                "programming_certified": random.random()
                < self.config["operator_skills"]["certifications"]["programming"],
                "error_rate": self.config["operator_skills"]["error_rates"][
                    skill_level
                ],
                "status": random.choices(
                    ["active", "on_leave", "training"], weights=[0.90, 0.05, 0.05]
                )[0],
            }
    ...
    def _generate_products(self):
        ...
        for _ in range(self.config["counts"]["products"]):
            complexity = random.choices(
                ...
                weights=list(
                    self.config["product_specifications"]["complexity"].values()
                ),
            )[0]

            # Determine tolerance based on complexity
            if complexity == "advanced":
                tolerance = self.config["product_specifications"]["tolerances"][
                    "ultra_precision"
                ]
            ...
            product = {
                ...
                "product_family": random.choice(
                    ["automotive", "electronics", "consumer", "industrial"]
                ),
                "complexity": complexity,
                "tolerance_mm": tolerance,
                "cycle_time_seconds": random.randint(30, 600),
                "material": random.choice(
                    ["steel", "aluminum", "plastic", "composite"]
                ),
    ...
    def _generate_production_runs(self):
        ...
        for work_order in self.work_orders:
            ...
            actual_start = work_order["scheduled_start"] + timedelta(
            ...
            availability = random.uniform(
                shift_config["availability"][0], shift_config["availability"][1]
            )
            performance = random.uniform(
                shift_config["performance"][0], shift_config["performance"][1]
            )
            quality = random.uniform(
                shift_config["quality"][0], shift_config["quality"][1]
            )
    ...
    def _generate_sensor_readings(self):
        ...
        for sensor in active_sensors:
            ...
            current_value = random.uniform(
                sensor_config["normal_range"][0], sensor_config["normal_range"][1]
            )
            ...
            while current_time <= end_date and readings_count < max_readings_per_sensor:
                ...
                machine = next(
                    m for m in self.machines if m["machine_id"] == sensor["machine_id"]
                )
                ...
                if machine["status"] == "running":
                    ...
                    current_value = max(
                        sensor_config["normal_range"][0],
                        min(
                            sensor_config["normal_range"][1], current_value + variation
                        ),
                    )
                    ...
                    if (
                        random.random()
                        < self.config["anomaly_patterns"]["sensor_drift"]
                    ):
                    ...
                    reading = {
                        ...
                        "quality": (
                            "good"
                            if sensor_config["normal_range"][0]
                            <= current_value
                            <= sensor_config["normal_range"][1]
                            else "warning"
                        ),
        ...
        for machine in self.machines[:20]:  # Limit for performance
            ...
            while current_time <= end_date:
                ...
                state_entry = {
                    ...
                    "operator_id": (
                        random.choice(self.operators)["operator_id"]
                        if current_state == "running"
                        else None
                    ),
                    "work_order_id": (
                        random.choice(self.work_orders)["work_order_id"]
                        if current_state == "running"
                        else None
                    ),
        ...
        for production_run in self.production_runs:
            ...
            for _ in range(num_inspections):
                ...
                product = next(
                    ...
                    if p["product_id"] == production_run["product_id"]
                )
                defect_rate = self.config["quality_defects"]["defect_rates"][
                ...
                inspection = {
                    ...
                    "inspection_time": production_run["start_time"]
                    ...
                    "defect_type": (
                        random.choices(
                            ...
                            weights=[
                                self.config["quality_defects"][k]
                                for k in list(self.config["quality_defects"].keys())[:5]
                            ],
                        )[0]
                        if has_defect
                        else None
            ...
            for _ in range(num_events):
                ...
                if downtime_type == "planned":
                    reason = random.choices(
                        ...
                        weights=list(
                            self.config["downtime_reasons"]["planned"].values()
                        ),
                    )[0]
                else:
                    reason = random.choices(
                        ...
                        weights=list(
                            self.config["downtime_reasons"]["unplanned"].values()
                        ),
                    )[0]
        ...
        for line in self.production_lines:
            ...
            while current_date <= end_date:
                for shift in ["morning", "afternoon", "night"]:
                    shift_config = self.config["production_metrics"][
                        "shift_performance"
                    ][shift]
                    ...
                    metric = {
                        ...
                        "target_oee": self.config["production_metrics"][
                            "availability_target"
                        ]
    ...
    def _generate_maintenance_records(self):
        ...
        for machine in self.machines:
            ...
            maintenance_type = random.choices(
                ...
                weights=list(
                    self.config["maintenance_patterns"]["maintenance_types"].values()
                ),
            )[0]
            ...
            maintenance = {
                ...
                "actual_date": machine["next_maintenance"]
                ...
                "parts_replaced": (
                    "Various components" if random.random() < 0.3 else None
                ),
                "cost": round(random.uniform(100, 5000), 2),
                "next_maintenance_due": machine["next_maintenance"]
                + timedelta(days=30),
        ...
        for sensor in self.sensors[:50]:  # Limit for performance
            ...
            for _ in range(num_alarms):
                severity = random.choices(
                    ...
                    weights=list(
                        self.config["alarm_patterns"]["severity_distribution"].values()
                    ),
                )[0]

                response_range = self.config["alarm_patterns"]["response_times"][
                    severity
                ]
                ...
                alarm = {
                    ...
                    + timedelta(minutes=response_time + random.uniform(5, 60)),
                    "threshold_value": sensor["critical_value"],
                    "actual_value": sensor["critical_value"]
                    * random.uniform(1.01, 1.5),
                    "operator_id": random.choice(self.operators)["operator_id"],
    ...
    def _generate_energy_consumption(self):
        ...
        for machine in self.machines[:5]:  # Only 5 machines for performance
            ...
            consumption_range = self.config["energy_consumption"][
                "machine_consumption"
            ].get(machine_type, [5, 15])
            ...
            while current_time <= end_date:
                ...
                if is_running:
                    base_consumption = random.uniform(
                        consumption_range[0], consumption_range[1]
                    )
    ...
    def _get_next_state(self, current_state: str) -> str:
        ...
        return random.choices(
            transitions[current_state], weights=weights[current_state]
        )[0]
