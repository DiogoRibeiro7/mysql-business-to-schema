...
class HotelChainGenerator(BaseGenerator):
    ...
    def __init__(self, host='localhost', port=3340, user='hotel_admin',
                 password='hotel_pass_2024', database='hotel_chain'):
        """Initialize the hotel chain generator"""
        super().__init__(host, port, user, password, database)

        # Hotel specific data
        self.property_types = ['hotel', 'resort', 'boutique', 'airport', 'extended_stay', 'conference_center']

        self.room_categories = ['standard', 'deluxe', 'suite', 'executive', 'presidential']

        self.bed_configurations = [
            'Single', 'Double', 'Queen', 'King', 'Twin', 'King + Sofa Bed'
        ]

        self.view_types = ['city', 'ocean', 'garden', 'pool', 'mountain', 'courtyard']

        self.booking_channels = ['direct', 'website', 'mobile_app', 'call_center', 'walk_in',
                                'ota_booking', 'ota_expedia', 'corporate', 'group', 'travel_agent']

        self.vip_status_levels = ['none', 'silver', 'gold', 'platinum', 'diamond', 'invitation_only']

        self.departments = ['front_office', 'housekeeping', 'food_beverage', 'maintenance',
                           'security', 'spa', 'concierge', 'management', 'accounting', 'hr']

        self.amenities = {
            'property': ['pool', 'gym', 'spa', 'restaurant', 'bar', 'parking', 'wifi',
                        'business_center', 'conference_rooms', 'concierge', 'room_service'],
            'room': ['wifi', 'tv', 'minibar', 'safe', 'coffee_maker', 'balcony',
                    'bathtub', 'air_conditioning', 'desk', 'refrigerator']
        }

        self.brand_names = ['Luxury Hotels', 'Comfort Suites', 'Business Express',
                           'Family Resorts', 'Urban Boutique', 'Beach Paradise']

    def generate_all_data(self, properties: int = 10, rooms_per_property: int = 200,
                         guests: int = 10000, reservations_per_day: int = 500):
    ...
    def generate_properties(self, count: int = 10):
        ...
        for i in range(min(count, len(cities))):
            ...
            property_data = (
                ...
                random.randint(2018, 2023) if random.random() > 0.5 else None,  # last_renovation
                '15:00:00',  # check_in_time
                '11:00:00',  # check_out_time
                json.dumps(random.sample(self.amenities['property'], random.randint(5, 10))),
                location,
                'active',
        ...
        self.bulk_insert('properties', properties, [
            'property_name', 'property_type', 'brand_name', 'address',
            'city', 'state_province', 'postal_code', 'country',
            'phone', 'email', 'website', 'star_rating', 'total_rooms',
            'total_floors', 'year_built', 'last_renovation',
            'check_in_time', 'check_out_time', 'amenities',
            'location_coordinates', 'status', 'general_manager_id',
        ...
        for property in properties:
            ...
            for category in self.room_categories:
                ...
                room_type = (
                    ...
                    base_rates[category],
                    random.uniform(200, 500) if category != 'standard' else random.uniform(150, 250),  # size_sqft
                    json.dumps(random.sample(self.amenities['room'], random.randint(5, 10))),
                    random.choice([0, 1]),  # is_active
                    self.faker.date_time_between('-5 years', 'now')
            )
                room_types.append(room_type)

        self.bulk_insert('room_types', room_types, [
            'property_id', 'type_name', 'type_code', 'category',
            'description', 'max_occupancy', 'standard_occupancy',
            'extra_bed_allowed', 'base_rate', 'size_sqft',
            ...
            for floor in range(1, min(total_floors + 1, 21)):  # Max 20 floors
                ...
                for room_num in range(1, rooms_on_floor + 1):
                    ...
                    room = (
                        ...
                        room_number,
                        floor,
                        random.choice(['North', 'South', 'East', 'West']) if floor > 5 else None,
                        random.choice(self.view_types),
                        json.dumps(random.choice(self.bed_configurations)),
                        random.choice([0, 1]) if room_type['category'] == 'standard' else 0,  # is_accessible
                        ...
                        self.faker.date_between('-5 years', '-1 year') if random.random() > 0.5 else None,
                        self.faker.date_time_between('-5 years', 'now')
                    )
                    rooms.append(room)

                    if len(rooms) >= 1000:
                        self.bulk_insert('rooms', rooms, [
                            'property_id', 'room_type_id', 'room_number', 'floor_number',
                            'building_section', 'view_type', 'bed_configuration',
                            'is_accessible', 'is_smoking', 'status', 'housekeeping_status',
                            'last_cleaned', 'maintenance_status', 'last_renovated', 'created_at'
                        ])
                        rooms = []
    ...
    def generate_guests(self, count: int = 10000):
        ...
        for i in range(count):
            ...
            preferences = {
                'room': random.choice(['high_floor', 'low_floor', 'quiet', 'near_elevator']),
            ...
            guest = (
                ...
                random.choice([None, 'vegetarian', 'vegan', 'gluten_free', 'halal', 'kosher']),
                random.randint(1, 10) if random.random() > 0.3 else None,  # loyalty_member_id
                random.choice([0, 1]),  # marketing_consent
                random.choice([0, 1]) if vip_status == 'none' and random.random() > 0.95 else 0,  # blacklisted
                ...
                self.faker.date_between('-1 year', 'today') if random.random() > 0.3 else None,
                self.faker.date_time_between('-5 years', 'now'),
                self.faker.date_time_between('-5 years', 'now')
            )
            guests.append(guest)
            ...
            if (i + 1) % 1000 == 0:
                self.bulk_insert('guests', guests, [
                    'first_name', 'last_name', 'email', 'phone_number',
                    'alternate_phone', 'address', 'city', 'state_province',
                    'postal_code', 'country', 'date_of_birth', 'nationality',
                    'passport_number', 'identity_document', 'company_name',
                    'vip_status', 'preferences', 'dietary_restrictions',
                    'loyalty_member_id', 'marketing_consent', 'blacklisted',
                    'blacklist_reason', 'total_stays', 'total_spent',
    ...
    def generate_loyalty_members(self):
        ...
        for guest in guests:
            ...
            member = (
                ...
                tier_requirements.get(tier, {}).get('points', 999999) - points,  # next_tier_points
                self.faker.date_between('-5 years', '-1 year'),  # member_since
                self.faker.date_between('+6 months', '+1 year') if tier != 'member' else None,  # tier_expiry
                self.faker.date_between('+1 year', '+2 years') if points > 0 else None,  # points_expiry
                random.choice([0, 1]),  # is_active
                self.faker.date_time_between('-5 years', 'now')
            )
            loyalty_members.append(member)

        self.bulk_insert('loyalty_members', loyalty_members, [
            'guest_id', 'member_number', 'tier_level', 'current_points',
            'lifetime_points', 'ytd_points', 'ytd_nights', 'lifetime_nights',
            'tier_qualifying_points', 'next_tier_points', 'member_since',
            'tier_expiry_date', 'points_expiry_date', 'is_active', 'created_at'
        ])
        ...
        for property in properties:
            ...
            for dept in dept_distribution.keys():
                manager = (
                    ...
                    self.faker.phone_number(),
                    f"{random.randint(100, 999)} {self.faker.street_name()}",
                    self.faker.first_name() + ' ' + self.faker.last_name(),  # emergency_contact_name
            ...
            for _ in range(num_staff - len(managers)):
                ...
                positions = {
                    'front_office': ['Front Desk Agent', 'Night Auditor', 'Reservations Agent'],
                    'housekeeping': ['Room Attendant', 'Housekeeping Supervisor', 'Laundry Attendant'],
                    'food_beverage': ['Server', 'Bartender', 'Cook', 'Chef'],
                    'maintenance': ['Maintenance Technician', 'HVAC Specialist', 'Electrician'],
                ...
                employee = (
                    ...
                    None if random.random() > 0.1 else self.faker.date_between('-1 year', 'today'),
                ...
                if len(staff) >= 500:
                    self.bulk_insert('staff', staff, [
                        'property_id', 'employee_id', 'first_name', 'last_name',
                        'email', 'phone', 'address', 'emergency_contact_name',
                        'emergency_contact_phone', 'department', 'position_title',
                        'manager_id', 'hire_date', 'termination_date',
                        'employment_status', 'employment_type', 'salary', 'created_at'
                    ])
                    staff = []
    ...
    def generate_reservations(self, reservations_per_day: int = 500):
        ...
        for days_offset in range(-60, 31):
            ...
            daily_reservations = random.randint(int(reservations_per_day * 0.7), int(reservations_per_day * 1.3))
            ...
            for _ in range(daily_reservations):
                ...
                if check_in_date < date.today() - timedelta(days=1):
                    status = random.choice(['completed', 'checked_out', 'no_show', 'cancelled'])
                elif check_in_date == date.today():
                    status = random.choice(['confirmed', 'checked_in'])
                else:
                    status = random.choice(['pending', 'confirmed', 'guaranteed', 'cancelled'])
                ...
                reservation = (
                    ...
                    check_out_date,
                    nights,
                    random.randint(1, min(4, room_type['max_occupancy'])),  # adults_count
                    random.randint(0, 2) if room_type['max_occupancy'] > 2 else 0,  # children_count
                    room_type['room_type_id'],
                    f"RATE{random.randint(100, 999)}",  # rate_code
                    room_rate,
                    total_amount,
                    total_amount * 0.2 if status in ['confirmed', 'guaranteed'] else None,  # deposit_amount
                    status,
                    f"{random.randint(14, 18)}:00:00",  # arrival_time
                    f"{random.randint(9, 11)}:00:00",  # departure_time
                    self.faker.sentence() if random.random() > 0.7 else None,  # special_requests
                    None,  # internal_notes
                    random.randint(1, 100) if random.random() > 0.9 else None,  # group_booking_id
                    random.choice([0, 1]) if random.random() > 0.8 else 0,  # is_corporate
                    f"CORP{random.randint(100, 999)}" if random.random() > 0.9 else None,  # corporate_id
                    random.randint(1, 50) if random.random() > 0.7 else None,  # agent_id
                    None,  # broker_id
                    self.faker.date_time_between(check_in_date - timedelta(days=30), check_in_date) if status == 'cancelled' else None,
                ...
                if len(reservations) >= 1000:
                    self.bulk_insert('reservations', reservations, [
                        'confirmation_number', 'property_id', 'guest_id', 'booking_channel',
                        'check_in_date', 'check_out_date', 'room_nights', 'adults_count',
                        'children_count', 'room_type_id', 'rate_code', 'room_rate',
                        'total_amount', 'deposit_amount', 'status', 'arrival_time',
                        'departure_time', 'special_requests', 'internal_notes',
                        'group_booking_id', 'is_corporate', 'corporate_id',
                        'agent_id', 'broker_id', 'cancellation_date',
        ...
        for reservation in reservations:
            ...
            if key in rooms_by_property_type and rooms_by_property_type[key]:
                ...
                if reservation['check_out_date'] <= date.today():
                    ...
                    status = 'occupied' if reservation['check_in_date'] <= date.today() else 'reserved'
                ...
                assignment = (
                    ...
                    status,
                    random.choice([0, 1]) if random.random() > 0.9 else 0,  # early_check_in
                    random.choice([0, 1]) if random.random() > 0.9 else 0,  # late_check_out
                    None,  # upgraded_from
                    random.randint(1, 50) if random.random() > 0.5 else None,  # assigned_by
                    self.faker.date_time_between('-60 days', 'now')
                )
                assignments.append(assignment)

        self.bulk_insert('room_assignments', assignments, [
            'reservation_id', 'room_id', 'check_in_date', 'check_out_date',
            'actual_check_in', 'actual_check_out', 'adults_count',
            'children_count', 'status', 'early_check_in', 'late_check_out',
            'upgraded_from', 'assigned_by', 'created_at'
        ])
    ...
    def generate_folios_and_charges(self):
        ...
        for reservation in reservations:
            ...
            folio_record = (
                ...
                self.faker.date_time_between('-60 days', 'now'),
                self.faker.date_time_between('-30 days', 'now') if status == 'closed' else None
            )
            ...
            if len(folios) >= 500:
                self.bulk_insert('folios', folios, [
                    'folio_number', 'property_id', 'reservation_id', 'guest_id',
                    'status', 'total_charges', 'total_payments', 'balance_due',
                    'tax_amount', 'currency', 'master_folio_id', 'created_at',
                    'closed_at'
                ])
                folios = []

            if len(charges) >= 1000:
                self.bulk_insert('folio_charges', charges, [
                    'folio_id', 'charge_type', 'description', 'quantity',
                    'unit_price', 'amount', 'tax_amount', 'department',
                    'reference', 'posted_date', 'posted_by', 'voided_at',
                    'void_reason'
                ])
                charges = []
    ...
    def generate_payments(self):
        ...
        for folio in folios:
            ...
            for i in range(num_payments):
                ...
                payment = (
                    folio['folio_id'],
                    random.choice(['credit_card', 'debit_card', 'cash', 'bank_transfer', 'mobile_payment']),
                    amount,
                    'USD',
                    f"TXN{random.randint(100000, 999999)}",
                    f"AUTH{random.randint(100000, 999999)}",
                    str(random.randint(1000, 9999)) if random.random() > 0.5 else None,  # card_last_four
        ...
        self.bulk_insert('payments', payments, [
            'folio_id', 'payment_method', 'amount', 'currency',
            'reference_number', 'authorization_code', 'card_last_four',
            'payment_date', 'processed_by', 'is_processed', 'refunded_amount'
        ])
    ...
    def generate_loyalty_transactions(self):
        ...
        for member in members:
            ...
            for folio in guest_folios[:5]:  # Limit to 5 transactions per member
                points_earned = int(float(folio['total_charges']) * 10)  # 10 points per dollar
            ...
            if member['current_points'] > 10000 and random.random() > 0.5:
                points_redeemed = random.randint(5000, min(20000, member['current_points']))
        ...
        self.bulk_insert('loyalty_transactions', transactions, [
            'member_id', 'transaction_type', 'points', 'description',
            'folio_id', 'expiry_date', 'transaction_date'
        ])
    ...
    def generate_housekeeping_tasks(self):
        ...
        for room in rooms:
            ...
            task_type = 'daily_cleaning' if room['housekeeping_status'] != 'clean' else 'inspection'
            ...
            task = (
                ...
                self.faker.date_time_between('-8 hours', 'now') if random.random() > 0.5 else None,
                self.faker.date_time_between('-4 hours', 'now') if random.random() > 0.3 else None,
                random.choice(housekeepers_list) if housekeepers_list and random.random() > 0.7 else None,
                self.faker.date_time_between('-2 hours', 'now') if random.random() > 0.5 else None,
                None,  # notes
                json.dumps({'towels': 2, 'sheets': 1, 'toiletries': 1}) if random.random() > 0.5 else None,
                self.faker.date_time_between('-24 hours', 'now')
            )
            tasks.append(task)

        self.bulk_insert('housekeeping_tasks', tasks, [
            'room_id', 'task_type', 'priority', 'status', 'scheduled_date',
            'scheduled_time', 'assigned_to', 'started_at', 'completed_at',
            'inspected_by', 'inspected_at', 'notes', 'supplies_used', 'created_at'
        ])
    ...
    def generate_maintenance_requests(self):
        ...
        issue_types = ['plumbing', 'electrical', 'hvac', 'appliance', 'structural', 'cosmetic', 'other']
        ...
        for room in rooms:
            ...
            request = (
                ...
                random.choice(maintenance_list) if maintenance_list else None,  # reported_by
                self.faker.date_time_between('-30 days', 'now'),
                random.choice(maintenance_list) if maintenance_list and random.random() > 0.5 else None,
                ...
                self.faker.date_time_between('-7 days', 'now') if random.random() > 0.5 else None,
                self.faker.date_time_between('-30 days', 'now')
            )
        ...
        self.bulk_insert('maintenance_requests', requests, [
            'room_id', 'property_id', 'issue_type', 'description', 'priority',
            'status', 'reported_by', 'reported_date', 'assigned_to',
            'estimated_cost', 'actual_cost', 'parts_required',
            'resolution_notes', 'completed_at', 'created_at'
        ])
    ...
    def generate_guest_services(self):
        ...
        # Guest requests
        guest_requests = []
        request_types = ['housekeeping', 'concierge', 'maintenance', 'room_service', 'amenities', 'other']
        ...
        self.bulk_insert('guest_requests', guest_requests, [
            'guest_id', 'reservation_id', 'request_type', 'request_details',
            'priority', 'status', 'assigned_to', 'requested_at',
            'completed_at', 'resolution_notes', 'satisfaction_rating', 'created_at'
        ])
    ...
    def generate_rate_plans(self):
        ...
        for room_type in room_types:
            ...
            selected_plans = random.sample(plan_types, random.randint(2, min(4, len(plan_types))))
            ...
            for plan_name, plan_code, rate_multiplier in selected_plans:
                ...
                rate_plan = (
                    ...
                    base_rate * rate_multiplier * 1.2 if plan_code == 'WKD' else None,  # weekend_rate
                    ...
                    '48 hours' if plan_code == 'ADV' else '24 hours',  # cancellation_policy
                    json.dumps(['breakfast']) if plan_code == 'PKG' else None,  # inclusions
        ...
        self.bulk_insert('rate_plans', rate_plans, [
            'property_id', 'room_type_id', 'plan_name', 'plan_code',
            'description', 'base_rate', 'weekend_rate', 'minimum_stay',
            'maximum_stay', 'advance_booking_days', 'cancellation_policy',
            'inclusions', 'valid_from', 'valid_to', 'is_active', 'created_at'
        ])
        ...
        for property in properties[:5]:
            ...
            for _ in range(10):
                ...
                property_room_types = [rt for rt in room_types if rt['property_id'] == property['property_id']]
                ...
                if property_room_types:
                    ...
                    override = (
                        ...
                        random.choice(['Holiday', 'Special Event', 'Peak Season', 'Convention']),
        ...
        self.bulk_insert('rate_overrides', rate_overrides, [
            'property_id', 'room_type_id', 'override_date', 'override_rate',
            'reason', 'minimum_stay', 'closed_to_arrival', 'closed_to_departure',
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate test data for Hotel Chain Management')
    ...
    parser.add_argument('--properties', type=int, default=10, help='Number of properties')
    parser.add_argument('--rooms-per-property', type=int, default=200, help='Rooms per property')
    parser.add_argument('--guests', type=int, default=10000, help='Number of guests')
    parser.add_argument('--reservations-per-day', type=int, default=500, help='Average reservations per day')
