...
SCHEMA_CONFIGS = {
    'clinic_db': {
        'tables': ['patients', 'doctors', 'appointments', 'prescriptions', 'medical_records'],
        'relationships': True,
        'generator': 'clinic'
    },
    'iot_bins_db': {
...
class DataGenerator:
    ...
    def generate_iot_sensors(self, count: int) -> List[Dict]:
        ...
        sensors = []
        sensor_types = ['temperature', 'humidity', 'pressure', 'motion', 'light', 'sound', 'vibration']
        for i in range(count):
            sensors.append({
                ...
                'installation_date': self.fake.date_between(start_date='-3y', end_date='-1m'),
                'status': random.choice(['active', 'inactive', 'maintenance']),
                'last_reading': self.fake.date_time_between(start_date='-1d', end_date='now'),
...
def generate_schema_data(schema_name: str, config: Dict, records_per_table: int) -> Dict[str, int]:
    ...
    try:
        ...
        if 'clinic' in schema_name or 'healthcare' in schema_name:
            ...
            if transactions:
                results['transactions'] = generator.insert_batch('transactions', transactions)
            ...
            if table not in results:
                ...
                for _ in range(min(records_per_table, 100)):  # Limit generic data
                    generic_data.append({
                        ...
                        'created_at': generator.fake.date_time_between(start_date='-1y', end_date='now'),
...
def print_summary(results: Dict[str, Dict[str, int]]):
    ...
    print(f"{Fore.GREEN}Successful Schemas: {successful_schemas}/{len(results)}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}{'='*60}{Style.RESET_ALL}\n")
...
def main():
    ...
    parser.add_argument('--parallel', action='store_true',
                       help='Generate data in parallel')
    ...
    results_file = f"data_generation_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
