...
class HealthcareIoTGenerator:
    ...
    def generate_all(self):
        ...
        print(
            f"[OK] Generated data for {len(self.hospitals)} hospitals, "
    ...
    def _generate_departments(self):
        ...
        for hospital in self.hospitals:
            depts_per_hospital = self.config["counts"]["departments"] // len(
                self.hospitals
            )
            ...
            for i in range(min(depts_per_hospital, len(self.config["departments"]))):
                department = {
                    ...
                    "is_critical_care": self.config["departments"][i]
                    in ["ICU", "Emergency"],
                    "nurse_ratio": (
                        "1:2" if self.config["departments"][i] == "ICU" else "1:4"
                    ),
                }
    ...
    def _generate_patients(self):
        ...
        for _ in range(self.config["counts"]["patients"]):
            ...
            patient = {
                ...
                "discharge_date": (
                    admission_date + timedelta(days=random.randint(1, 14))
    ...
    def _generate_device_assignments(self):
        ...
        for patient in self.patients:
            ...
            assigned_devices = random.sample(
                self.devices, min(num_devices, len(self.devices))
            )
        ...
        for patient in recent_patients:
            ...
            while current_time <= end_date and reading_count < max_readings:
                ...
                vital = {
                    ...
                    "heart_rate": int(
                        random.gauss(sum(normal_ranges["heart_rate"]) / 2, hr_variation)
                    ),
                    "respiratory_rate": int(
                        random.gauss(sum(normal_ranges["respiratory_rate"]) / 2, 5)
                    ),
        ...
        for assignment in recent_assignments:
            ...
            for _ in range(10):  # Limited readings per device
                ...
                for parameter in device_config.get("parameters", []):
                    thresholds = device_config.get("critical_thresholds", {}).get(
                        parameter, [0, 100]
                    )
                    ...
                    reading = {
                        ...
                        "quality": (
                            "good" if device["status"] == "active" else "questionable"
                        ),
                    }
        ...
        while current_date <= end_date:
            ...
            for _ in range(daily_alerts):
                ...
                alert = {
                    ...
                    "acknowledged_by": (
                        random.choice(self.medical_staff)["staff_id"]
                        if random.random() > 0.1
                        else None
                    ),
                    "acknowledged_at": (
                        triggered_at + timedelta(minutes=random.randint(1, 30))
                        if random.random() > 0.1
                        else None
                    ),
                    "resolved_at": (
    ...
    def _generate_medications(self):
        ...
        for patient in self.patients[:30]:  # Limit for performance
            ...
            patient_meds = random.sample(
                self.config["medications"],
                min(num_meds, len(self.config["medications"])),
            )
            ...
            for med_config in patient_meds:
                ...
                for day in range(min(3, self.config["counts"]["days_of_data"])):
                    ...
                    medication = {
                        ...
                        "dose": random.uniform(
                            med_config["typical_dose"][0], med_config["typical_dose"][1]
                        ),
                        ...
                        "administered_by": (
                            random.choice(
                                [s for s in self.medical_staff if s["role"] == "nurse"]
                            )["staff_id"]
                            if any(s["role"] == "nurse" for s in self.medical_staff)
                            else 1
                        ),
                        "notes": None,
                    }
    ...
    def _generate_clinical_scores(self):
        ...
        for vital in self.vital_signs[:100]:  # Limit for performance
            ...
            score = {
                ...
                "components": {
                    "heart_rate": self._get_mews_component(
                        "heart_rate", vital["heart_rate"]
                    ),
                    "respiratory_rate": self._get_mews_component(
                        "respiratory_rate", vital["respiratory_rate"]
                    ),
                    "systolic_bp": self._get_mews_component(
                        "systolic_bp", vital["systolic_bp"]
                    ),
                    "temperature": self._get_mews_component(
                ...
                "risk_level": (
                    "high"
                    if mews_score >= 5
                    else "medium" if mews_score >= 3 else "low"
                ),
            }
        ...
        for _ in range(100):  # Limited audit entries
            audit = {
                ...
                "resource": random.choice(
                    ["vital_signs", "medications", "lab_results", "notes"]
                ),
                "ip_address": f"192.168.{random.randint(1, 254)}.{random.randint(1, 254)}",
                "success": random.random() > 0.02,
                "details": (
                    "Access granted" if random.random() > 0.02 else "Access denied"
                ),
            }
