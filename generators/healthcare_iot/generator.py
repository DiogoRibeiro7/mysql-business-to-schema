...
class HealthcareIoTGenerator:
    ...
    def generate_all(self):
        ...
        print(f"  Total departments: {CONFIG['hospitals'] * CONFIG['departments_per_hospital']}")
        print(f"  Total rooms: {CONFIG['hospitals'] * CONFIG['departments_per_hospital'] * CONFIG['rooms_per_department']}")
    ...
    def generate_departments_and_rooms(self):
        ...
        department_types = ['ICU', 'Emergency', 'Surgery', 'Pediatrics', 'Cardiology',
                          'Neurology', 'Oncology', 'Maternity', 'General']
            ...
            for dept_num in range(CONFIG['departments_per_hospital']):
                ...
                for room_num in range(CONFIG['rooms_per_department']):
                    ...
                    room = {
                        ...
                        'has_monitoring': dept_type in ['ICU', 'Emergency', 'Cardiology'],
                        'equipment_list': json.dumps(['Bed', 'Monitor', 'IV Stand', 'Oxygen']),
    ...
    def generate_staff(self):
        ...
        specializations = {
            'Doctor': ['Cardiologist', 'Neurologist', 'Surgeon', 'Pediatrician', 'Internist', 'Emergency'],
            'Nurse': ['RN', 'LPN', 'Critical Care', 'Pediatric', 'Emergency'],
            'Technician': ['Radiology', 'Laboratory', 'Respiratory', 'EKG', 'Phlebotomy']
        }
        ...
        for department in self.departments:
            for staff_num in range(CONFIG['staff_per_department']):
                ...
                staff_member = {
                    ...
                    'license_number': fake.bothify(text='LIC########') if role in ['Doctor', 'Nurse'] else None,
                    'license_expiry': fake.date_between(start_date='+1y', end_date='+5y') if role in ['Doctor', 'Nurse'] else None,
    ...
    def generate_patients(self):
        ...
        chronic_conditions = ['Diabetes', 'Hypertension', 'Asthma', 'COPD', 'Heart Disease',
                             'Arthritis', 'Cancer', 'Kidney Disease', 'None']
        ...
        for i in range(CONFIG['patients']):
            ...
            patient = {
                ...
                'emergency_contact_relation': random.choice(['Spouse', 'Parent', 'Child', 'Sibling', 'Friend']),
                'insurance_provider': random.choice(['BlueCross', 'Aetna', 'UnitedHealth', 'Cigna', 'Medicare']),
                'insurance_id': fake.bothify(text='INS#########'),
                'primary_physician_id': random.choice([s['staff_id'] for s in self.staff if s['role'] == 'Doctor']),
                'allergies': json.dumps(random.sample(allergies_list, k=random.randint(0, 2))),
                'chronic_conditions': json.dumps(random.sample(chronic_conditions, k=random.randint(0, 3))),
    ...
    def generate_admissions(self):
        ...
        chief_complaints = [
            ...
            'Injury', 'Surgery', 'Cardiac event', 'Stroke symptoms',
            'Fracture', 'Infection', 'Chronic condition management'
        ]
        ...
        for patient in admitted_patients:
            ...
            for _ in range(num_admissions):
                ...
                admission_date = fake.date_time_between(start_date=self.start_date, end_date='now')
                stay_length = random.randint(1, 14)  # 1-14 days
                discharge_date = admission_date + timedelta(days=stay_length) if random.random() > 0.2 else None

                department = random.choice(self.departments)
                room = random.choice([r for r in self.rooms if r['department_id'] == department['department_id']])
                ...
                admission = {
                    ...
                    'admission_source': random.choice(['Emergency Room', 'Physician Referral', 'Transfer']),
                    'chief_complaint': random.choice(chief_complaints),
                    'diagnosis_codes': json.dumps([f"ICD-{random.randint(100, 999)}" for _ in range(random.randint(1, 3))]),
                    'attending_physician_id': random.choice([s['staff_id'] for s in self.staff
                                                            if s['role'] == 'Doctor' and s['department_id'] == department['department_id']]),
                    'admitting_physician_id': random.choice([s['staff_id'] for s in self.staff if s['role'] == 'Doctor']),
                    'status': 'Discharged' if discharge_date and discharge_date < datetime.now() else 'Active',
                    'discharge_disposition': random.choice(['Home', 'Rehab', 'Transfer']) if discharge_date else None,
                    'total_charges': round(random.uniform(5000, 100000), 2),
                    'insurance_coverage': round(random.uniform(0.7, 1.0) * random.uniform(5000, 100000), 2),
                    'notes': fake.sentence() if random.random() > 0.7 else None,
                    'created_at': admission_date,
                    'updated_at': discharge_date or fake.date_time_between(start_date=admission_date, end_date='now')
                }
    ...
    def generate_devices(self):
        ...
        device_types = ['Heart Monitor', 'Blood Pressure', 'Pulse Oximeter', 'Glucose Monitor',
                       'Temperature', 'Ventilator', 'Infusion Pump', 'ECG']
        ...
        for department in self.departments:
            ...
            for device_num in range(num_devices):
                ...
                device = {
                    ...
                    'hospital_id': department['hospital_id'],
                    'department_id': department['department_id'],
                    'room_id': random.choice([r['room_id'] for r in self.rooms
                    ...
                    'last_maintenance_date': fake.date_between(start_date='-3m', end_date='today'),
                    'next_maintenance_date': fake.date_between(start_date='+1m', end_date='+6m'),
                    'calibration_date': fake.date_between(start_date='-1m', end_date='today'),
                    'battery_level': random.randint(20, 100) if device_type != 'Ventilator' else None,
                    'connectivity_status': random.choices(['Online', 'Offline', 'Intermittent'],
                                                        weights=[85, 10, 5])[0],
    ...
    def generate_device_assignments(self):
        ...
        for admission in active_admissions:
            ...
            department = next(d for d in self.departments if d['department_id'] == admission['department_id'])
            ...
            dept_devices = [d for d in self.devices
                          if d['department_id'] == admission['department_id']
                          and d['is_active']
                          and d['current_patient_id'] is None]

            if dept_devices:
                assigned_devices = random.sample(dept_devices, min(num_devices, len(dept_devices)))
                ...
                for device in assigned_devices:
                    ...
                    assignment = {
                        ...
                        'unassigned_by': admission['attending_physician_id'] if admission['discharge_date'] else None,
                    ...
                    device['current_patient_id'] = admission['patient_id'] if admission['status'] == 'Active' else None
    ...
    def generate_alert_rules(self):
        ...
        rules = [
            # Vital sign rules
            {'name': 'High Heart Rate', 'metric': 'heart_rate', 'operator': '>', 'value1': 120, 'severity': 'Warning'},
            {'name': 'Critical Heart Rate', 'metric': 'heart_rate', 'operator': '>', 'value1': 150, 'severity': 'Critical'},
            {'name': 'Low Heart Rate', 'metric': 'heart_rate', 'operator': '<', 'value1': 50, 'severity': 'Warning'},
            {'name': 'High Blood Pressure', 'metric': 'systolic_bp', 'operator': '>', 'value1': 160, 'severity': 'Warning'},
            {'name': 'Critical Blood Pressure', 'metric': 'systolic_bp', 'operator': '>', 'value1': 180, 'severity': 'Critical'},
            {'name': 'Low Blood Pressure', 'metric': 'systolic_bp', 'operator': '<', 'value1': 90, 'severity': 'Warning'},
            {'name': 'Low Oxygen', 'metric': 'oxygen_saturation', 'operator': '<', 'value1': 92, 'severity': 'Warning'},
            {'name': 'Critical Low Oxygen', 'metric': 'oxygen_saturation', 'operator': '<', 'value1': 88, 'severity': 'Critical'},
            {'name': 'High Temperature', 'metric': 'temperature', 'operator': '>', 'value1': 38.5, 'severity': 'Warning'},
            {'name': 'Critical Temperature', 'metric': 'temperature', 'operator': '>', 'value1': 40, 'severity': 'Critical'},
        ]
    ...
    def generate_medications(self):
        ...
        medications_list = [
            {'name': 'Aspirin', 'generic': 'Acetylsalicylic acid', 'class': 'Analgesic', 'form': 'Tablet', 'strength': '81mg'},
            {'name': 'Lisinopril', 'generic': 'Lisinopril', 'class': 'ACE Inhibitor', 'form': 'Tablet', 'strength': '10mg'},
            {'name': 'Metformin', 'generic': 'Metformin HCl', 'class': 'Antidiabetic', 'form': 'Tablet', 'strength': '500mg'},
            {'name': 'Omeprazole', 'generic': 'Omeprazole', 'class': 'Proton Pump Inhibitor', 'form': 'Capsule', 'strength': '20mg'},
            {'name': 'Atorvastatin', 'generic': 'Atorvastatin', 'class': 'Statin', 'form': 'Tablet', 'strength': '40mg'},
            {'name': 'Amoxicillin', 'generic': 'Amoxicillin', 'class': 'Antibiotic', 'form': 'Capsule', 'strength': '500mg'},
            {'name': 'Morphine', 'generic': 'Morphine Sulfate', 'class': 'Opioid', 'form': 'Injection', 'strength': '10mg/mL'},
            {'name': 'Insulin', 'generic': 'Insulin Regular', 'class': 'Hormone', 'form': 'Injection', 'strength': '100units/mL'},
            {'name': 'Warfarin', 'generic': 'Warfarin Sodium', 'class': 'Anticoagulant', 'form': 'Tablet', 'strength': '5mg'},
            {'name': 'Furosemide', 'generic': 'Furosemide', 'class': 'Diuretic', 'form': 'Tablet', 'strength': '40mg'},
        ]
        ...
        for med in medications_list:
            ...
            medication = {
                ...
                'manufacturer': random.choice(['Pfizer', 'Novartis', 'Roche', 'Merck', 'GSK']),
                'controlled_substance_schedule': 'II' if med['name'] == 'Morphine' else None,
        ...
        for admission in self.admissions:
            ...
            for _ in range(num_prescriptions):
                ...
                prescription = {
                    ...
                    'end_date': admission['discharge_date'] if admission['discharge_date'] else None,
                    'duration_days': (admission['discharge_date'] - admission['admission_date']).days if admission['discharge_date'] else None,
    ...
    def generate_vital_signs(self):
        ...
        active_admissions = [a for a in self.admissions
                            if a['admission_date'] <= datetime.now()
                            and (a['discharge_date'] is None or a['discharge_date'] > start_date)]

        for admission in random.sample(active_admissions, min(len(active_admissions), 50)):
            patient = next(p for p in self.patients if p['patient_id'] == admission['patient_id'])
            age = (datetime.now().date() - patient['date_of_birth']).days // 365

            # Get assigned devices
            assigned_devices = [a for a in self.device_assignments
            ...
            while current_date < end_date:
                ...
                for hour in range(24):
                    ...
                    vital = {
                        ...
                        'patient_id': admission['patient_id'],
                        'admission_id': admission['admission_id'],
                        'device_id': assigned_devices[0]['device_id'] if assigned_devices else None,
                        ...
                        'blood_glucose': round(random.uniform(70, 140), 2) if random.random() > 0.5 else None,
                        'pain_level': random.randint(0, 5) if random.random() > 0.3 else None,
                        'consciousness_level': 'Alert',
                        'recorded_by': random.choice([s['staff_id'] for s in self.staff
                                                     if s['role'] == 'Nurse' and s['department_id'] == admission['department_id']]),
    ...
    def check_vital_alerts(self, vital, admission):
        ...
        for rule in self.alert_rules:
            ...
            if rule['metric_type'] == 'heart_rate' and vital['heart_rate']:
                ...
                if rule['condition_operator'] == '>' and value > rule['threshold_value1']:
                    triggered = True
                elif rule['condition_operator'] == '<' and value < rule['threshold_value1']:
                    triggered = True
            ...
            elif rule['metric_type'] == 'oxygen_saturation' and vital['oxygen_saturation']:
            ...
            if triggered and random.random() > 0.7:  # Don't generate too many alerts
                ...
                alert = {
                    ...
                    'acknowledged_at': vital['recorded_at'] + timedelta(minutes=random.randint(1, 30)) if random.random() > 0.3 else None,
                    'acknowledged_by': random.choice([s['staff_id'] for s in self.staff
                                                     if s['role'] == 'Nurse' and s['department_id'] == admission['department_id']]) if random.random() > 0.3 else None,
                    'resolved_at': vital['recorded_at'] + timedelta(minutes=random.randint(30, 120)) if random.random() > 0.4 else None,
                    'resolved_by': random.choice([s['staff_id'] for s in self.staff
                                                 if s['role'] == 'Doctor']) if random.random() > 0.4 else None,
                    'escalated': rule['severity'] == 'Critical',
                    'response_time_seconds': random.randint(30, 600) if random.random() > 0.3 else None,
    ...
    def generate_device_readings(self):
        ...
        for assignment in random.sample(active_assignments, min(len(active_assignments), 20)):
        ...
        for staff_member in random.sample(self.staff, min(len(self.staff), 100)):
            ...
            while current_date <= datetime.now().date():
                ...
                schedule = {
                    ...
                    'actual_start': datetime.combine(current_date, shift_start) + timedelta(minutes=random.randint(-15, 15)),
                    'actual_end': datetime.combine(current_date, shift_end) + timedelta(minutes=random.randint(-15, 30)),
                    'created_at': datetime.combine(current_date, time(0, 0)) - timedelta(days=7)
                }
