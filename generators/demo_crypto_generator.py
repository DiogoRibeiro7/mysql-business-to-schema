...
def demonstrate_generator():
    ...
    for i in range(5):  # Generate 5 sample users
        ...
        print(f"  Type: {user['user_type']}, KYC: {user['kyc_level']}, "
    ...
    for user in users[:3]:  # First 3 users
        ...
        for currency in user_currencies:
            if currency in crypto_symbols:
                ...
                balance = random.uniform(0, 10) if currency == 'BTC' else random.uniform(0, 100)
            else:
                # Fiat wallet
            ...
            print(f"User #{user['user_id']} - {currency}: {balance:.4f} "
    ...
    for user in users[:3]:
        ...
        for _ in range(2):
            ...
            print(f"Order #{order_id}: {side.upper()} {quantity:.4f} {pair['base']} "
    ...
    for days_ago in range(7, 0, -1):
        ...
        print(f"{candle['timestamp']}: Open=${candle['open']:.2f}, "
    ...
    for user in users[:2]:
        ...
        print(f"User #{user['user_id']}: Staking {amount:.2f} {currency} @ {apy:.2f}% APY")

    print("\n8. DATA GENERATION SUMMARY")
    print("-" * 50)
    print(f"Users Generated: {len(users)}")
    print(f"Currencies: {len(currencies)} ({len(crypto_symbols)} crypto, {len(fiat_currencies)} fiat)")
...
if __name__ == '__main__':
    ...
    user = data['users'][0]
    print(f"  INSERT INTO users (email, username, user_type, kyc_level, daily_limit, ...)")
    print(f"  VALUES ('{user['email']}', '{user['username']}', '{user['user_type']}', "
    ...
    print(f"  VALUES ({order['user_id']}, '{order['pair']}', '{order['type']}', "
