...
class ClinicDataGenerator(BaseGenerator):
    ...
    def __init__(self, config_path: str = 'config.yaml', **db_params):
        ...
        self.lab_test_types = [
            'Complete Blood Count', 'Blood Chemistry Panel', 'Lipid Panel',
            'Thyroid Function', 'Liver Function', 'Kidney Function',
            'Urinalysis', 'X-Ray', 'MRI', 'CT Scan', 'Ultrasound'
        ]

        self.medications = [
            'Amoxicillin', 'Lisinopril', 'Levothyroxine', 'Metformin',
            'Atorvastatin', 'Omeprazole', 'Simvastatin', 'Losartan',
            'Albuterol', 'Gabapentin', 'Metoprolol', 'Sertraline',
    ...
    def generate_departments(self, departments_per_clinic: int):
        ...
        for clinic in self.clinics:
            for dept_type in random.sample(self.department_types, min(departments_per_clinic, len(self.department_types))):
                department = {
                    ...
                    'operating_hours': '08:00-17:00' if dept_type != 'Emergency' else '24/7',
                    'is_active': 1
                }
    ...
    def generate_doctors(self, count: int):
        ...
        for _ in range(count):
            doctor = {
                ...
                'department_id': random.choice(self.departments)['department_id'] if self.departments else 1,
    ...
    def generate_patients(self, count: int):
        ...
        for _ in range(count):
            patient = {
                ...
                'allergies': json.dumps(random.sample(['Penicillin', 'Peanuts', 'Latex', 'None'],
                                                      random.randint(0, 2))),
                'chronic_conditions': json.dumps(random.sample(['Diabetes', 'Hypertension', 'Asthma', 'None'],
                                                               random.randint(0, 2)))
            }
    ...
    def generate_appointments(self, count: int):
        ...
        start_date = datetime.strptime(self.config['settings']['start_date'], '%Y-%m-%d')
        ...
        for _ in range(count):
            ...
            appointment = {
                ...
                'patient_id': random.choice(self.patients)['patient_id'] if self.patients else 1,
                'doctor_id': random.choice(self.doctors)['doctor_id'] if self.doctors else 1,
    ...
    def insert_data_to_database(self):
        ...
        try:
            ...
            if self.departments:
                ...
                self.bulk_insert('departments', dept_data, list(self.departments[0].keys()))
            ...
            if self.patients:
                ...
                self.bulk_insert('patients', patient_data, list(self.patients[0].keys()))
                print(f"Inserted {len(self.patients)} patients")

            # Insert appointments
            if self.appointments:
                appointment_data = [tuple(appointment.values()) for appointment in self.appointments]
                self.bulk_insert('appointments', appointment_data, list(self.appointments[0].keys()))
    ...
    def export_to_sql(self, filename: str):
        ...
        for clinic in self.clinics:
            values = ', '.join([f"'{v}'" if v is not None else 'NULL' for v in clinic.values()])
            sql = f"INSERT INTO clinics ({', '.join(clinic.keys())}) VALUES ({values});"
            sql_statements.append(sql)

        for dept in self.departments:
            values = ', '.join([f"'{v}'" if v is not None else 'NULL' for v in dept.values()])
            sql = f"INSERT INTO departments ({', '.join(dept.keys())}) VALUES ({values});"
            sql_statements.append(sql)
...
def main():
    ...
    parser.add_argument('--format', choices=['sql', 'json', 'database'], default='database',
