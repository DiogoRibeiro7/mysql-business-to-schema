...
class ClinicDataGenerator:
    def __init__(self, config_path: str = 'config.yaml'):
        ...
        self.test_types = [
            'Blood Count', 'Blood Sugar', 'Lipid Panel', 'Liver Function',
            'Kidney Function', 'Thyroid', 'Urinalysis', 'X-Ray', 'CT Scan',
            'MRI', 'Ultrasound', 'ECG', 'Echocardiogram'
        ]

        self.medications = [
            'Paracetamol', 'Ibuprofen', 'Amoxicillin', 'Metformin', 'Lisinopril',
            'Atorvastatin', 'Omeprazole', 'Aspirin', 'Levothyroxine', 'Amlodipine',
            'Metoprolol', 'Losartan', 'Albuterol', 'Gabapentin', 'Hydrochlorothiazide'
        ]

        self.diagnoses = [
            'Hypertension', 'Type 2 Diabetes', 'Hyperlipidemia', 'Asthma',
            'COPD', 'Arthritis', 'Depression', 'Anxiety', 'Migraine',
            'GERD', 'Hypothyroidism', 'Anemia', 'UTI', 'Pneumonia',
            'Bronchitis', 'Sinusitis', 'Allergic Rhinitis', 'Back Pain'
        ]

        self.equipment_types = [
            'X-Ray Machine', 'MRI Scanner', 'CT Scanner', 'Ultrasound Machine',
            'ECG Machine', 'Ventilator', 'Defibrillator', 'Patient Monitor',
            'Infusion Pump', 'Surgical Instruments', 'Autoclave', 'Wheelchair'
        ]
    ...
    def generate_departments(self, departments_per_clinic: int) -> List[Dict]:
        ...
        for clinic in self.clinics:
            for dept_type in random.sample(self.department_types,
                ...
                department = {
                    ...
                    'operating_hours': "08:00-18:00" if dept_type != 'Emergency' else "24/7",
                    'is_active': True,
                    'created_at': datetime.now()
                }
    ...
    def generate_doctors(self, count: int) -> List[Dict]:
        ...
        for _ in range(count):
            ...
            doctor = {
                ...
                'certifications': json.dumps([f"Board Certified - {random.choice(self.specializations)}"]),
                'created_at': datetime.now()
            }
    ...
    def generate_patients(self, count: int) -> List[Dict]:
        ...
        for _ in range(count):
            ...
            patient = {
                ...
                'insurance_provider': random.choice(['Blue Cross', 'Aetna', 'Cigna', 'United', 'Medicare', None]),
                'insurance_policy_number': f"POL{random.randint(100000000, 999999999)}" if random.random() > 0.2 else None,
                'allergies': json.dumps(random.sample(['Penicillin', 'Aspirin', 'Peanuts', 'Latex', 'None'],
                                                     random.randint(0, 2))),
                'chronic_conditions': json.dumps(random.sample(self.diagnoses, random.randint(0, 3))),
                'created_at': self.fake.date_between(start_date='-5y', end_date='today'),
                'updated_at': datetime.now()
            }
    ...
    def generate_appointments(self, count: int) -> List[Dict]:
        ...
        for _ in range(count):
            ...
            appointment_date = self.fake.date_time_between(start_date=start_date, end_date=end_date)
            ...
            appointment = {
                ...
                'notes': self.fake.text(max_nb_chars=200) if status == 'completed' else None,
                'created_at': appointment_date - timedelta(days=random.randint(1, 30)),
                'updated_at': appointment_date if status == 'completed' else datetime.now()
            }
    ...
    def generate_medical_records(self) -> List[Dict]:
        ...
        completed_appointments = [a for a in self.appointments if a['status'] == 'completed']
        ...
        for appointment in completed_appointments:
            ...
            age = (appointment['appointment_date'] - patient['date_of_birth']).days // 365
            ...
            medical_record = {
                ...
                'follow_up_date': (appointment['appointment_date'] +
                                  timedelta(days=random.randint(7, 30))) if random.random() > 0.5 else None,
            ...
            if random.random() > 0.4:  # 60% of visits require lab tests
                ...
                for _ in range(num_tests):
                    ...
                    lab_test = {
                        ...
                        'test_category': random.choice(['Blood', 'Urine', 'Imaging', 'Cardiology']),
                        'ordered_date': record['visit_date'],
                        'status': random.choice(['pending', 'in-progress', 'completed']),
                        'urgency': random.choice(['routine', 'urgent', 'stat']),
                        'special_instructions': self.fake.sentence() if random.random() > 0.7 else None,
                        'created_at': record['visit_date']
                    }
    ...
    def generate_lab_results(self) -> List[Dict]:
        ...
        for test in completed_tests:
            ...
            lab_result = {
                ...
                'is_abnormal': random.choice([True, False]) if result_value != 'Normal' else False,
                'result_date': test['ordered_date'] + timedelta(days=random.randint(1, 3)),
                'performed_by': random.choice(self.staff)['staff_id'] if self.staff else None,
                'verified_by': random.choice(self.doctors)['doctor_id'],
                'comments': self.fake.sentence() if random.random() > 0.7 else None,
                'created_at': test['ordered_date'] + timedelta(days=random.randint(1, 3))
            }
    ...
    def generate_invoices(self) -> List[Dict]:
        ...
        for appointment in completed_appointments:
            ...
            lab_charges = sum(random.randint(50, 500) for t in self.lab_tests
                            if t['patient_id'] == appointment['patient_id']
                            and t['ordered_date'] == appointment['appointment_date'])
            medication_charges = sum(random.randint(20, 200) for p in self.prescriptions
            ...
            invoice = {
                ...
                'payment_method': random.choice(['cash', 'credit_card', 'insurance', 'debit_card']) if random.random() > 0.3 else None,
                'notes': None,
                'created_at': appointment['appointment_date'],
                'updated_at': datetime.now()
            }
...
def main():
    parser = argparse.ArgumentParser(description='Generate sample data for clinic database')
    ...
    parser.add_argument('--format', choices=['sql', 'json', 'csv'], default='sql',
                       help='Output format')
