...
class MedicalProvider(BaseProvider):
    ...
    allergies = [
        'Penicillin', 'Pollen', 'Dust', 'Peanuts', 'Shellfish',
        'Latex', 'Milk', 'Eggs', 'Tree nuts', 'Soy', 'None'
    ]

    chronic_conditions = [
        'Diabetes Type 2', 'Hypertension', 'Asthma', 'Arthritis',
        'COPD', 'Heart Disease', 'Chronic Kidney Disease', 'None'
    ]

    medications = [
        'Metformin', 'Lisinopril', 'Atorvastatin', 'Omeprazole',
        'Amlodipine', 'Metoprolol', 'Albuterol', 'Losartan',
    ...
    specialties = [
        ...
        'Pulmonology', 'Rheumatology', 'General Practice'
    ]
...
class PatientGenerator:
    ...
    def generate_patients(self, count: int) -> List[Dict]:
        ...
        for i in range(count):
            ...
            patient = {
                ...
                'allergies': ', '.join(random.sample([self.fake.allergy() for _ in range(3)], k=random.randint(0, 3))),
                'chronic_conditions': ', '.join(random.sample([self.fake.chronic_condition() for _ in range(2)], k=random.randint(0, 2))),
                'current_medications': ', '.join(random.sample([self.fake.medication() for _ in range(5)], k=random.randint(0, 5))),
                'registration_date': self.fake.date_between(start_date='-5y', end_date='today'),
                'last_visit_date': self.fake.date_between(start_date='-1y', end_date='today'),
                'status': random.choice(['Active', 'Inactive', 'Deceased'] if age > 70 else ['Active', 'Inactive']),
    ...
    def generate_doctors(self, count: int) -> List[Dict]:
        ...
        for i in range(count):
            doctor = {
                ...
                'available_days': random.choice(['Mon-Fri', 'Mon-Sat', 'Tue-Sat', 'Mon-Wed-Fri']),
                'working_hours': random.choice(['9:00-17:00', '8:00-16:00', '10:00-18:00', '14:00-22:00']),
                'rating': round(random.uniform(3.5, 5.0), 1),
                'patients_seen': random.randint(100, 10000),
                'joined_date': self.fake.date_between(start_date='-10y', end_date='today'),
                'status': random.choice(['Active', 'On Leave', 'Retired'] if random.random() > 0.9 else ['Active']),
                'created_at': datetime.now()
            }
    ...
    def generate_appointments(self, count: int, patient_ids: List[str], doctor_ids: List[str]) -> List[Dict]:
        """Generate appointment records"""
        appointments = []
        appointment_types = ['Consultation', 'Follow-up', 'Emergency', 'Routine Check-up', 'Vaccination', 'Test Results']
        statuses = ['Scheduled', 'Completed', 'Cancelled', 'No Show', 'Rescheduled']

        for i in range(count):
            appointment_date = self.fake.date_time_between(start_date='-1y', end_date='+1m')
            ...
            appointment = {
                ...
                'notes': self.fake.text(max_nb_chars=200) if random.random() > 0.5 else None,
    ...
    def generate_medical_records(self, count: int, patient_ids: List[str], doctor_ids: List[str]) -> List[Dict]:
        """Generate medical record entries"""
        records = []
        diagnoses = [
            'Common Cold', 'Influenza', 'Hypertension', 'Diabetes Mellitus',
            'Acute Bronchitis', 'Gastroenteritis', 'Urinary Tract Infection',
            'Migraine', 'Back Pain', 'Anxiety Disorder', 'Allergic Rhinitis'
        ]
        ...
        for i in range(count):
            record = {
                ...
                'prescriptions': ', '.join([self.fake.medication() for _ in range(random.randint(0, 3))]),
                'lab_orders': ', '.join(random.sample(['CBC', 'Metabolic Panel', 'Lipid Panel', 'Thyroid Panel', 'Urinalysis'], k=random.randint(0, 3))),
                'vital_signs': {
                ...
                'follow_up_date': self.fake.date_between(start_date='today', end_date='+2m') if random.random() > 0.3 else None,
    ...
    def generate_prescriptions(self, count: int, patient_ids: List[str], doctor_ids: List[str]) -> List[Dict]:
        """Generate prescription records"""
        prescriptions = []
        dosages = ['5mg', '10mg', '20mg', '50mg', '100mg', '250mg', '500mg']
        frequencies = ['Once daily', 'Twice daily', 'Three times daily', 'Four times daily', 'As needed', 'Every 6 hours']
        ...
        for i in range(count):
            ...
            prescription = {
                ...
                'dispensed_date': start_date + timedelta(days=random.randint(0, 2)) if random.random() > 0.2 else None,
