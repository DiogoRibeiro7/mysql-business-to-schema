...
class SocialMediaGenerator:
    ...
    def generate_users(self):
        ...
        statuses = ['active', 'active', 'active', 'suspended', 'deactivated']  # Most users are active
        ...
        for i in range(CONFIG['users']):
            ...
            account_type = 'verified' if is_influencer else random.choices(
                ['personal', 'business', 'creator'],
                weights=[0.7, 0.2, 0.1]
            )[0]

            username = fake.user_name() + str(random.randint(1, 999)) if random.random() < 0.3 else fake.user_name()

            self.users.append({
                'user_id': self.user_id,
                'username': username[:50],
                'email': fake.email(),
                'password_hash': hashlib.sha256(f"password{self.user_id}".encode()).hexdigest(),
                'display_name': fake.name(),
                'account_type': account_type,
                'verification_status': 'verified' if account_type == 'verified' else random.choice(['unverified', 'pending']),
    ...
    def generate_user_profiles(self):
        ...
        for user in self.users:
            ...
            if user['account_type'] == 'business':
                ...
                bio = random.choice(['Content Creator', 'Digital Artist', 'Photographer', 'Writer']) + " | " + fake.catch_phrase()
            ...
            self.user_profiles.append({
                ...
                'birth_date': fake.date_of_birth(minimum_age=13, maximum_age=80) if user['account_type'] == 'personal' else None,
                ...
                'cover_image_url': f"https://covers.example.com/user_{user['user_id']}.jpg" if random.random() < 0.5 else None,
    ...
    def generate_user_settings(self):
        ...
        for user in self.users:
            ...
            self.user_settings.append({
                ...
                'privacy_message_requests': random.choice(['everyone', 'followers', 'none']),
    ...
    def generate_relationships(self):
        ...
        for user in self.users:
            ...
            if influence > 0.7:  # Influencers
                num_followers = random.randint(min(500, CONFIG['users'] // 2), min(2000, CONFIG['users'] - 1))
            elif influence > 0.5:  # Popular users
                num_followers = random.randint(min(100, CONFIG['users'] // 4), min(500, CONFIG['users'] // 2))
            else:  # Regular users
                num_followers = random.randint(min(10, CONFIG['users'] // 10), min(150, CONFIG['users'] // 3))

            # Limit to available users
            num_followers = min(num_followers, CONFIG['users'] - 1)

            # Select followers
            potential_followers = [u for u in self.users if u['user_id'] != user['user_id'] and u['status'] == 'active']
            followers = random.sample(potential_followers, min(num_followers, len(potential_followers)))

            for follower in followers:
                self.relationship_id += 1

                follow_date = max(user['created_at'], follower['created_at']) + timedelta(
                ...
                self.relationships.append({
                    ...
                    'is_close_friend': random.random() < 0.1,  # 10% are close friends
    ...
    def generate_relationship_requests(self):
        ...
        private_users = [u for u in self.users if u['is_private'] and u['status'] == 'active']
        ...
        for user in private_users:
            ...
            for _ in range(num_requests):
                ...
                requester = random.choice([u for u in self.users
                                         if u['user_id'] != user['user_id']
                                         and u['status'] == 'active'])

                request_date = max(user['created_at'], requester['created_at']) + timedelta(
                    days=random.randint(1, CONFIG['days_of_history'])
                )

                status = random.choices(['pending', 'approved', 'rejected'], weights=[0.5, 0.3, 0.2])[0]
                ...
                self.relationship_requests.append({
                    ...
                    'responded_at': request_date + timedelta(hours=random.randint(1, 72)) if status != 'pending' else None
                })
    ...
    def generate_hashtags(self):
        ...
        hashtag_categories = ['tech', 'fashion', 'food', 'travel', 'fitness', 'art', 'music', 'photography', 'news', 'meme']
        ...
        for i in range(CONFIG['hashtags']):
            ...
            if category == 'tech':
                name = random.choice(['AI', 'MachineLearning', 'Coding', 'Tech', 'Innovation']) + str(random.randint(1, 100))
            elif category == 'fashion':
                name = random.choice(['Fashion', 'Style', 'OOTD', 'Outfit', 'Trends']) + fake.word().capitalize()
            elif category == 'food':
                name = random.choice(['Food', 'Recipe', 'Cooking', 'Foodie', 'Yummy']) + fake.word().capitalize()
            else:
                name = fake.word().capitalize() + random.choice(['Life', 'Daily', 'Love', 'Vibes', ''])
    ...
    def generate_posts(self):
        ...
        for user in self.users:
            ...
            if user['account_type'] in ['creator', 'verified']:
                num_posts = random.randint(min(20, CONFIG['posts_per_user'][0] * 2), CONFIG['posts_per_user'][1])
            elif user['account_type'] == 'business':
                num_posts = random.randint(min(10, CONFIG['posts_per_user'][0]), min(30, CONFIG['posts_per_user'][1]))
            else:
            ...
            for _ in range(num_posts):
                ...
                post_date = fake.date_time_between(start_date=user['created_at'], end_date='now')
                ...
                influence = self.user_influence.get(user['user_id'], 0.3)
                is_viral = random.random() < (influence * 0.1)  # Influenced users have higher viral chance
                ...
                self.posts.append({
                    ...
                    'media_url': f"https://media.example.com/post_{self.post_id}" if post_type in ['image', 'video'] else None,
                    'location': fake.city() if random.random() < 0.3 else None,
                    'visibility': 'public' if not user['is_private'] else 'followers',
                    'is_pinned': random.random() < 0.05,  # 5% are pinned
                    'is_archived': random.random() < 0.1,  # 10% are archived
                    'created_at': post_date,
                    'updated_at': post_date if random.random() > 0.1 else post_date + timedelta(minutes=random.randint(1, 60)),
                    'view_count': random.randint(10, 10000) if is_viral else random.randint(10, 500),
                ...
                if is_viral:
                    ...
                    self.viral_content_tracking.append({
                        ...
                        'post_id': self.post_id,
                        'detection_time': post_date + timedelta(hours=random.randint(1, 24)),
                        'peak_engagement_time': post_date + timedelta(hours=random.randint(24, 72)),
        ...
        for post in media_posts:
            ...
            for i in range(num_media):
                ...
                self.post_media.append({
                    ...
                    'media_order': i + 1,
                    'width': random.choice([1080, 1920, 3840]) if media_type == 'image' else 1920,
                    'height': random.choice([1080, 1920, 2160]) if media_type == 'image' else 1080,
    ...
    def generate_comments(self):
        ...
        for post in self.posts:
            ...
            if post['view_count'] > 5000:
                num_comments = random.randint(min(10, CONFIG['comments_per_post'][0]), max(CONFIG['comments_per_post'][1], 10))
            elif post['view_count'] > 1000:
                num_comments = random.randint(min(5, CONFIG['comments_per_post'][0]), max(15, CONFIG['comments_per_post'][1]))
            else:
            ...
            potential_commenters = [r['follower_id'] for r in self.relationships
                                   if r['following_id'] == post['user_id']]

            if not potential_commenters:
                potential_commenters = [u['user_id'] for u in self.users if u['user_id'] != post['user_id']]
            ...
            for _ in range(min(num_comments, len(potential_commenters))):
                ...
                if self.comments and random.random() < 0.3:  # 30% are replies
                    post_comments = [c for c in self.comments if c['post_id'] == post['post_id']]
                    if post_comments:
                ...
                self.comments.append({
                    ...
                    'content': fake.sentence() if random.random() > 0.1 else random.choice(['😍', '❤️', '👏', '🔥', '💯']),
    ...
    def generate_reactions(self):
        ...
        for post in self.posts:
            ...
            num_reactions = min(num_reactions, CONFIG['users'] // 2)  # Cap at half of users

            # Get potential reactors
            potential_reactors = [u['user_id'] for u in self.users
                                 if u['user_id'] != post['user_id'] and u['status'] == 'active']

            reactors = random.sample(potential_reactors, min(num_reactions, len(potential_reactors)))
    ...
    def generate_shares(self):
        ...
        for post in popular_posts:
            ...
            for _ in range(num_shares):
                ...
                sharer = random.choice([u for u in self.users
                                       if u['user_id'] != post['user_id'] and u['status'] == 'active'])

                share_time = post['created_at'] + timedelta(
                ...
                self.shares.append({
                    ...
                    'share_comment': fake.sentence() if random.random() < 0.3 else None,
    ...
    def generate_bookmarks(self):
        ...
        for user in self.users:
            ...
            bookmarked_posts = random.sample(self.posts, min(num_bookmarks, len(self.posts)))
    ...
    def generate_trending_topics(self):
        ...
        sorted_hashtags = sorted(self.hashtags, key=lambda x: x['usage_count'], reverse=True)
        ...
        while current_date <= end_date:
            ...
            daily_trending = random.sample(trending_hashtags, min(random.randint(5, 10), len(trending_hashtags)))
            ...
            for rank, hashtag in enumerate(daily_trending, 1):
                ...
                self.trending_topics.append({
                    ...
                    'rank': rank,
                    'tweet_volume': hashtag['usage_count'] * random.randint(10, 100),
                    'growth_rate': random.uniform(-0.5, 2.0),  # -50% to +200% growth
    ...
    def generate_conversations(self):
        ...
        for i in range(CONFIG['conversations']):
            ...
            self.conversations.append({
                ...
                'created_at': fake.date_time_between(start_date=self.start_date, end_date='now'),
            ...
            for participant in participants:
                ...
                self.conversation_participants.append({
                    ...
                    'left_at': None,
                    'is_admin': participant == participants[0] if is_group else False,
    ...
    def generate_messages(self):
        ...
        for conversation in self.conversations:
            ...
            participants = [p for p in self.conversation_participants
            ...
            for _ in range(num_messages):
                ...
                message_time = last_message_time + timedelta(minutes=random.randint(1, 60))

                # Message content
                message_types = ['text', 'image', 'video', 'audio', 'file']
                message_type = random.choices(message_types, weights=[0.7, 0.15, 0.05, 0.05, 0.05])[0]
                ...
                if len(self.messages) > 0 and random.random() < 0.2:  # 20% are replies
                    conv_messages = [m for m in self.messages if m['conversation_id'] == conversation['conversation_id']]
                    if conv_messages:
                ...
                self.messages.append({
                    ...
                    'read_by': json.dumps([p['user_id'] for p in participants if random.random() > 0.3])
                })
    ...
    def generate_user_lists(self):
        ...
        for user in self.users[:100]:  # Top 100 users create lists
            ...
            for _ in range(num_lists):
                ...
                list_name = random.choice(['Tech Leaders', 'News Sources', 'Friends', 'Inspirational', 'Funny People'])
                ...
                for member in members:
                    ...
                    self.list_members.append({
                        ...
                        'added_at': created_list['created_at'] + timedelta(days=random.randint(0, 30))
                    })
    ...
    def generate_reports(self):
        ...
        report_reasons = ['spam', 'harassment', 'hate_speech', 'violence', 'nudity', 'false_information', 'other']
        ...
        for post in reported_posts:
            ...
            reporter = random.choice([u for u in self.users if u['user_id'] != post['user_id']])
            ...
            self.reports.append({
                ...
                'status': random.choice(['pending', 'reviewed', 'resolved', 'dismissed']),
                'moderator_id': random.randint(1, 10) if random.random() < 0.7 else None,
    ...
    def generate_banned_content(self):
        ...
        for report in resolved_reports[:20]:  # Limit to 20 bans
            ...
            self.banned_content.append({
                ...
                'ban_date': report['created_at'] + timedelta(hours=random.randint(1, 24)),
                'appeal_status': random.choice(['none', 'pending', 'approved', 'rejected']),
    ...
    def generate_notifications(self):
        ...
        notification_types = ['new_follower', 'like', 'comment', 'mention', 'share', 'message']
        ...
        for user in self.users:
            ...
            user_followers = [r for r in self.relationships if r['following_id'] == user['user_id']]
            ...
            for post in user_posts[:5]:  # Last 5 posts
                post_likes = [r for r in self.reactions if r['post_id'] == post['post_id']]
    ...
    def generate_user_activity_logs(self):
        ...
        activities = ['login', 'logout', 'post_created', 'post_deleted', 'profile_updated', 'settings_changed']
        ...
        for user in self.users:
            ...
            while current <= datetime.now():
                ...
                if random.random() < CONFIG['daily_active_users']:
                    ...
                    login_time = current.replace(hour=random.randint(6, 22), minute=random.randint(0, 59))
                    ...
                    # Logout
                    self.activity_id += 1
                    logout_time = login_time + timedelta(minutes=random.randint(10, 300))
    ...
    def generate_engagement_metrics(self):
        ...
        for post in self.posts:
            if post['view_count'] > 0:
                engagement = (post['like_count'] + post['comment_count'] + post['share_count']) / post['view_count']
    ...
    def generate_summary(self):
        ...
        print(f"  Active Users: {len([u for u in self.users if u['status'] == 'active'])}")
        print(f"  Verified Users: {len([u for u in self.users if u['account_type'] == 'verified'])}")
        print(f"  Private Accounts: {len([u for u in self.users if u['is_private']])}")

        print(f"\nSocial Graph:")
        print(f"  Total Relationships: {len(self.relationships)}")
        print(f"  Average Followers: {sum(u['follower_count'] for u in self.users) / len(self.users):.1f}")
        ...
        print(f"  Group Chats: {len([c for c in self.conversations if c['conversation_type'] == 'group'])}")

        print(f"\nEngagement:")
        avg_engagement = sum(p['engagement_rate'] for p in self.posts) / len(self.posts) if self.posts else 0
