...
class SocialMediaDataGenerator:
    def __init__(self):
        ...
        self.messages = []
        self.notifications = []
        self.start_date = datetime.now() - timedelta(days=config['counts']['days_of_activity'])
    ...
    def generate_users(self):
        ...
        follower_counts = self._generate_power_law_distribution(
            num_users,
            config['engagement']['power_law_alpha']
        )
        ...
        for i in range(1, num_users + 1):
            ...
            user = {
                ...
                'phone_number': faker.phone_number()[:20] if random.random() > 0.5 else None,
                ...
                'last_active': created_at + timedelta(
                    days=random.randint(0, (datetime.now() - created_at).days)
                )
            }
            ...
            profile = {
                'user_id': i,
                'display_name': faker.name() if user_type == 'personal' else faker.company(),
                'bio': faker.text(max_nb_chars=200) if random.random() > 0.3 else None,
                'profile_picture_url': f"https://avatars.example.com/{i}.jpg" if random.random() > 0.3 else None,
                'cover_picture_url': f"https://covers.example.com/{i}.jpg" if random.random() > 0.6 else None,
                'website': faker.url() if user_type in ['business', 'creator', 'verified'] else None,
                'location': faker.city() if random.random() > 0.4 else None,
                'birth_date': faker.date_of_birth(minimum_age=13, maximum_age=80) if user_type == 'personal' else None,
                'gender': random.choice(['male', 'female', 'other', 'prefer_not_to_say']) if random.random() > 0.3 else None,
                'language': random.choice(['en', 'es', 'fr', 'de', 'ja', 'zh']),
                'timezone': faker.timezone(),
                'is_private': random.choices(
                    [True, False],
                    weights=[0.2, 0.8]
                )[0],
                'verified_badge': user_type == 'verified' or (user_type == 'creator' and random.random() > 0.7),
                'follower_count': follower_counts[i - 1],
                'following_count': random.randint(10, min(1000, follower_counts[i - 1] * 2)),
    ...
    def generate_relationships(self):
        ...
        for user in self.users[:config['counts']['users'] // 2]:  # Generate for half the users to avoid too many relationships
            ...
            num_relationships = int(
                base_relationships * activity_multiplier.get(profile['activity_level'], 1.0)
            )
            ...
            targets = random.sample(
                potential_targets,
                min(num_relationships, len(potential_targets))
            )

            for target_id in targets:
                rel_type = random.choices(
                    list(config['distributions']['relationship_types'].keys()),
                    weights=list(config['distributions']['relationship_types'].values())
                )[0]
    ...
    def generate_posts(self):
        ...
        for user in self.users:
            ...
            for _ in range(num_posts):
                ...
                base_engagement = profile['follower_count'] * config['counts']['engagement_rate']

                if profile['verified_badge']:
                    base_engagement *= config['engagement']['network_effects']['verified_engagement_boost']
    ...
    def generate_hashtags(self):
        ...
        for normalized, data in hashtag_counts.items():
            self.hashtags[hashtag_id] = {
                ...
                'created_at': min(
                    p['created_at'] for p in self.posts
        ...
        with open(output_dir / 'users.csv', 'w', newline='', encoding='utf-8') as f:
            if self.users:
                writer = csv.DictWriter(f, fieldnames=[
                    'user_id', 'username', 'email', 'email_verified',
                    'password_hash', 'phone_number', 'phone_verified',
                    'status', 'account_type', 'created_at', 'last_active'
                ])
                writer.writeheader()
                writer.writerows(self.users)

        # Write user profiles
        with open(output_dir / 'user_profiles.csv', 'w', newline='', encoding='utf-8') as f:
            if self.user_profiles:
                writer = csv.DictWriter(f, fieldnames=[
                    'user_id', 'display_name', 'bio', 'profile_picture_url',
                    'cover_picture_url', 'website', 'location', 'birth_date',
                    'gender', 'language', 'timezone', 'is_private',
                    'verified_badge', 'follower_count', 'following_count',
                    'post_count'
                ], extrasaction='ignore')
                writer.writeheader()
