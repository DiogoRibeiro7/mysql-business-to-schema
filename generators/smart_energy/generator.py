...
class SmartEnergyGenerator:
    ...
    def generate_all(self):
        ...
        print(
            f"  Total zones: {CONFIG['buildings'] * CONFIG['floors_per_building'] * CONFIG['zones_per_floor']}"
        )
    ...
    def generate_buildings(self):
        ...
        for i in range(CONFIG["buildings"]):
            building = {
                ...
                "occupancy_type": random.choice(
                    ["owner_occupied", "single_tenant", "multi_tenant"]
                ),
        ...
        for building in self.buildings:
            ...
            for floor_num in range(building["floors_count"]):
                ...
                floor = {
                    ...
                    "floor_name": (
                        f"Floor {floor_num}" if floor_num > 0 else "Ground Floor"
                    ),
                    "area_sqm": round(floor_area, 2),
                    "height_meters": round(random.uniform(3.0, 4.5), 2),
                    "is_mechanical": floor_num
                    == building["floors_count"] - 1,  # Top floor
                    "has_hvac_zone": True,
                    "typical_occupancy": building["typical_occupancy"]
                ...
                for zone_num in range(CONFIG["zones_per_floor"]):
                    ...
                    zone = {
                        ...
                        "window_orientation": (
                            random.choice(orientations) if zone_num < 4 else "None"
                        ),
    ...
    def generate_tenants(self):
        ...
        for i in range(CONFIG["tenants"]):
            tenant = {
                ...
                "lease_start_date": fake.date_between(
                ...
                "status": random.choices(
                    ["active", "pending", "expired"], weights=[90, 5, 5]
                )[0],
            ...
            for zone in assigned_zones:
                ...
                assignment = {
                    ...
                    "usage_percentage": (
                        ...
                        else round(random.uniform(20, 80), 2)
                    ),
    ...
    def generate_meter_types(self):
        ...
        for i, (code, name, unit, freq, is_utility, is_sub) in enumerate(
            meter_type_configs, 1
        ):
            meter_type = {
                "meter_type_id": i,
                "type_code": code,
            ...
            for meter_type in self.meter_types:
                if meter_type["is_utility_meter"]:
                    ...
                    meter = {
                        ...
                        "next_calibration_date": fake.date_between(
                            start_date="+6m", end_date="+1y"
                        ),
                        ...
                        "status": random.choices(
                            ["active", "inactive", "faulty"], weights=[95, 3, 2]
                        )[0],
            ...
            for zone in random.sample(building_zones, min(10, len(building_zones))):
                if zone["zone_type"] in ["office", "server_room", "kitchen"]:
                    ...
                    parent_meter = next(
                        (
                            m
                            for m in self.energy_meters
                            if m["building_id"] == building["building_id"]
                            and m["meter_type_id"] == 1
                        ),
                        None,
                    )  # ELEC_MAIN
                    ...
                    meter = {
                        ...
                        "status": "active",
                        "parent_meter_id": (
                            parent_meter["meter_id"] if parent_meter else None
                        ),
    ...
    def generate_solar_systems(self):
        ...
        solar_buildings = random.sample(
            self.buildings, min(CONFIG["solar_systems"], len(self.buildings))
        )
        ...
        for building in solar_buildings:
            ...
            system = {
                ...
                "warranty_end_date": fake.date_between(
                    start_date="+15y", end_date="+25y"
                ),
                "status": "active",
                "created_at": building["created_at"],
                "updated_at": fake.date_time_between(start_date="-7d", end_date="now"),
    ...
    def generate_battery_storage(self):
        ...
        battery_buildings = [
            s["building_id"] for s in self.solar_systems[: CONFIG["battery_systems"]]
        ]
        ...
        for building_id in battery_buildings:
            ...
            solar_system = next(
                s for s in self.solar_systems if s["building_id"] == building_id
            )
    ...
    def generate_hvac_units(self):
        ...
        for building in self.buildings:
            ...
            for unit_num in range(CONFIG["hvac_units_per_building"]):
                ...
                served_zones = random.sample(
                    building_zones, min(num_zones_served, len(building_zones))
                )
                ...
                unit = {
                    ...
                    "manufacturer": random.choice(
                        ["Carrier", "Trane", "Daikin", "Johnson Controls"]
                    ),
        ...
        for zone in random.sample(self.zones, min(200, len(self.zones))):
            ...
            for _ in range(num_equipment):
                ...
                equipment = {
                    ...
                    "quantity": (
                        random.randint(1, 10) if equipment_type == "lighting" else 1
                    ),
                    ...
                    "status": random.choices(
                        ["active", "inactive", "maintenance"], weights=[95, 3, 2]
                    )[0],
                    "created_at": zone["created_at"],
                    "updated_at": fake.date_time_between(
                        start_date="-7d", end_date="now"
    ...
    def generate_energy_consumption(self):
        ...
        electricity_meters = [
            m for m in self.energy_meters if m["meter_type_id"] == 1
        ]  # ELEC_MAIN

        for meter in random.sample(electricity_meters, min(len(electricity_meters), 5)):
            building = next(
                b for b in self.buildings if b["building_id"] == meter["building_id"]
            )
            base_load = building["total_area_sqm"] * 0.05  # 50W/m2 base load

            current_date = start_date
            daily_consumptions = []
            cumulative_reading = random.uniform(
                100000, 500000
            ...
            while current_date < datetime.now():
                ...
                for hour in range(24):
                    ...
                    if 6 <= hour <= 18:
                        ...
                        base_production = solar_system["capacity_kw"] * (
                            1 - hour_offset * 0.15
                        )
                    ...
                    production = {
                        ...
                        "energy_kwh": round(
                            max(0, power_kw), 3
                        ),  # Hourly energy = average power
                        "irradiance_w_m2": (
                            round(power_kw / solar_system["capacity_kw"] * 1000, 2)
                            if power_kw > 0
                            else 0
                        ),
                        "panel_temp_c": (
                            round(random.uniform(15, 45), 2) if power_kw > 0 else None
                        ),
                        "ambient_temp_c": round(random.uniform(10, 35), 2),
                        "efficiency_pct": (
                            round(random.uniform(15, 20), 2) if power_kw > 0 else 0
                        ),
        ...
        for battery in self.battery_storage:
            ...
            while current_date < datetime.now():
                for hour in range(24):
                    ...
                    solar_system = next(
                        s
                        for s in self.solar_systems
                        if s["building_id"] == battery["building_id"]
                    )
                    solar_production = next(
                        (
                            p
                            for p in self.solar_production
                    ...
                    if solar_production and solar_production["power_kw"] > 0:
                        ...
                        power_kw = min(
                            solar_production["power_kw"], battery["max_power_kw"]
                        )
                        soc = min(95, soc + (power_kw / battery["capacity_kwh"]) * 100)
                    elif 17 <= hour <= 21:  # Peak evening hours
                        # Discharge during peak
                        power_kw = -min(
                            battery["max_power_kw"], battery["capacity_kwh"] * soc / 100
                        )
                        soc = max(
                            20, soc - (abs(power_kw) / battery["capacity_kwh"]) * 100
                        )
                    else:
                        power_kw = 0
                    ...
                    status = {
                        ...
                        "current_a": (
                            round(abs(power_kw) * 2.5, 2) if power_kw != 0 else 0
                        ),
            ...
            while current_date < datetime.now():
                for hour in range(24):
                    ...
                    building = next(
                        ...
                        if b["building_id"] == hvac["building_id"]
                    )
                    ...
                    telemetry = {
                        ...
                        "timestamp": timestamp,
                        "supply_temp_c": (
                            round(setpoint + random.uniform(-2, 2), 2)
                            if operation_mode != "off"
                            else None
                        ),
                        "return_temp_c": (
                            round(setpoint + random.uniform(-1, 3), 2)
                            if operation_mode != "off"
                            else None
                        ),
                        ...
                        "power_kw": (
                            round(hvac["capacity_kw"] * fan_speed / 100, 3)
                            if operation_mode != "off"
                            else 0
                        ),
                        "efficiency_cop": (
                        ...
                        "runtime_minutes": (
                            random.randint(30, 60) if operation_mode != "off" else 0
                        ),
                        "fault_code": (
                            None
                            if random.random() > 0.02
                            else f"E{random.randint(100, 999)}"
                        ),
                        "created_at": timestamp,
                    }
        ...
        for i in range(random.randint(5, 15)):
            ...
            event = {
                ...
                "start_time": event_date.replace(
                    hour=random.choice([14, 15, 16, 17]), minute=0, second=0
                ),
                ...
                "notification_time": event_date
                - timedelta(hours=random.choice([1, 2, 24])),
                "response_status": random.choice(["completed", "accepted", "declined"]),
                "actual_reduction_kw": (
                    round(random.uniform(50, 400), 3) if random.random() > 0.3 else None
                ),
                "compliance_pct": (
                    round(random.uniform(60, 110), 2) if random.random() > 0.3 else None
                ),
                "revenue_earned": (
                    round(random.uniform(50, 500), 2) if random.random() > 0.3 else None
                ),
    ...
    def generate_summary(self):
        ...
        renewable_pct = (
            (total_solar / total_consumption * 100) if total_consumption > 0 else 0
        )
