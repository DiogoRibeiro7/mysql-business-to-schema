...
class GamingPlatformGenerator(BaseGenerator):
    ...
    def generate_players(self, count: int = 10000):
        ...
        for i in range(count):
            ...
            account_type = self.faker.random_element(
                [("free", 0.70), ("premium", 0.25), ("vip", 0.05)]
            )
            ...
            player = (
                ...
                (
                    random.choice([0, 1]) if account_type != "free" else 0
                ),  # two_factor_enabled
    ...
    def generate_games(self, count: int = 50):
        ...
        for _ in range(min(count, len(game_names) * 3)):
            ...
            game = (
                ...
                (
                    random.uniform(0, 59.99) if random.random() > 0.3 else 0
                ...
                (
                    random.choice([0, 1]) if random.random() > 0.7 else 0
                ),  # has_competitive
    ...
    def generate_social_connections(self):
        ...
        for player in players:
            ...
            if num_friends > 0:
                potential_friends = [
                    p for p in players if p["player_id"] != player["player_id"]
                ]
                player_friends = random.sample(
        ...
        self.bulk_insert(
            "friends", friends, ["player_id", "friend_id", "status", "created_at"]
        )
    ...
    def generate_matches(self, matches_per_day: int = 5000):
        ...
        for days_ago in range(30, 0, -1):
            ...
            daily_matches = random.randint(
                int(matches_per_day * 0.7), int(matches_per_day * 1.3)
            )
            ...
            for _ in range(daily_matches):
                ...
                match_player_list = random.sample(
                    players, min(num_players, len(players))
                )
                ...
                match_record = (
                    ...
                    num_players,
                    (
                        random.choice(match_player_list)["player_id"]
                        if num_players > 0
                        else None
                    ),  # winner_id
                    None,  # winner_team
                ...
                for i, player in enumerate(match_player_list):
                    team = (
                        (i % 2) + 1
                        if match_mode in ["team_deathmatch", "capture_flag"]
                        else None
                    )
                    ...
                    player_record = (
                        ...
                        (
                            random.randint(10, 100) if random.random() > 0.3 else 0
                        ),  # currency_earned
                    )
    ...
    def generate_tournaments(self):
        ...
        for game in games[:10]:  # Create tournaments for first 10 competitive games
            for _ in range(random.randint(2, 5)):  # 2-5 tournaments per game
                tournament_name = (
                    f"{self.faker.company()} Championship {random.randint(2024, 2025)}"
                )
    ...
    def generate_achievements(self):
        ...
        achievement_list = self.fetch_all(
            "SELECT achievement_id, game_id FROM achievements"
        )
            ...
            if profile["game_id"] in achievements_by_game:
                ...
                for achievement_id in unlocked:
                    player_achievement = (
                        ...
                        self.faker.date_time_between("-1 year", "now"),
                        (
                            random.uniform(0.01, 100) if random.random() > 0.5 else None
        ...
        for game in games:
            ...
            for _ in range(num_items):
                ...
                item = (
                    ...
                    (
                        random.uniform(0.99, 99.99)
                        if item_type != "consumable"
                        else random.uniform(0.99, 9.99)
                    ),
                    (
                        random.randint(0, 10000) if item_type == "consumable" else 0
                    ),  # in_game_currency_price
                    random.choice([0, 1]),  # is_tradeable
                    random.choice([0, 1]),  # is_marketable
                    (
                        random.choice([0, 1]) if item_type == "consumable" else 0
                    ),  # is_consumable
                    (
                        random.randint(1, 100) if item_type == "consumable" else None
                    ),  # stack_size
                    self.faker.date_time_between("-2 years", "now"),
                )
                items.append(item)
    ...
    def generate_player_inventory(self):
        ...
        items = self.fetch_all(
            "SELECT item_id, is_consumable, stack_size FROM virtual_items"
        )
        ...
        for player in players:
            ...
            for item in player_items:
                quantity = (
                    random.randint(1, item["stack_size"])
                    if item["is_consumable"] and item["stack_size"]
                    else 1
                )
    ...
    def generate_marketplace(self):
        ...
        for _ in range(1000):
            ...
            if listing_status == "sold":
                buyer = random.choice(
                    [s for s in sellers if s["player_id"] != seller["player_id"]]
                )
                transaction = (
    ...
    def generate_player_reports(self):
        ...
        for _ in range(500):
            ...
            reported = random.choice(
                [p for p in players if p["player_id"] != reporter["player_id"]]
            )
            ...
            report = (
                ...
                random.choice(
                    ["no_action", "warning", "temporary_ban", "permanent_ban", None]
                ),
                (
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate test data for Gaming Platform"
    )
    ...
    parser.add_argument(
        "--matches-per-day", type=int, default=5000, help="Average matches per day"
    )
    ...
    try:
        ...
        generator.generate_all_data(
            players=args.players, games=args.games, matches_per_day=args.matches_per_day
        )
    finally:
