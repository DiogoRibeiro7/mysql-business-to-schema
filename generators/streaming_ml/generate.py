...
class StreamingMLGenerator:
            ...
            self.categories.append(
                {
                    ...
                    "min_age": (
                        0
                        if name == "Kids"
                        else (18 if name in ["Politics", "Business"] else 13)
                    ),
                }
            )
    ...
    def generate_creators(self):
        ...
        for i in range(self.config["counts"]["creators"]):
            join_date = self.fake.date_between(
                self.platform_launch, self.data_start - timedelta(days=30)
            )
    ...
    def generate_content_items(self):
        ...
        for i in range(self.config["counts"]["content_items"]):
            content_type = random.choices(
                content_types,
                weights=list(
                    self.config["content_distribution"]["categories"].values()
                ),
            )[0]
            ...
            publish_date = self.fake.date_between(self.platform_launch, self.data_end)
        ...
        for i in range(self.config["counts"]["users"]):
            ...
            if segment == "new":
                registration_date = self.fake.date_between(
                    self.data_start - timedelta(days=30), self.data_end
                )
            else:
                registration_date = self.fake.date_between(
                    self.platform_launch, self.data_start - timedelta(days=31)
                )
            ...
            self.users.append(
                {
                    ...
                    "primary_device": random.choices(devices, weights=device_weights)[
                        0
                    ],
                    "subscription_tier": subscription_tier,
                    "subscription_start": (
                        registration_date if subscription_tier != "free" else None
                    ),
        ...
        while current_date <= self.data_end:
            ...
            for user in daily_users[:1000]:  # Limit for performance
                ...
                for _ in range(num_sessions):
                    ...
                    # Session duration
                    duration_range = self.config["session_patterns"]["duration"][
                        segment
                    ]
                    ...
                    if random.random() < 0.2:  # 20% chance of different device
                        device = random.choice(
                            ["mobile", "desktop", "tablet", "smart_tv"]
                        )
                    ...
                    self.user_sessions.append(
                        {
                            ...
                            < (0.1 if segment == "power_user" else 0.3),
                        }
                    )
                    ...
                    for _ in range(min(num_events, 20)):  # Cap at 20 events per session
                        ...
                        event_type = random.choices(event_types, weights=event_weights)[
                            0
                        ]
                        ...
                        if event_type == "view":
                            completion_rate = self.config["engagement_patterns"][
                                "completion_rates"
                            ][segment]
                            watch_time = int(
                                content["duration_seconds"]
                                * random.uniform(0, completion_rate)
                            )
                        else:
                            watch_time = None
    ...
    def generate_recommendations(self):
        ...
        for event in random.sample(view_events, min(len(view_events), 5000)):
            ...
            candidates = random.sample(
                self.content_items, min(50, len(self.content_items))
            )
    ...
    def generate_ml_features(self):
        ...
        for user in self.users[:1000]:  # Limit for performance
            ...
            user_sessions = [
                s for s in self.user_sessions if s["user_id"] == user["user_id"]
            ]
            ...
            total_time_spent = sum(
                (s["end_time"] - s["start_time"]).total_seconds() for s in user_sessions
            )
            ...
            peak_hour = (
                max(set(event_hours), key=event_hours.count) if event_hours else 12
            )

            # Weekend vs weekday
            weekend_events = sum(
                1 for e in user_events if e["event_time"].weekday() >= 5
            )
            ...
            self.ml_features.append(
                {
                    ...
                    "churn_probability": round(
                        min(0.9, days_since_last * 0.1 * (1 - engagement_rate)), 3
                    ),
                    "predicted_ltv": round(
                        user["lifetime_value"] * (1 + engagement_rate), 2
                    ),
            ...
            if user["segment"] in ["power_user", "regular"] and random.random() < 0.3:
                ...
                for _ in range(num_purchases):
                    ...
                    purchase_type = random.choice(
                        ["virtual_currency", "premium_content", "features", "gifts"]
                    )
    ...
    def write_summary(self):
        ...
        print(
            f"  Total Revenue: ${summary['revenue_summary']['total_subscription_revenue'] + summary['revenue_summary']['total_iap_revenue']:,.2f}"
        )
