...
class EcommerceGenerator:
    ...
    def generate_customers(self):
        ...
        customer_types = ['regular'] * 70 + ['prime'] * 25 + ['vip'] * 4 + ['wholesale'] * 1
        ...
        for i in range(CONFIG['customers']):
            ...
            customer = {
                ...
                'preferred_language': random.choice(['en'] * 80 + ['es'] * 10 + ['fr'] * 5 + ['de'] * 3 + ['zh'] * 2),
                'preferred_currency': random.choice(['USD'] * 70 + ['EUR'] * 15 + ['GBP'] * 10 + ['CAD'] * 5),
                'referral_code': fake.bothify(text='REF####???'),
                'referred_by': random.choice([None] * 80 + list(range(1, min(i, 100)))),
                'loyalty_points': random.randint(0, 5000) if random.random() > 0.5 else 0,
                'lifetime_value': 0,  # Will be calculated from orders
                'status': random.choice(['active'] * 95 + ['inactive'] * 4 + ['suspended'] * 1),
            ...
            for j in range(num_addresses):
                ...
                address = {
                    ...
                    'delivery_instructions': fake.sentence() if random.random() > 0.8 else None,
    ...
    def generate_categories(self):
        ...
        main_categories = [
            ...
            'Food & Beverages', 'Office Supplies', 'Pet Supplies', 'Tools & Hardware'
        ]
    ...
    def generate_products(self):
        ...
        statuses = ['active'] * 80 + ['inactive'] * 10 + ['out_of_stock'] * 5 + ['discontinued'] * 3 + ['draft'] * 2
        ...
        for i in range(CONFIG['products']):
            ...
            product = {
                ...
                'product_name': ' '.join([fake.word().capitalize() for _ in range(random.randint(2, 5))]),
                'slug': fake.slug(),
                'brand_id': brand['brand_id'] if brand else None,
                'category_id': category['category_id'],
                'description': fake.paragraph(nb_sentences=5),
                'short_description': fake.sentence(),
                'features': json.dumps([fake.sentence() for _ in range(random.randint(3, 7))]),
                'specifications': json.dumps({
                    ...
                    'warranty': random.choice(['6 months', '1 year', '2 years', '3 years', 'Lifetime'])
                }),
                'base_price': base_price,
                'compare_at_price': round(base_price * random.uniform(1.1, 1.5), 2) if random.random() > 0.6 else None,
                'cost': cost,
                ...
                'discontinue_date': fake.date_between(start_date='+1y', end_date='+3y') if random.random() > 0.95 else None,
            ...
            if random.random() > 0.3:  # 70% of products have variants
                ...
                for j, variant_name in enumerate(variant_types[:num_variants]):
                    ...
                    variant = {
                        ...
                        'compare_at_price': round(base_price * price_modifier * 1.2, 2) if random.random() > 0.7 else None,
    ...
    def generate_warehouses(self):
        ...
        for i, (code, name, city, state) in enumerate(warehouse_locations[:CONFIG['warehouses']]):
            warehouse = {
                'warehouse_id': i + 1,
                'warehouse_code': code,
                'warehouse_name': name,
    ...
    def generate_inventory(self):
        ...
        for product in self.products:
            ...
            product_variants = [v for v in self.variants if v['product_id'] == product['product_id']]
    ...
    def generate_shopping_behavior(self):
        ...
        for customer in random.sample(self.customers, int(len(self.customers) * 0.3)):
            ...
            for _ in range(num_items):
                ...
                variant = random.choice([v for v in self.variants if v['product_id'] == product['product_id']] or [None])
                ...
                cart_item = {
                    ...
                    'price_at_time': variant['price'] if variant else product['base_price'],
                    'discount_amount': 0,
    ...
    def generate_orders(self):
        ...
        print(f"Generating {CONFIG['orders_per_day']} orders per day for {CONFIG['days_of_history']} days...")

        order_statuses = ['delivered'] * 60 + ['shipped'] * 15 + ['processing'] * 10 + ['confirmed'] * 10 + ['pending'] * 3 + ['cancelled'] * 2
        ...
        for day in range(CONFIG['days_of_history']):
            ...
            daily_orders = random.randint(int(CONFIG['orders_per_day'] * 0.7), int(CONFIG['orders_per_day'] * 1.3))
            ...
            for _ in range(daily_orders):
                ...
                order = {
                    ...
                    'guest_email': None,
                    'status': status,
                    'payment_status': 'paid' if status in ['shipped', 'delivered'] else random.choice(['pending', 'paid', 'failed']),
                    ...
                    'exchange_rate': 1.0,
                    'shipping_address_id': random.choice([a['address_id'] for a in self.addresses if a['customer_id'] == customer['customer_id']]),
                    'billing_address_id': random.choice([a['address_id'] for a in self.addresses if a['customer_id'] == customer['customer_id']]),
                    'shipping_method': random.choice(shipping_methods)['service_name'],
                    'tracking_number': fake.bothify(text='??########??') if status in ['shipped', 'delivered'] else None,
                    'notes': fake.sentence() if random.random() > 0.9 else None,
                    'internal_notes': fake.sentence() if random.random() > 0.95 else None,
                    'ip_address': fake.ipv4(),
                    'user_agent': fake.user_agent(),
                    'referred_from': random.choice(['google', 'facebook', 'instagram', 'email', 'direct', None]),
                    'coupon_code': fake.bothify(text='????##') if random.random() > 0.8 else None,
                    'created_at': current_date,
                    'updated_at': current_date + timedelta(hours=random.randint(1, 24)),
                    'confirmed_at': current_date + timedelta(hours=random.randint(1, 4)) if status != 'pending' else None,
                    'shipped_at': current_date + timedelta(days=random.randint(1, 3)) if status in ['shipped', 'delivered'] else None,
                    'delivered_at': current_date + timedelta(days=random.randint(3, 7)) if status == 'delivered' else None,
                    'cancelled_at': current_date + timedelta(hours=random.randint(1, 48)) if status == 'cancelled' else None
                }
                ...
                for _ in range(num_items):
                    ...
                    item_discount = round(unit_price * random.uniform(0, 0.2), 2) if random.random() > 0.7 else 0
                    item_tax = round((unit_price - item_discount) * 0.08 * quantity, 2)  # 8% tax
                    ...
                    order_item = {
                        ...
                        'gift_message': fake.sentence() if random.random() > 0.98 else None,
                ...
                order['total_amount'] = round(order['subtotal'] + order['tax_amount'] + order['shipping_amount'] - order['discount_amount'], 2)

                self.orders.append(order)

                # Update customer lifetime value
                customer['lifetime_value'] = float(customer.get('lifetime_value', 0)) + float(order['total_amount'])

                # Create payment method if doesn't exist
                if random.random() > 0.3:  # 70% use saved payment method
                    self.payment_id += 1
                    payment_type = random.choice(['credit_card', 'debit_card', 'paypal', 'digital_wallet'])
                    payment_method = {
                        'payment_method_id': self.payment_id,
                        'customer_id': customer['customer_id'],
                        'type': payment_type,
                        'provider': random.choice(['stripe', 'paypal', 'square', 'braintree']),
                        'is_default': random.random() > 0.7,
                        'card_brand': random.choice(['visa', 'mastercard', 'amex', 'discover']) if 'card' in payment_type else None,
                ...
                transaction = {
                    ...
                    'order_id': self.order_id,
                    'payment_method_id': random.randint(1, self.payment_id) if self.payment_id > 0 else None,
                    'transaction_type': 'charge',
                    'amount': order['total_amount'],
                    'currency_code': order['currency_code'],
                    'status': 'succeeded' if order['payment_status'] == 'paid' else order['payment_status'],
                    ...
                    'failure_reason': None if order['payment_status'] == 'paid' else 'Insufficient funds',
                    'processed_at': order['created_at'] + timedelta(minutes=random.randint(1, 10)),
                    'created_at': order['created_at']
                }
                payment_transactions.append(transaction)

                # Create order status history
                statuses_sequence = ['pending', 'processing', 'confirmed', 'shipped', 'delivered']
                current_status_index = statuses_sequence.index(status) if status in statuses_sequence else 0

                for i, hist_status in enumerate(statuses_sequence[:current_status_index + 1]):
                    history_entry = {
                ...
                if status in ['shipped', 'delivered']:
                    ...
                    shipment = {
                        ...
                        'order_id': self.order_id,
                        'warehouse_id': random.choice(self.warehouses)['warehouse_id'],
                        'shipping_method_id': random.choice(shipping_methods)['shipping_method_id'],
                        'tracking_number': order['tracking_number'],
                        'carrier_name': random.choice(['UPS', 'FedEx', 'USPS', 'DHL']),
                        'status': 'delivered' if status == 'delivered' else 'in_transit',
                        'weight_kg': sum([oi['weight_kg'] * oi['quantity'] for oi in self.order_items if oi['order_id'] == self.order_id]),
                        'dimensions_cm': json.dumps({'length': 30, 'width': 20, 'height': 15}),
                        'shipping_label_url': f"https://labels.example.com/{self.shipment_id}.pdf",
                        'shipped_at': order['shipped_at'],
                        'delivered_at': order['delivered_at'],
                        'delivery_signature': fake.name() if status == 'delivered' else None,
                        'notes': None,
                        'created_at': order['created_at'],
                        'updated_at': order['updated_at']
                    }
    ...
    def generate_reviews(self):
        ...
        for order in random.sample(delivered_orders, int(len(delivered_orders) * CONFIG['review_rate'])):
            order_items_list = [oi for oi in self.order_items if oi['order_id'] == order['order_id']]
            ...
            for item in order_items_list:
                if random.random() < 0.3:  # 30% chance to review each item
                    ...
                    review = {
                        ...
                        'admin_reply': fake.sentence() if random.random() > 0.9 else None,
                        'admin_reply_at': fake.date_time_between(start_date=order['delivered_at'], end_date='now') if random.random() > 0.9 else None,
                        'status': random.choice(['approved'] * 90 + ['pending'] * 5 + ['flagged'] * 3 + ['rejected'] * 2),
                        'images': json.dumps([f"https://reviews.example.com/{self.review_id}_{i}.jpg" for i in range(random.randint(0, 3))]),
                        'created_at': fake.date_time_between(start_date=order['delivered_at'], end_date='now'),
                        'updated_at': fake.date_time_between(start_date=order['delivered_at'], end_date='now')
                    }
                    self.reviews.append(review)

                    # Generate votes for popular reviews
                    if review['helpful_count'] > 0:
                        num_votes = min(review['helpful_count'] + review['unhelpful_count'], 50)
                        voters = random.sample(self.customers, min(num_votes, len(self.customers) // 10))
                        ...
                        for voter in voters[:review['helpful_count']]:
                            ...
                            vote = {
                                ...
                                'is_helpful': True,
                                'created_at': fake.date_time_between(start_date=review['created_at'], end_date='now')
                            }
    ...
    def generate_support_and_returns(self):
        ...
        # Support tickets
        for order in random.sample(self.orders, int(len(self.orders) * CONFIG['support_ticket_rate'])):
            self.ticket_id += 1
            ...
            ticket = {
                ...
                'category': random.choice(['order', 'product', 'shipping', 'payment', 'return', 'technical', 'other']),
                'priority': random.choice(['low', 'medium', 'high', 'urgent']),
                'status': random.choice(['resolved'] * 60 + ['closed'] * 20 + ['open'] * 10 + ['in_progress'] * 10),
                'subject': fake.sentence()[:100],
                'description': fake.paragraph(),
                'resolution': fake.paragraph() if random.random() > 0.3 else None,
                'assigned_to': random.randint(1, 10),
                'resolved_at': fake.date_time_between(start_date=order['created_at'], end_date='now') if random.random() > 0.3 else None,
                'satisfaction_rating': random.randint(1, 5) if random.random() > 0.4 else None,
                'created_at': fake.date_time_between(start_date=order['created_at'], end_date='now'),
                'updated_at': fake.date_time_between(start_date=order['created_at'], end_date='now')
            }
            self.support_tickets.append(ticket)

        # Returns
        delivered_orders = [o for o in self.orders if o['status'] == 'delivered']
        for order in random.sample(delivered_orders, int(len(delivered_orders) * CONFIG['return_rate'])):
            self.return_id += 1

            return_status = random.choice(['completed'] * 50 + ['processing'] * 20 + ['approved'] * 15 + ['requested'] * 10 + ['rejected'] * 5)
            ...
            return_data = {
                ...
                'reason': random.choice(['defective', 'wrong_item', 'not_as_described', 'no_longer_needed', 'damaged', 'other']),
                'reason_details': fake.paragraph(),
                'return_shipping_method': random.choice(['Prepaid Label', 'Customer Ships', 'Store Dropoff']),
                'return_tracking_number': fake.bothify(text='RET########') if return_status != 'requested' else None,
                'refund_amount': round(order['total_amount'] * random.uniform(0.5, 1.0), 2),
                'restocking_fee': round(order['total_amount'] * 0.1, 2) if random.random() > 0.8 else 0,
                'return_label_url': f"https://returns.example.com/label_{self.return_id}.pdf",
                'received_condition': random.choice(['new', 'like_new', 'good', 'fair', 'poor']) if return_status in ['processing', 'completed'] else None,
                'inspection_notes': fake.sentence() if return_status in ['processing', 'completed'] else None,
                'requested_at': fake.date_time_between(start_date=order['delivered_at'], end_date='now'),
                'approved_at': fake.date_time_between(start_date=order['delivered_at'], end_date='now') if return_status != 'requested' else None,
                'received_at': fake.date_time_between(start_date=order['delivered_at'], end_date='now') if return_status in ['processing', 'completed'] else None,
                'refunded_at': fake.date_time_between(start_date=order['delivered_at'], end_date='now') if return_status == 'completed' else None
            }
            self.returns.append(return_data)

            # Add return items
            items_to_return = random.sample([oi for oi in self.order_items if oi['order_id'] == order['order_id']],
                                          random.randint(1, min(3, len([oi for oi in self.order_items if oi['order_id'] == order['order_id']]))))
            ...
            for item in items_to_return:
                ...
                return_item = {
                    ...
                    'condition': random.choice(['unopened', 'opened', 'used', 'damaged', 'defective']),
                    'refund_amount': round(item['total_price'] * random.uniform(0.5, 1.0), 2),
    ...
    def generate_promotions(self):
        ...
        for i in range(50):
            coupon = {
                ...
                'discount_type': random.choice(['percentage', 'fixed_amount', 'free_shipping']),
                ...
                'applicable_to': random.choice(['all', 'specific_categories', 'specific_products']),
                'applicable_ids': json.dumps([random.randint(1, 100) for _ in range(random.randint(1, 5))]) if random.random() > 0.5 else None,
            ...
            # Track coupon usage
            if coupon['is_active']:
                orders_with_coupon = random.sample(self.orders, min(random.randint(0, 50), len(self.orders)))
        ...
        for i in range(30):
            rule = {
                ...
                'applicable_to': random.choice(['all', 'specific_categories', 'specific_products']),
                'applicable_ids': json.dumps([random.randint(1, 100) for _ in range(random.randint(1, 10))]),
    ...
    def generate_analytics(self):
        ...
        for _ in range(50000):  # Generate substantial page view data
            ...
            customer = random.choice(self.customers + [None] * int(len(self.customers) * 0.3))  # 30% anonymous
            product = random.choice(self.products + [None] * int(len(self.products) * 0.2))  # 20% non-product pages
            ...
            page_view = {
                ...
                'page_type': random.choice(['product', 'category', 'search', 'home', 'checkout', 'account']),
                'page_url': f"/products/{product['slug']}" if product else f"/{fake.slug()}",
                'referrer_url': random.choice(['https://google.com', 'https://facebook.com', None, '/home', '/search']),
                'ip_address': fake.ipv4(),
        ...
        # Search queries
        search_terms = ['laptop', 'shoes', 'phone', 'dress', 'watch', 'headphones', 'camera', 'tablet', 'gaming', 'kitchen']
        for _ in range(10000):
            self.search_id += 1

            customer = random.choice(self.customers + [None] * int(len(self.customers) * 0.4))

            search_query = {
                'search_id': self.search_id,
                'customer_id': customer['customer_id'] if customer else None,
                'session_id': fake.uuid4()[:128],
                'query_text': ' '.join(random.sample(search_terms, random.randint(1, 3))),
                'results_count': random.randint(0, 200),
                'clicked_position': random.choice([None, 1, 2, 3, 4, 5]),
                'clicked_product_id': random.choice(self.products)['product_id'] if random.random() > 0.5 else None,
                'device_type': random.choice(['desktop', 'mobile', 'tablet']),
                'created_at': fake.date_time_between(start_date='-90d', end_date='now')
            }
        ...
        for customer in random.sample(self.customers, int(len(self.customers) * 0.5)):
            ...
            for _ in range(num_recs):
                ...
                recommendation = {
                    ...
                    'customer_id': customer['customer_id'],
                    'product_id': product['product_id'],
                    'recommendation_type': random.choice(['also_bought', 'viewed_together', 'personalized', 'trending', 'similar']),
    ...
    def generate_shipping_methods(self):
        ...
        for i, (service, carrier, code, min_days, max_days, base_rate) in enumerate(shipping_options):
            method = {
