...
class EcommerceGenerator:
    ...
    def generate_all(self):
        ...
        print(f"[OK] Generated data for {len(self.users)} users, "
    ...
    def _get_subcategories(self, main_category: str) -> List[str]:
        ...
        subcategories = {
            ...
            'clothing': ['Men\'s Clothing', 'Women\'s Clothing', 'Shoes', 'Accessories', 'Kids'],
            'home_garden': ['Furniture', 'Kitchen', 'Bedding', 'Garden Tools', 'Decor'],
            'books': ['Fiction', 'Non-Fiction', 'Educational', 'Comics', 'E-books'],
            'sports': ['Exercise Equipment', 'Outdoor Gear', 'Team Sports', 'Fitness Accessories'],
    ...
    def _generate_brands(self):
        ...
        brand_names = [
            'TechPro', 'StyleMax', 'HomeComfort', 'BookWorld', 'SportZone',
            'BeautyLux', 'ToyLand', 'GourmetDelight', 'EcoFriend', 'ValueBest',
            'PremiumChoice', 'QuickShip', 'GlobalTrade', 'LocalCraft', 'Innovation'
        ]

        for i in range(self.config['counts']['brands']):
            brand = {
                'brand_id': i + 1,
                'brand_name': f"{random.choice(brand_names)} {chr(65 + (i % 26))}" if i >= len(brand_names)
    ...
    def _generate_users(self):
        ...
        for _ in range(self.config['counts']['users']):
            ...
            created_date = self.fake.date_between(
                start_date=datetime.strptime(self.config['date_ranges']['business_start'], '%Y-%m-%d'),
            ...
            user = {
                ...
                'last_login': self.fake.date_between(start_date=created_date, end_date='now'),
                'is_active': random.random() > 0.05,  # 95% active
                'email_verified': random.random() > 0.1,  # 90% verified
                'loyalty_points': random.randint(0, 5000) if segment == 'premium' else random.randint(0, 1000)
            }
    ...
    def _generate_products(self):
        ...
        for _ in range(self.config['counts']['products']):
            ...
            category = random.choice([c for c in self.categories if c['parent_category_id'] is not None])
            main_category_name = next(c['category_name'] for c in self.categories
                                     if c['category_id'] == category['parent_category_id']).lower().replace(' ', '_')
            ...
            price_range = self.config['price_ranges'].get(main_category_name, [9.99, 99.99])
            ...
            product = {
                ...
                'product_name': self._generate_product_name(category['category_name'], brand['brand_name']),
                ...
                'average_rating': round(random.uniform(3.0, 5.0), 1) if random.random() > 0.3 else None,
    ...
    def _generate_product_name(self, category: str, brand: str) -> str:
        ...
        adjectives = ['Premium', 'Professional', 'Ultimate', 'Essential', 'Classic', 'Modern', 'Smart']
        product_types = {
    ...
    def _generate_inventory(self):
        ...
        for product in self.products:
            ...
            strategy = random.choices(
                ...
                weights=list(self.config['inventory']['distribution_strategies'].values())
            )[0]
            ...
            if strategy == 'centralized':
                ...
                inventory = {
                    ...
                    'reorder_point': int(total_stock * self.config['inventory']['reorder_point_percentage']),
                    'reorder_quantity': random.randint(100, 500),
                    'last_restock_date': self.fake.date_between(start_date='-30days', end_date='today'),
                    'location_in_warehouse': f"{random.choice(['A','B','C'])}{random.randint(1,99):02d}"
                }
                self.inventory.append(inventory)
            else:
                # Distribute across warehouses
                warehouses_to_use = self.warehouses if strategy == 'distributed' else random.sample(self.warehouses, 3)
                ...
                for warehouse in warehouses_to_use:
                    if stock_per_warehouse > 0:
                        inventory = {
                            ...
                            'product_id': product['product_id'],
                            'warehouse_id': warehouse['warehouse_id'],
                            'quantity_on_hand': stock_per_warehouse + random.randint(-10, 10),
                            'quantity_reserved': random.randint(0, min(20, stock_per_warehouse)),
                            'reorder_point': int(stock_per_warehouse * self.config['inventory']['reorder_point_percentage']),
    ...
    def _generate_coupons(self):
        ...
        for i in range(50):  # Generate 50 coupons
            coupon = {
                ...
                'discount_value': random.choice([5, 10, 15, 20, 25]) if random.random() > 0.5
                                 else random.choice([5, 10, 15, 20]),
                'valid_from': self.fake.date_between(start_date='-60days', end_date='today'),
                'valid_to': self.fake.date_between(start_date='today', end_date='+30days'),
    ...
    def _generate_orders(self):
        ...
        for _ in range(self.config['counts']['orders']):
            ...
            user_addresses = [a for a in self.addresses if a['user_id'] == user['user_id']]
            if not user_addresses:
                continue

            shipping_address = random.choice(user_addresses)
            billing_address = shipping_address if random.random() > 0.2 else random.choice(user_addresses)
            ...
            coupon_code = None
            discount_amount = 0
            if random.random() < self.config['promotional_campaigns']['coupon_usage_rate']:
            ...
            for product in selected_products:
                ...
                order_item = {
                    ...
                    'discount': random.uniform(0, 0.1) * item_price if random.random() < 0.2 else 0,
            ...
            if coupon_code:
                coupon = next(c for c in self.coupons if c['coupon_code'] == coupon_code)
            ...
            order['tax_amount'] = round((subtotal - discount_amount) * 0.08, 2)  # 8% tax
            order['total_amount'] = round(subtotal - discount_amount + order['tax_amount'] + shipping_cost, 2)
            ...
            if status not in ['cancelled', 'pending']:
                payment = {
                    ...
                    'gateway_response': 'Success' if status != 'cancelled' else 'Payment declined'
                }
            ...
            if status in ['shipped', 'delivered', 'completed', 'returned']:
                shipment = {
                    ...
                    'actual_delivery': order_date + timedelta(days=random.randint(3, 8))
                                      if status in ['delivered', 'completed'] else None,
                    'status': 'delivered' if status in ['delivered', 'completed'] else 'in_transit'
                }
    ...
    def _generate_carts(self):
        ...
        users_with_carts = random.sample(
            self.users,
            int(len(self.users) * 0.3)  # 30% of users have carts
        )
        ...
        for user in users_with_carts:
            ...
            for _ in range(num_carts):
                created_date = self.fake.date_time_between(start_date='-30days', end_date='now')
                ...
                cart = {
                    ...
                    'created_at': created_date,
                    'updated_at': created_date + timedelta(minutes=random.randint(1, 120)),
                    'is_active': random.random() > self.config['counts']['cart_abandonment_rate'],
                    'total_items': 0,
                    'total_amount': 0
                }
    ...
    def _generate_reviews(self):
        ...
        orders_to_review = random.sample(
            completed_orders,
            min(int(len(completed_orders) * self.config['counts']['reviews_percentage']), len(completed_orders))
        )
        ...
        for order in orders_to_review:
            ...
            items_to_review = random.sample(
                order_items_list,
                random.randint(1, min(3, len(order_items_list)))
            )
            ...
            for item in items_to_review:
                ...
                review = {
                    ...
                    'helpful_count': random.randint(0, 50) if random.random() < 0.3 else 0,
                    'created_at': order['order_date'] + timedelta(days=random.randint(1, 30)),
                    'has_images': random.random() < self.config['review_distribution']['with_images']
                }
                self.reviews.append(review)
                self.review_id_counter += 1

                # Update product review count
                product = next(p for p in self.products if p['product_id'] == item['product_id'])
    ...
    def _generate_wishlists(self):
        ...
        users_with_wishlists = random.sample(
            self.users,
            int(len(self.users) * 0.4)  # 40% of users
        )
        ...
        for user in users_with_wishlists:
            ...
            wishlist_products = random.sample(self.products, min(num_items, len(self.products)))
            ...
            for product in wishlist_products:
                wishlist_item = {
                    ...
                    'user_id': user['user_id'],
                    'product_id': product['product_id'],
                    'added_date': self.fake.date_between(start_date='-60days', end_date='today'),
