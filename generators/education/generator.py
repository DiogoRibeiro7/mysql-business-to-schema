...
class EducationDataGenerator:
    def __init__(self, config_path: str = 'config.json'):
        ...
        self.course_subjects = {
            'Computer Science': ['Programming', 'Data Structures', 'Algorithms', 'Databases', 'Networks',
                               'Web Development', 'Mobile Apps', 'AI', 'Machine Learning', 'Cybersecurity'],
            'Mathematics': ['Calculus', 'Linear Algebra', 'Statistics', 'Discrete Math', 'Number Theory',
                          'Differential Equations', 'Probability', 'Topology', 'Abstract Algebra'],
            'Business': ['Management', 'Marketing', 'Finance', 'Accounting', 'Economics',
                        'Entrepreneurship', 'Operations', 'Strategy', 'Leadership', 'Analytics'],
            'Science': ['Physics', 'Chemistry', 'Biology', 'Astronomy', 'Geology',
                       'Environmental Science', 'Anatomy', 'Genetics', 'Ecology', 'Microbiology'],
            'Liberal Arts': ['Literature', 'History', 'Philosophy', 'Psychology', 'Sociology',
                           'Art History', 'Music Theory', 'Creative Writing', 'Ethics', 'Anthropology'],
            'Engineering': ['Mechanical', 'Electrical', 'Civil', 'Chemical', 'Aerospace',
                          'Biomedical', 'Materials', 'Industrial', 'Systems', 'Software']
        }
        ...
        self.assignment_titles = {
            'HOMEWORK': ['Problem Set', 'Reading Assignment', 'Practice Exercises', 'Weekly Assignment', 'Homework'],
            'QUIZ': ['Quiz', 'Pop Quiz', 'Weekly Quiz', 'Chapter Quiz', 'Quick Assessment'],
            'EXAM': ['Midterm Exam', 'Final Exam', 'Unit Test', 'Comprehensive Exam', 'Assessment'],
            'PROJECT': ['Final Project', 'Group Project', 'Research Project', 'Capstone Project', 'Term Project'],
            'PRESENTATION': ['Presentation', 'Final Presentation', 'Group Presentation', 'Research Presentation'],
            'PARTICIPATION': ['Class Participation', 'Discussion Participation', 'Forum Activity', 'Attendance']
        }
        ...
        for inst_type_config in self.config['institutions_config']['types']:
            for i in range(inst_type_config['count']):
                ...
                tier = self._weighted_choice(
                    list(self.config['institutions_config']['subscription_tiers'].keys()),
                    list(self.config['institutions_config']['subscription_tiers'].values())
                )
                ...
                institution = {
                    ...
                    'subscription_expires': (datetime.now() + timedelta(days=365)).date(),
                    'storage_used_gb': round(random.uniform(1, storage_limits[tier] * 0.7), 2),
    ...
    def generate_academic_terms(self):
        ...
        for institution in self.institutions:
            ...
            terms_config = [
                ...
                ('SPRING', f"Spring {year+1}", 'SEMESTER', f"{year+1}-01-15", f"{year+1}-05-31"),
                ('SUMMER', f"Summer {year+1}", 'SUMMER', f"{year+1}-06-01", f"{year+1}-08-15")
            ]
    ...
    def generate_users(self):
        ...
        for dept in self.departments:
            ...
            inst_instructors = [u for u in self.users
    ...
    def _create_user(self, user_type: str) -> Dict:
        ...
        email = f"{username}@{institution['institution_name'].lower().replace(' ', '')}.edu"
        ...
        user = {
            ...
            'notification_preferences': json.dumps(self.config['notification_settings']),
            'last_login_at': self.fake.date_time_between(start_date='-7d', end_date='now'),
            'last_activity_at': self.fake.date_time_between(start_date='-1d', end_date='now'),
            'login_count': random.randint(1, 500),
            'failed_login_attempts': random.randint(0, 3),
            'account_locked_until': None,
            'two_factor_enabled': random.random() > 0.7,
            'two_factor_secret': hashlib.sha256(f"2fa{user_id}".encode()).hexdigest() if random.random() > 0.7 else None,
            'status': status,
            'graduation_year': random.randint(2024, 2028) if user_type == 'STUDENT' else None
        }
    ...
    def _create_user_profile(self, user: Dict):
        ...
        if user['user_type'] == 'STUDENT':
            # Student profile
            major = random.choice([dept['department_name'] for dept in self.departments])
            ...
            profile = {
                ...
                'major': major,
                'minor': random.choice([dept['department_name'] for dept in self.departments]) if random.random() > 0.7 else None,
                'gpa': gpa,
                'credits_earned': credits_earned,
                'credits_required': 120,
                'expected_graduation': date(user['graduation_year'], 5, 31) if user['graduation_year'] else None,
                ...
                'interests': json.dumps(random.sample(list(self.course_subjects.keys()), k=3)),
                'skills': json.dumps(random.sample(['Python', 'Java', 'SQL', 'Excel', 'Writing', 'Research'], k=3)),
                'certifications': json.dumps([]),
                'accessibility_needs': json.dumps([]) if random.random() > 0.95 else json.dumps([]),
            ...
            profile = {
                ...
                'employer': next(i['institution_name'] for i in self.institutions if i['institution_id'] == user['institution_id']),
                'years_experience': random.randint(1, 30),
                'learning_style': None,
                'preferred_pace': None,
                'interests': json.dumps(random.sample(list(self.course_subjects.keys()), k=2)),
                'skills': json.dumps(random.sample(['Teaching', 'Research', 'Mentoring', 'Publishing'], k=3)),
                'certifications': json.dumps(['PhD', 'Teaching Certificate']) if random.random() > 0.5 else json.dumps(['Masters']),
    ...
    def generate_courses(self):
        ...
        for _ in range(self.config['counts']['courses']):
            ...
            institution = next(i for i in self.institutions if i['institution_id'] == department['institution_id'])
            ...
            course = {
                ...
                'duration_weeks': random.choice(self.config['course_config']['duration_weeks']),
                'hours_per_week': random.choice(self.config['course_config']['hours_per_week']),
                'max_students': random.choice(self.config['course_config']['max_students']),
                'min_students': 5,
                'prerequisites': json.dumps([]) if level == 'INTRODUCTORY' else json.dumps([random.randint(1, course_id-1)]) if course_id > 1 else json.dumps([]),
                'corequisites': json.dumps([]),
                'learning_outcomes': json.dumps(outcomes),
                'required_materials': json.dumps([f"Textbook: {topic} ({random.randint(1, 10)}th Edition)"]),
                'tags': json.dumps([subject_area, topic, level]),
                'is_active': True,
                'version': 1,
                'created_by': random.choice(self.instructor_ids),
                'approved_by': random.choice(self.admin_ids) if self.admin_ids else None,
    ...
    def _generate_course_modules(self, course_id: int, topic: str):
        ...
        for module_num in range(1, num_modules + 1):
            ...
            module = {
                ...
                'learning_objectives': json.dumps([self.fake.sentence() for _ in range(3)]),
    ...
    def _generate_lessons(self, module_id: int, module_num: int):
        ...
        for lesson_num in range(1, num_lessons + 1):
            ...
            lesson = {
                ...
                'lesson_type': random.choice(['VIDEO', 'TEXT', 'INTERACTIVE', 'ASSIGNMENT']),
    ...
    def _generate_learning_resources(self, lesson_id: int):
        ...
        for _ in range(num_resources):
            ...
            resource = {
                ...
                'file_size_mb': random.randint(1, 500) if resource_type in ['VIDEO', 'PDF'] else None,
                'duration_minutes': random.randint(5, 45) if resource_type == 'VIDEO' else None,
    ...
    def generate_course_sections(self):
        ...
        for course in self.courses:
            ...
            for section_num in range(1, num_sections + 1):
                ...
                # Select term
                inst_terms = [t for t in self.academic_terms if t['institution_id'] == course['institution_id']]
                term = random.choice(inst_terms) if inst_terms else self.academic_terms[0]

                # Select instructor
                inst_instructors = [u for u in self.users
                                  if u['user_type'] == 'INSTRUCTOR'
                                  and u['institution_id'] == course['institution_id']]
                instructor = random.choice(inst_instructors) if inst_instructors else self.users[0]
                ...
                if course['format'] == 'IN_PERSON':
                    ...
                    room = None
                    is_online = True
                    meeting_url = f"https://zoom.us/j/{random.randint(10000000000, 99999999999)}"
                else:  # HYBRID
                ...
                section = {
                    ...
                    'teaching_assistants': json.dumps([]) if course['course_level'] != 'GRADUATE' else json.dumps([random.choice(self.student_ids)]),
                    'meeting_pattern': meeting_pattern,
                    'meeting_days': json.dumps(list(meeting_days)) if meeting_days else json.dumps([]),
    ...
    def generate_enrollments(self):
        ...
        for student_id in self.student_ids:
            ...
            inst_sections = [s for s in self.course_sections
                           if any(c['institution_id'] == student['institution_id']
                                 for c in self.courses if c['course_id'] == s['course_id'])]
            ...
            for section in selected_sections:
                ...
                if random.random() < self.config['enrollment_patterns']['drop_rate']:
                    status = 'DROPPED'
                    grade = None
                elif random.random() < self.config['enrollment_patterns']['waitlist_rate']:
                    status = 'WAITLISTED'
                    grade = None
                else:
                    status = 'ENROLLED'
                ...
                enrollment = {
                    ...
                    'enrollment_date': datetime.now() - timedelta(days=random.randint(60, 90)),
                    'enrollment_status': status,
                    'enrollment_type': 'CREDIT',
                    'grade': grade,
                    'grade_points': self._calculate_grade_points(grade) if grade else None,
                    'attendance_percentage': random.uniform(60, 100) if status == 'ENROLLED' else None,
                    'last_accessed': datetime.now() - timedelta(days=random.randint(0, 7)),
                    'completion_date': datetime.now() if status == 'ENROLLED' and grade else None
                }
        ...
        for section in self.course_sections:
            ...
            for assignment_type, count, weight in assignments_config:
                for i in range(count):
                    ...
                    due_date = assigned_date + timedelta(days=7 if assignment_type == 'HOMEWORK' else 1)
                    ...
                    assignment = {
                        ...
                        'submission_type': random.choice(['FILE_UPLOAD', 'TEXT_ENTRY', 'URL', 'QUIZ']),
                        'max_attempts': 1 if assignment_type in ['EXAM', 'PROJECT'] else random.choice([1, 2, 3]),
                        'time_limit_minutes': self.config['assessment_config']['time_limit_minutes'].get(assignment_type),
                        'is_group_assignment': assignment_type == 'PROJECT' and random.random() > 0.5,
    ...
    def _generate_submissions(self, assignment: Dict, section_id: int):
        ...
        for student_id in enrolled_students:
            ...
            if random.random() < 0.9:  # 90% submission rate
                ...
                if timing == 'early':
                    submitted_at = assignment['due_date'] - timedelta(days=random.randint(1, 3))
                elif timing == 'on_time':
                    submitted_at = assignment['due_date'] - timedelta(hours=random.randint(1, 24))
                else:  # late
                    submitted_at = assignment['due_date'] + timedelta(hours=random.randint(1, 48))
                ...
                submission = {
                    ...
                    'submission_content': self.fake.text(max_nb_chars=1000) if assignment['submission_type'] == 'TEXT_ENTRY' else None,
                    'submission_url': f"https://submissions.edu/{submission_id}" if assignment['submission_type'] in ['FILE_UPLOAD', 'URL'] else None,
                    'file_name': f"submission_{submission_id}.pdf" if assignment['submission_type'] == 'FILE_UPLOAD' else None,
                    'file_size_kb': random.randint(100, 5000) if assignment['submission_type'] == 'FILE_UPLOAD' else None,
                    'is_late': timing == 'late',
                    'attempt_number': 1
                }
    ...
    def _generate_grade(self, submission: Dict, assignment: Dict, is_late: bool):
        ...
        grade = {
            ...
            'graded_at': submission['submitted_at'] + timedelta(days=random.randint(1, 7)),
            'is_final': True
        }
    ...
    def _generate_discussion_posts(self, discussion: Dict, section_id: int):
        ...
        participants = self.section_students.get(section_id, [])[:10]  # Limit participants
        ...
        for _ in range(num_posts):
            ...
            post = {
                ...
                'author_id': author_id,
                'parent_post_id': None if random.random() > 0.5 else max(1, post_id - random.randint(1, 3)),
                'content': self.fake.paragraph(nb_sentences=random.randint(2, 5)),
                'posted_at': discussion['created_at'] + timedelta(days=random.randint(1, 30)),
                'edited_at': None,
        ...
        for section in self.course_sections[:50]:  # Limit for performance
            ...
            for session_num in range(1, 11):
                ...
                for student_id in enrolled_students:
                    ...
                    if random.random() < self.config['enrollment_patterns']['attendance_rate']:
                        status = 'PRESENT'
                    ...
                    attendance = {
                        ...
                        'status': status,
                        'check_in_time': session_date.replace(hour=section['start_time'].hour if section['start_time'] else 9,
                                                             minute=random.randint(0, 15)) if status in ['PRESENT', 'LATE'] else None,
                        'duration_minutes': random.randint(45, 90) if status == 'PRESENT' else None,
                        'notes': None
                    }
    ...
    def generate_analytics(self):
        ...
        for _ in range(1000):  # Sample activity logs
            ...
            activity = {
                ...
                'user_id': user['user_id'],
                'activity_type': random.choice(['LOGIN', 'VIEW_COURSE', 'SUBMIT_ASSIGNMENT', 'VIEW_RESOURCE', 'POST_DISCUSSION']),
                'activity_details': json.dumps({'ip': self.fake.ipv4(), 'user_agent': self.fake.user_agent()}),
        ...
        for section in self.course_sections[:50]:
            ...
            analytics = {
                ...
                'active_students': len(self.section_students.get(section['section_id'], [])),
    ...
    def generate_all_data(self):
        ...
        print(f"  Configuration: {self.config['counts']['institutions']} institutions, {self.config['counts']['courses']} courses")
