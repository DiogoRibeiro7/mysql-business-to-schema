...
class FinTechGenerator:
    ...
    def setup_risk_rules(self):
        ...
        for rule_id, (code, desc, condition, risk_level, is_active) in enumerate(
            rules, 1
        ):
        ...
        for _ in range(num_individual):
            ...
            customer = {
                ...
                "status": random.choices(
                    ["active", "inactive", "suspended"], weights=[0.9, 0.08, 0.02]
                )[0],
                ...
                "kyc_status": random.choices(
                    ["verified", "pending", "failed"], weights=[0.85, 0.1, 0.05]
                )[0],
            ...
            self.business_customers.append(
                {
                    ...
                    "ein_hash": hashlib.sha256(
                        f"EIN{self.customer_id}".encode()
                    ).hexdigest(),
            ...
            self.customer_addresses.append(
                {
                    ...
                    "street_address2": (
                        fake.secondary_address() if random.random() < 0.2 else None
                    ),
                    "city": fake.city(),
    ...
    def generate_accounts(self):
        ...
        for customer in self.customers:
            ...
            for i in range(min(num_accounts, len(account_types))):
                ...
                opened_date = customer["registration_date"] + timedelta(
                ...
                account = {
                    ...
                    "interest_rate": (
                        Decimal("0.0001") if account_type == "savings" else Decimal("0")
                    ),
                    "overdraft_limit": (
                        Decimal("500") if account_type == "checking" else Decimal("0")
                    ),
                    "minimum_balance": (
                        Decimal("100") if account_type == "checking" else Decimal("500")
                    ),
                    "opened_date": opened_date,
                    "closed_date": None,
                    "last_transaction_date": None,
                    "last_interest_date": (
                        opened_date.replace(day=1)
                ...
                self.create_journal_entry(
                    ...
                    [
                        (
                            ...
                            balance,
                            Decimal("0"),
                        ),  # Debit: Customer Deposits (Asset)
                        (
                            "2010" if account_type == "checking" else "2020",
                            Decimal("0"),
                            balance,
    ...
    def generate_payment_methods(self):
        ...
        for customer in self.customers:
            ...
            customer_accounts = [
                ...
                if any(
                    h["customer_id"] == customer["customer_id"]
    ...
    def generate_cards(self):
        ...
        for account in self.accounts:
            if (
                account["account_type"] in ["checking", "credit_card"]
                ...
                card = {
                    ...
                    "card_number_hash": hashlib.sha256(
                        f"CARD{self.card_id}".encode()
                    ).hexdigest(),
                    "card_last_four": f"{random.randint(1000, 9999)}",
                    "card_brand": random.choice(card_brands),
                    "card_type": (
                        "debit" if account["account_type"] == "checking" else "credit"
                    ),
                    "cardholder_name": fake.name(),
                    "issue_date": issue_date,
                    "expiry_date": expiry_date,
                    "cvv_hash": hashlib.sha256(
                        f"CVV{self.card_id}".encode()
                    ).hexdigest(),
                    "pin_hash": hashlib.sha256(
                        f"PIN{self.card_id}".encode()
                    ).hexdigest(),
                    "status": "active",
    ...
    def generate_daily_transactions(self):
        ...
        while current_date <= end_date:
            daily_transactions = random.randint(
                min(50, CONFIG["transactions_per_day"]),
                max(50, CONFIG["transactions_per_day"]),
            )
            ...
            for _ in range(daily_transactions):
                ...
                transaction = {
                    ...
                    "merchant_name": (
                        fake.company() if trans_type == "payment" else None
                    ),
                    "merchant_category": (
                        random.choice(["5411", "5812", "5999"])
                ...
                if trans_type == "deposit":
                    entries = [
                        ...
                        (
                            "2010" if account["account_type"] == "checking" else "2020",
                            Decimal("0"),
                            amount,
                        ),  # Credit: Customer Account
                    ]
                    ...
                    entries = [
                        ...
                        (
                            ...
                            Decimal("0"),
                            amount,
                        ),  # Credit: Account Maintenance Fees
                    ]
                else:  # interest
                    if account["account_type"] == "savings":
                        entries = [
                            (
                                ...
                                amount,
                                Decimal("0"),
                            ),  # Debit: Deposit Interest Expense
    ...
    def generate_transfers(self):
        ...
        for _ in range(100):  # Generate 100 transfers
            ...
            transfer_time = fake.date_time_between(
            ...
            entries = [
                (
                    "2010" if from_account["account_type"] == "checking" else "2020",
                    amount,
                    Decimal("0"),
                ),  # Debit from account
                (
                    "2010" if to_account["account_type"] == "checking" else "2020",
                    Decimal("0"),
                    amount,
                ),  # Credit to account
            ]
    ...
    def generate_loan_applications(self):
        ...
        for _ in range(CONFIG["loan_applications"]):
            ...
            if risk_score > 700:
                status = random.choices(
                    ["approved", "pending", "rejected"], weights=[0.8, 0.15, 0.05]
                )[0]
            elif risk_score > 500:
                status = random.choices(
                    ["approved", "pending", "rejected"], weights=[0.5, 0.3, 0.2]
                )[0]
            else:
                status = random.choices(
                    ["approved", "pending", "rejected"], weights=[0.2, 0.3, 0.5]
                )[0]
            ...
            self.loan_applications.append(
                {
                    ...
                    "interest_rate": Decimal(str(random.uniform(3.5, 15.0))).quantize(
                        Decimal("0.01")
                    ),
                    ...
                    "status": status,
                    "decision_date": (
                        application_date + timedelta(days=random.randint(1, 7))
                        if status != "pending"
                        else None
                    ),
                    "rejection_reason": (
                        "Low credit score"
                        if status == "rejected" and risk_score < 600
                        else None
                    ),
    ...
    def generate_loan_accounts(self):
        ...
        approved_apps = [
            app for app in self.loan_applications if app["status"] == "approved"
        ]

        for app in approved_apps:
            self.loan_account_id += 1

            disbursement_date = app["decision_date"] + timedelta(
            ...
            self.loan_accounts.append(
                {
                    ...
                    "first_payment_date": disbursement_date.replace(day=1)
                    + timedelta(days=32),
                    "maturity_date": disbursement_date
                    + timedelta(days=30 * app["loan_term_months"]),
                    "outstanding_balance": app["requested_amount"],
                    "total_interest_paid": Decimal("0"),
                    "total_principal_paid": Decimal("0"),
                    "next_payment_date": disbursement_date.replace(day=1)
                    + timedelta(days=32),
                ...
                self.aml_checks.append(
                    {
                        ...
                        "check_type": random.choice(
                            ["pep", "sanctions", "adverse_media", "periodic_review"]
                        ),
                        "risk_score": customer["risk_score"],
                        "status": (
                            "cleared"
                            if customer["risk_score"] > 500
                            else random.choice(["cleared", "review", "escalated"])
                        ),
    ...
    def generate_fraud_alerts(self):
        ...
        suspicious_transactions = random.sample(
            self.transactions,
            min(
                int(len(self.transactions) * CONFIG["fraud_rate"]),
            ...
            self.fraud_alerts.append(
                {
                    ...
                    "triggered_rules": json.dumps(
                        [r["rule_code"] for r in triggered_rules]
                    ),
                    ...
                    "investigation_date": trans["transaction_date"]
                ...
                self.device_fingerprints.append(
                    {
                        ...
                        "os_name": (
                            "iOS"
                            if device_type == "ios"
                            else "Android" if device_type == "android" else "Windows"
                        ),
                        "os_version": f"{random.randint(10, 15)}.{random.randint(0, 5)}",
                        "browser_name": (
                            random.choice(browsers) if device_type == "web" else None
                        ),
                        "browser_version": (
                            f"{random.randint(90, 110)}.0"
                            if device_type == "web"
                            else None
                        ),
    ...
    def generate_sar_reports(self):
        ...
        high_risk_customers = [
            c for c in self.customers if c["risk_category"] == "high"
        ]

        for customer in random.sample(
            high_risk_customers, min(10, len(high_risk_customers))
        ):
            self.sar_id += 1
            ...
            self.sar_reports.append(
                {
                    ...
                    "suspicious_activity_date": filing_date
                    ...
                    "fincen_tracking": (
                        f"FIN{random.randint(1000000, 9999999)}"
    ...
    def generate_notification_preferences(self):
        ...
        for customer in self.customers:
            for notif_type in notification_types:
                ...
                enabled_channels = random.sample(
                    channels, random.randint(1, len(channels))
                )
                ...
                self.notification_preferences.append(
                    {
                        ...
                        "frequency": (
                            random.choice(["immediate", "daily", "weekly"])
                            if notif_type != "transaction"
                            else "immediate"
                        ),
                        "created_at": customer["created_at"],
                    }
            ...
            self.audit_logs.append(
                {
                    ...
                    "resource_type": (
                        "account"
                        if "account" in random.choice(actions)
                        else "transaction"
                    ),
                    ...
                    "status": random.choices(
                        ["success", "failure"], weights=[0.95, 0.05]
                    )[0],
                    "error_message": (
                        "Invalid credentials" if random.random() < 0.05 else None
                    ),
    ...
    def generate_summary(self):
        ...
        print(
            f"  Savings: {len([a for a in self.accounts if a['account_type'] == 'savings'])}"
        )
        print(
