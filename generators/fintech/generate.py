...
class FinTechDataGenerator:
    def __init__(self):
        ...
        self.kyc_documents = []
        self.exchange_rates = {}
        self.start_date = datetime.now() - timedelta(days=config['counts']['days_of_history'])
    ...
    def generate_customers(self):
        ...
        total_customers = config['counts']['individual_customers'] + config['counts']['business_customers']
        ...
        for i in range(1, total_customers + 1):
            ...
            if is_business:
                ...
                customer['business_type'] = random.choice(['LLC', 'Corporation', 'Partnership', 'Sole Proprietorship'])
                customer['registration_number'] = faker.uuid4()[:20]
                customer['tax_id'] = faker.uuid4()[:20]  # Would be encrypted in real system
                customer['incorporation_date'] = faker.date_between(start_date='-10y', end_date='-1y')
                ...
                customer['middle_name'] = faker.first_name() if random.random() > 0.5 else None
                customer['date_of_birth'] = faker.date_of_birth(minimum_age=18, maximum_age=80)
        ...
        for customer in self.customers:
            ...
            for account_type in account_types:
                ...
                balance_config = config['financial']['initial_balance'].get(
                    account_type,
                    config['financial']['initial_balance']['checking']
                )
                ...
                if account_type in config['financial']['interest_rates']:
                    interest_config = config['financial']['interest_rates'][account_type]
    ...
    def generate_payment_methods(self):
        ...
        for customer in self.customers:
            ...
            for i in range(num_methods):
                method_type = random.choices(['card', 'bank_account'], weights=[0.7, 0.3])[0]
            ...
            for _ in range(num_devices):
                device = {
                    ...
                    'first_seen': faker.date_time_between(
                        start_date=customer['onboarding_date'],
                        end_date='now'
                    ),
            ...
            for account in self.accounts:
                ...
                for _ in range(num_transactions):
                    ...
                    tx_type = random.choices(
                        ...
                        weights=list(config['distributions']['transaction_types'].values())
                    )[0]
                    ...
                    amount_range = config['financial']['transaction_amounts'][amount_category]
                    ...
                    if tx_type in ['withdrawal', 'payment', 'transfer']:
                        ...
                        new_balance = account['balance'] - amount if random.random() > 0.5 else account['balance'] + amount

                    # Skip if would overdraw (unless it's a credit account)
                    if new_balance < -account['overdraft_limit'] and account['account_type'] != 'credit_card':
                        continue
                    ...
                    transaction = {
                        ...
                        'description': self.generate_transaction_description(tx_type, amount),
                        'reference_number': faker.uuid4()[:20],
                        'status': random.choices(
                            ['completed', 'pending', 'failed'],
                            weights=[0.95, 0.03, 0.02]
                        )[0],
                        'initiated_at': current_date + timedelta(
                            hours=random.randint(0, 23),
                            minutes=random.randint(0, 59)
                        ),
                        'channel': random.choices(
                            list(config['distributions']['transaction_channels'].keys()),
                            weights=list(config['distributions']['transaction_channels'].values())
                        )[0],
                        'ip_address': faker.ipv4(),
                    ...
                    if transaction['status'] == 'completed':
                        transaction['completed_at'] = transaction['initiated_at'] + timedelta(seconds=random.randint(1, 60))
    ...
    def generate_fraud_alert_for_transaction(self, transaction: Dict, account: Dict):
        ...
        customer_id = next(
            (acc['customer_id'] for acc in self.accounts if acc['account_id'] == account['account_id']),
            None
        )
        ...
        for customer in self.customers:
            ...
            for doc_type in doc_types:
                document = {
                    ...
                    'document_number': faker.uuid4()[:20] if doc_type in ['passport', 'drivers_license'] else None,
                    'issue_date': faker.date_between(start_date='-5y', end_date='-1y') if doc_type in ['passport', 'drivers_license'] else None,
                    'expiry_date': faker.date_between(start_date='+1y', end_date='+10y') if doc_type in ['passport', 'drivers_license'] else None,
            ...
            while check_date <= date.today():
                for check_type in ['sanctions', 'pep', 'adverse_media']:
                    ...
                    check = {
                        ...
                        'check_provider': random.choice(['Provider_A', 'Provider_B', 'Internal']),
                        'check_date': check_date,
                        'result': random.choices(
                            ['clear', 'potential_match', 'confirmed_match'],
                            weights=result_weights
                        )[0],
                        'match_details': {} if random.random() > 0.1 else {'score': random.uniform(0.5, 1.0)}
                    }
                ...
                check_date = check_date + timedelta(days=config['compliance']['aml_check_frequency_days'])
    ...
    def write_all_to_csv(self):
        ...
        with open(output_dir / 'transactions.csv', 'w', newline='', encoding='utf-8') as f:
            if self.transactions:
                fieldnames = [k for k in self.transactions[0].keys() if k != 'completed_at']
                writer = csv.DictWriter(f, fieldnames=fieldnames, extrasaction='ignore')
                writer.writeheader()
                writer.writerows(self.transactions)

        # Write fraud alerts
        with open(output_dir / 'fraud_alerts.csv', 'w', newline='', encoding='utf-8') as f:
        ...
        self.write_simple_csv('currencies.csv', self.currencies,
    ...
    def write_customers_csv(self):
        ...
        base_fields = ['customer_id', 'customer_type', 'email', 'phone_number',
                      'status', 'risk_level', 'onboarding_date', 'preferred_currency']
        ...
        if individuals:
            ind_fields = ['customer_id', 'first_name', 'last_name', 'middle_name',
                         'date_of_birth', 'ssn', 'nationality', 'occupation', 'annual_income']

            with open(output_dir / 'individual_customers.csv', 'w', newline='', encoding='utf-8') as f:
        ...
        if businesses:
            bus_fields = ['customer_id', 'business_name', 'business_type', 'registration_number',
                         'tax_id', 'incorporation_date', 'incorporation_country',
                         'annual_revenue', 'industry']

            with open(output_dir / 'business_customers.csv', 'w', newline='', encoding='utf-8') as f:
    ...
    def write_payment_methods_csv(self):
        ...
        with open(output_dir / 'payment_methods.csv', 'w', newline='', encoding='utf-8') as f:
            if self.payment_methods:
                base_fields = ['payment_method_id', 'customer_id', 'method_type',
        ...
        if cards:
            ...
            if card_data:
                with open(output_dir / 'cards.csv', 'w', newline='', encoding='utf-8') as f:
                    writer = csv.DictWriter(f, fieldnames=card_data[0].keys())
                    writer.writeheader()
                    writer.writerows(card_data)

    def write_simple_csv(self, filename: str, data: List, fieldnames: Optional[List[str]] = None):
