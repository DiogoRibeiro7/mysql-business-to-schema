...
from faker.providers import person, address, phone_number, company, date_time, python, lorem, internet
...
class EventTicketingDataGenerator:
    def __init__(self, config_path: str = 'config.json'):
        ...
        self.event_name_parts = {
            'Music': {
                'prefixes': ['Summer', 'Winter', 'Spring', 'Fall', 'Annual', 'World', 'Ultimate', 'Epic'],
                'suffixes': ['Tour', 'Festival', 'Concert', 'Live', 'Experience', 'Showcase', 'Jam', 'Fest']
            },
            'Sports': {
                'prefixes': ['Championship', 'Premier', 'Elite', 'Pro', 'National', 'International'],
                'suffixes': ['Game', 'Match', 'Tournament', 'Series', 'Finals', 'Playoffs', 'Cup']
            },
            'Theater': {
            ...
            'Conference': {
                ...
                'suffixes': ['Summit', 'Conference', 'Expo', 'Convention', 'Forum', 'Symposium']
            },
            'Family': {
                'prefixes': ['Amazing', 'Magical', 'Wonderful', 'Spectacular'],
                'suffixes': ['Show', 'Adventure', 'Experience', 'Spectacular', 'On Ice']
            }
        }

        # Artist/performer names
        self.performer_first_names = ['The', 'DJ', 'MC', 'Sir', 'Lady', 'Dr.', 'Professor']
        self.performer_last_names = ['Stars', 'Band', 'Orchestra', 'Ensemble', 'Collective', 'Experience', 'Project']
    ...
    def generate_venues(self):
        ...
        venue_names = {
            ...
            'concert_hall': ['Music Hall', 'Symphony Hall', 'Concert Hall', 'Auditorium'],
            'club': ['Club', 'Lounge', 'Venue', 'Room', 'Stage'],
            'conference_center': ['Convention Center', 'Conference Center', 'Expo Center']
        }
        ...
        for _ in range(self.config['counts']['venues']):
            ...
            venue_type = self._weighted_choice(
                ...
                [self.config['venue_types'][vt]['distribution'] for vt in self.config['venue_types'].keys()]
            )

            venue_config = self.config['venue_types'][venue_type]
            capacity = random.randint(venue_config['capacity_min'], venue_config['capacity_max'])
            ...
            self._generate_venue_seating(venue_id, venue_type, capacity, venue_config['sections'])

    def _generate_venue_seating(self, venue_id: int, venue_type: str, total_capacity: int, section_types: List[str]):
        """Generate detailed seating configuration for a venue"""

        self.seat_cache[venue_id] = {}
        remaining_capacity = total_capacity
        sections_count = min(len(section_types), self.config['counts']['sections_per_venue'])
        ...
        for i, section_type in enumerate(section_types[:sections_count]):
            ...
            section = {
                ...
                'default_price_tier': random.choice(['VIP', 'Premium', 'Standard', 'Economy']),
    ...
    def _generate_rows_and_seats(self, section_id: int, section_capacity: int, venue_type: str):
        ...
        venue_id = next(s['venue_id'] for s in self.venue_sections if s['section_id'] == section_id)
        ...
        for row_num in range(rows_needed):
            ...
            if row_num == rows_needed - 1:
                ...
                seats_in_row = min(avg_seats_per_row + random.randint(-5, 5), remaining_seats)
            ...
            row = {
                ...
                'is_accessible': row_num == 0 and random.random() > 0.5  # First row often accessible
            }
            ...
            for seat_num in range(seats_in_row):
                ...
                seat = {
                    ...
                    'is_aisle': seat_num == 0 or seat_num == seats_in_row - 1 or (seats_in_row > 20 and seat_num == seats_in_row // 2),
                    'notes': 'Limited leg room' if seat_type == 'obstructed_view' else None
                }
    ...
    def generate_performers(self):
        ...
        for _ in range(self.config['counts']['performers']):
            ...
            genre_map = {
                'artist': ['Pop', 'Rock', 'Jazz', 'Classical', 'Electronic', 'Hip-Hop', 'Country'],
                'band': ['Rock', 'Pop', 'Metal', 'Punk', 'Indie', 'Alternative'],
                'team': ['Basketball', 'Hockey', 'Baseball', 'Football', 'Soccer'],
                'speaker': ['Technology', 'Business', 'Motivation', 'Science', 'Politics'],
    ...
    def generate_events(self):
        ...
        for _ in range(self.config['counts']['events']):
            ...
            # Select category
            main_categories = [c for c in self.event_categories if c['parent_category_id'] is None]
            main_category = random.choice(main_categories)
            subcategories = [c for c in self.event_categories if c['parent_category_id'] == main_category['category_id']]
            category = random.choice(subcategories) if subcategories else main_category

            # Generate event name
            event_type = random.choice(['single', 'tour', 'season', 'festival', 'conference'])

            name_parts = self.event_name_parts.get(main_category['category_name'], self.event_name_parts['Music'])
            ...
            event = {
                ...
                'age_restriction': age_restrictions.get(main_category['category_name'], 'All Ages'),
            ...
            selected_performers = random.sample(self.performers, min(num_performers, len(self.performers)))
    ...
    def _generate_performances(self, event: Dict):
        ...
        for _ in range(num_performances):
            ...
            days_ahead = random.randint(1, self.config['date_ranges']['performance_window'])
            ...
            performance_datetime = performance_date.replace(hour=hour, minute=0, second=0, microsecond=0)
            ...
            # Calculate pricing
            base_price = random.uniform(self.config['pricing']['base_price_min'], self.config['pricing']['base_price_max'])
            ...
            performance = {
                ...
                'available_capacity': int(venue['capacity'] * random.uniform(0, 0.8)) if status == 'on_sale' else 0,
                'min_ticket_price': round(min_price, 2),
                'max_ticket_price': round(max_price, 2),
                'sales_start_datetime': performance_datetime - timedelta(days=random.randint(30, 90)),
                'sales_end_datetime': performance_datetime + timedelta(hours=2),
                'is_general_admission': venue['venue_type'] == 'club' or random.random() < 0.1,
            ...
            self._generate_performance_pricing(performance_id, venue['venue_id'], min_price, max_price)

    def _generate_performance_pricing(self, performance_id: int, venue_id: int, min_price: float, max_price: float):
        ...
        for section in sections:
            ...
            for i, tier in enumerate(tiers):
                ...
                tier_multiplier = 1.5 - (i * 0.2)  # VIP=1.5, Premium=1.3, Standard=1.1, Economy=0.9
                ...
                self.price_tiers.append({
                    ...
                    'service_fee': round(tier_price * self.config['pricing']['service_fee_percentage'], 2),
                    'facility_fee': self.config['pricing']['facility_fee'],
                    'processing_fee': self.config['pricing']['processing_fee'],
                    'total_price': round(tier_price * (1 + self.config['pricing']['service_fee_percentage']) +
                                       self.config['pricing']['facility_fee'] +
                                       self.config['pricing']['processing_fee'], 2)
                })
    ...
    def generate_customers(self):
        ...
        for _ in range(self.config['counts']['customers']):
            ...
            if random.random() < self.config['customer_behavior']['loyalty_signup_rate']:
    ...
    def _generate_customer_preferences(self, customer_id: int):
        ...
        main_categories = [c['category_name'] for c in self.event_categories if c['parent_category_id'] is None]
        num_favorites = min(random.randint(1, 3), len(main_categories))
        categories = random.sample(main_categories, k=num_favorites) if main_categories else []

        self.customer_preferences.append({
            'preference_id': pref_id,
            'customer_id': customer_id,
            'favorite_categories': json.dumps(categories),
            'favorite_venues': json.dumps(random.sample([v['venue_id'] for v in self.venues],
                                                       k=min(3, len(self.venues)))),
            'price_range_min': random.choice([25, 50, 75, 100]),
            'price_range_max': random.choice([100, 200, 300, 500, 1000]),
            'preferred_days': json.dumps(random.sample(['Monday', 'Tuesday', 'Wednesday', 'Thursday',
                                                       'Friday', 'Saturday', 'Sunday'], k=3)),
            'notification_frequency': random.choice(['daily', 'weekly', 'monthly', 'never']),
            'language': 'en'
        })
    ...
    def _generate_loyalty_member(self, customer_id: int):
        ...
        self.loyalty_members.append({
            ...
            'benefits': json.dumps({
                ...
                'discount_percentage': {'Bronze': 5, 'Silver': 10, 'Gold': 15, 'Platinum': 20}[tier],
    ...
    def generate_bookings(self):
        ...
        available_performances = [p for p in self.performances if p['performance_status'] in ['on_sale', 'sold_out']]

        total_bookings = min(
            self.config['counts']['total_tickets'] // 3,  # Average 3 tickets per booking
    ...
    def _generate_booking(self):
        ...
        is_group = num_tickets >= self.config['booking_patterns']['group_booking_threshold']
        ...
        payment_method = random.choice([pm for pm in self.payment_methods if pm['customer_id'] == customer['customer_id']])
        ...
        for ticket_data in tickets_data:
            ...
            ticket = {
                ...
                'ticket_status': 'valid' if booking['booking_status'] == 'confirmed' else booking['booking_status'],
                'is_resale': False,
            ...
            # Mark seat as booked
            self.performance_seats[performance['performance_id']].add(ticket_data['seat_id'])
        ...
        for _ in range(10):  # Try up to 10 times
            ...
            if section['section_id'] in self.seat_cache.get(venue_id, {}):
                ...
                for row_id, seats in rows.items():
                    available_seats = [s for s in seats if s not in self.performance_seats[performance['performance_id']]]

                    if available_seats:
                        seat_id = random.choice(available_seats)

                        # Get price for this section/tier
                        price_tier = random.choice([pt for pt in self.price_tiers
                        ...
                        return {
                            'seat_id': seat_id,
                            'section_id': section['section_id'],
                            'price': price_tier['base_price'] if price_tier else random.uniform(50, 200)
                        }
    ...
    def _create_payment_transaction(self, booking: Dict):
        ...
        self.payment_transactions.append({
            ...
            'transaction_status': 'completed' if booking['booking_status'] == 'confirmed' else 'failed',
            'gateway_transaction_id': self.fake.uuid4(),
            'gateway_response': 'Approved' if booking['booking_status'] == 'confirmed' else 'Declined',
            'transaction_datetime': booking['booking_datetime'],
            'refund_amount': booking['total_amount'] if booking['booking_status'] == 'refunded' else 0
        })
    ...
    def _generate_ticket_transfer(self, booking_id: int):
        ...
        # Select recipient
        original_customer = next(b['customer_id'] for b in self.booking_transactions if b['booking_id'] == booking_id)
        recipient = random.choice([c for c in self.customers if c['customer_id'] != original_customer])
        ...
        for ticket in tickets_to_transfer:
            ...
            self.ticket_transfers.append({
                ...
                'transfer_datetime': datetime.now() - timedelta(days=random.randint(1, 30)),
    ...
    def _generate_resale_listing(self, booking_id: int):
        ...
        tickets = [t for t in self.tickets if t['booking_id'] == booking_id and t['ticket_status'] == 'valid']
        if not tickets:
            return
        ...
        for ticket in tickets_to_resell:
            ...
            listing = {
                ...
                'ticket_id': ticket['ticket_id'],
                'seller_customer_id': next(b['customer_id'] for b in self.booking_transactions if b['booking_id'] == booking_id),
                'listing_price': round(resale_price, 2),
                'listing_status': random.choice(['active', 'sold', 'expired', 'cancelled']),
                'listed_datetime': datetime.now() - timedelta(days=random.randint(1, 30)),
                'sold_datetime': datetime.now() - timedelta(days=random.randint(0, 15)) if random.random() > 0.5 else None,
                'buyer_customer_id': random.choice(self.customers)['customer_id'] if random.random() > 0.5 else None,
    ...
    def generate_analytics(self):
        ...
        for performance in self.performances[:100]:  # Sample for demo
            tickets_sold = len([t for t in self.tickets if t.get('performance_id') == performance['performance_id']])
            revenue = sum([b['total_amount'] for b in self.booking_transactions
                         if any(t.get('performance_id') == performance['performance_id']
                               for t in self.tickets if t.get('booking_id') == b['booking_id'])])

            self.sales_metrics.append({
                'metric_id': self.counters['metric'],
                'performance_id': performance['performance_id'],
                'date': performance['performance_datetime'].date() if isinstance(performance['performance_datetime'], datetime) else performance['performance_datetime'],
                'tickets_sold': tickets_sold,
                'total_revenue': round(revenue, 2),
                'average_ticket_price': round(revenue / tickets_sold, 2) if tickets_sold > 0 else 0,
                'occupancy_rate': round(tickets_sold / performance['total_capacity'], 4) if performance['total_capacity'] > 0 else 0
            })
            self.counters['metric'] += 1

        # Generate venue utilization
        for venue in self.venues:
            performances_at_venue = [p for p in self.performances if p['venue_id'] == venue['venue_id']]
            ...
            self.venue_utilization.append({
                ...
                'tickets_sold': random.randint(1000, venue['capacity'] * len(performances_at_venue)),
    ...
    def generate_all_data(self):
        ...
        print(f"  Configuration: {self.config['counts']['venues']} venues, {self.config['counts']['events']} events")
...
def main():
    parser = argparse.ArgumentParser(description='Generate event ticketing platform sample data')
