...
class FleetManagementGenerator:
    ...
    def generate_all(self):
        ...
        print(f"[OK] Generated data for {len(self.vehicles)} vehicles, "
    ...
    def _generate_depots(self):
        ...
        for i in range(self.config['counts']['depots']):
            ...
            depot = {
                ...
                'latitude': self._get_city_coordinates(city_name)[0] + random.uniform(-0.1, 0.1),
                'longitude': self._get_city_coordinates(city_name)[1] + random.uniform(-0.1, 0.1),
    ...
    def _generate_vehicles(self):
        ...
        for _ in range(self.config['counts']['vehicles']):
            ...
            fuel_type = random.choices(
                ...
                weights=list(self.config['vehicle_distribution']['fuel_types'].values())
            )[0]
            ...
            purchase_date = datetime(manufacture_year, random.randint(1, 12), random.randint(1, 28))
            ...
            vehicle = {
                ...
                'make': random.choice(['Freightliner', 'Volvo', 'Peterbilt', 'Mack', 'International', 'Ford', 'Mercedes']),
                ...
                'last_service_date': self.fake.date_between(start_date='-60days', end_date='today'),
                'last_service_odometer_km': random.randint(5000, 295000),
                'capacity_kg': self._get_vehicle_capacity(vehicle_type),
                'status': random.choices(['active', 'maintenance', 'inactive'], weights=[0.85, 0.10, 0.05])[0],
                'has_gps': True,
    ...
    def _generate_drivers(self):
        ...
        for _ in range(self.config['counts']['drivers']):
            # Experience level
            experience_level = random.choice(list(self.config['driver_profiles']['experience_years'].keys()))
            exp_range = self.config['driver_profiles']['experience_years'][experience_level]
            ...
            license_class = random.choices(
                ...
                weights=list(self.config['driver_profiles']['license_classes'].values())
            )[0]

            # Hire date based on experience
            if years_experience > 0:
                hire_date = self.fake.date_between(
                    start_date=f'-{years_experience}years',
                    end_date='-1days'
                )
            else:
                hire_date = self.fake.date_between(
            ...
            driver = {
                ...
                'license_expiry': self.fake.date_between(start_date='today', end_date='+3years'),
                'phone': self.fake.phone_number()[:20],
                'email': self.fake.email(),
                'hire_date': hire_date,
                'years_experience': years_experience,
                'safety_score': random.randint(70, 100),
                'status': random.choices(['active', 'on_leave', 'terminated'], weights=[0.90, 0.08, 0.02])[0],
                'home_depot_id': random.choice(self.depots)['depot_id'],
                'medical_cert_expiry': self.fake.date_between(start_date='+30days', end_date='+2years'),
                'hazmat_certified': random.random() < 0.3,
                'last_drug_test': self.fake.date_between(start_date='-90days', end_date='today'),
                'violations_count': random.choices([0, 1, 2, 3], weights=[0.7, 0.2, 0.08, 0.02])[0]
            }
    ...
    def _generate_routes(self):
        ...
        for _ in range(self.config['counts']['routes']):
            ...
            route_type = random.choices(
                ...
                weights=list(self.config['geographic_distribution']['route_types'].values())
            )[0]
            ...
            route = {
                ...
                'estimated_duration_hours': distance_km / 60,  # Assuming 60 km/h average
                'toll_cost': round(distance_km * 0.15, 2) if random.random() < 0.4 else 0,
    ...
    def _generate_trips(self):
        ...
        while current_date <= end_date:
            for vehicle in self.vehicles:
                ...
                num_trips = random.randint(1, self.config['counts']['trips_per_vehicle_per_day'])

                for trip_num in range(num_trips):
                    # Assign driver and route
                    eligible_drivers = [d for d in self.drivers if d['status'] == 'active']
                    ...
                    trip_start = current_date.replace(hour=start_hour, minute=random.randint(0, 59))
                    trip_duration_hours = route['estimated_duration_hours'] * random.uniform(0.9, 1.3)
                    ...
                    trip = {
                        ...
                        'actual_distance_km': route['distance_km'] * random.uniform(0.95, 1.1),
                        'planned_duration_hours': route['estimated_duration_hours'],
                        'cargo_type': cargo_type,
                        'cargo_weight_kg': random.uniform(100, vehicle['capacity_kg'] * 0.9),
                        'fuel_consumed_liters': self._calculate_fuel_consumption(
                            vehicle['vehicle_type'],
                            route['distance_km']
                        ),
                        'status': 'completed' if trip_end < datetime.now() else 'in_progress'
                    }
    ...
    def _generate_trip_stops(self, trip: Dict, vehicle_type: str):
        ...
        for i in range(num_stops):
            stop_time = trip['start_time'] + timedelta(hours=(i + 1) * time_between_stops)
            ...
            stop = {
                ...
                'actual_arrival': stop_time + timedelta(minutes=random.randint(-15, 30)),
                ...
                'packages_delivered': random.randint(1, 20) if vehicle_type == 'delivery_van' else 0,
                'packages_picked_up': random.randint(0, 10) if random.random() < 0.3 else 0,
                'delay_minutes': max(0, random.randint(-5, 20)),
                'delay_reason': random.choice(['traffic', 'customer_not_ready', 'loading_delay', None])
            }
    ...
    def _generate_gps_positions(self):
        ...
        recent_trips = [t for t in self.trips
                       if t['start_time'] > datetime.now() - timedelta(days=2)][:50]

        for trip in recent_trips:
            current_time = trip['start_time']
            trip_stops_list = [s for s in self.trip_stops if s['trip_id'] == trip['trip_id']]
            ...
            while current_time <= trip['end_time']:
                ...
                # Calculate speed
                speed_config = self.config['driving_patterns']['speed_profiles'][road_type]
                ...
                for stop in trip_stops_list:
                    if abs((stop['actual_arrival'] - current_time).total_seconds()) < 600:
                        is_stopped = True
                        current_speed = 0
                        break
                ...
                if not is_stopped:
                    ...
                    lat += random.gauss(0, 0.0001) + (distance_km / 111)  # rough conversion
                ...
                gps_position = {
                    ...
                    'hdop': round(random.uniform(0.8, 2.5), 1),  # Horizontal dilution of precision
    ...
    def _generate_driver_events(self):
        ...
        for trip in self.trips:
            ...
            events_per_100km = {
                'harsh_brake': self.config['driving_patterns']['event_rates']['harsh_brake'],
                'harsh_acceleration': self.config['driving_patterns']['event_rates']['harsh_acceleration'],
                'harsh_cornering': self.config['driving_patterns']['event_rates']['harsh_cornering'],
                'speeding': self.config['driving_patterns']['event_rates']['speeding'],
                'idle_excessive': self.config['driving_patterns']['event_rates']['idle_excessive']
            }
            ...
            for event_type, rate in events_per_100km.items():
                ...
                num_events = max(0, int(random.gauss(expected_events, expected_events ** 0.5)))
                ...
                for _ in range(num_events):
                    ...
                    event = {
                        ...
                        'g_force': round(random.uniform(0.3, 0.8), 2) if 'harsh' in event_type else None
                    }
    ...
    def _generate_engine_diagnostics(self):
        ...
        for trip in self.trips[:100]:  # Limit for performance
            ...
            if random.random() < self.config['engine_diagnostics']['dtc_probability']:
                dtc_code = random.choice(self.config['engine_diagnostics']['common_dtc_codes'])
                dtc = {
    ...
    def _generate_maintenance_records(self):
        ...
        for vehicle in self.vehicles:
            ...
            for i in range(min(num_oil_changes, 10)):  # Limit history
                ...
                service_date = self.fake.date_between(start_date='-2years', end_date='today')
    ...
    def _generate_compliance_data(self):
        ...
        for trip in self.trips:
            driver = next(d for d in self.drivers if d['driver_id'] == trip['driver_id'])
            vehicle = next(v for v in self.vehicles if v['vehicle_id'] == trip['vehicle_id'])
            ...
            log = {
                ...
                'driving_hours': (trip['end_time'] - trip['start_time']).total_seconds() / 3600,
                'on_duty_hours': (trip['end_time'] - trip['start_time']).total_seconds() / 3600 + 1,
                'off_duty_hours': 24 - ((trip['end_time'] - trip['start_time']).total_seconds() / 3600 + 1),
            ...
            if random.random() < self.config['compliance']['dvir_completion_rate']:
                dvir = {
                    ...
                    'horn': 'pass',
                    'mirrors': 'pass',
                    'coupling': 'pass' if vehicle['vehicle_type'] == 'semi_truck' else 'n/a',
                    'emergency_equipment': 'pass',
                    'defects_found': random.random() < 0.05,
                    'defect_description': 'Minor tire wear' if random.random() < 0.05 else None,
    ...
    def _calculate_fuel_consumption(self, vehicle_type: str, distance_km: float) -> float:
        """Calculate fuel consumption for a trip"""
        mpg_range = self.config['fuel_consumption']['mpg_ranges'].get(vehicle_type, [10, 15])
