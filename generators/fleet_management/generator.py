...
class FleetManagementGenerator:
    ...
    def generate_all(self):
        ...
        print(f"  Total vehicles: {CONFIG['companies'] * CONFIG['vehicles_per_company']}")
    ...
    def generate_companies(self):
        ...
        company_names = ['TransLogistics Corp', 'Fleet Express LLC', 'Rapid Delivery Systems']

        for i in range(CONFIG['companies']):
            self.companies.append({
                'company_id': i + 1,
                'company_name': company_names[i] if i < len(company_names) else f"Transport Co {i+1}",
    ...
    def generate_depots(self):
        ...
        depot_types = ['Main Hub', 'Regional Center', 'Distribution Center', 'Service Terminal']
    ...
    def generate_vehicles(self):
        ...
        vehicle_types = ['delivery_van', 'box_truck', 'semi_truck', 'refrigerated', 'flatbed']
        makes = ['Freightliner', 'Volvo', 'Peterbilt', 'Kenworth', 'International',
                'Mercedes-Benz', 'Ford', 'Isuzu']
        fuel_types = ['diesel', 'diesel', 'diesel', 'gasoline', 'electric', 'hybrid']

        for company in self.companies:
            company_depots = [d for d in self.depots if d['company_id'] == company['company_id']]
            ...
            for v in range(CONFIG['vehicles_per_company']):
                ...
                self.vehicles.append({
                    ...
                    'year': year,
                    'color': random.choice(['White', 'Blue', 'Red', 'Gray', 'Black']),
                    ...
                    'registration_expiry': purchase_date.replace(year=purchase_date.year + 1),
                    'insurance_expiry': purchase_date.replace(year=purchase_date.year + 1),
                    'last_service_date': fake.date_between(start_date='-30d', end_date='today'),
                    'last_service_miles': round(base_miles - random.uniform(3000, 5000), 1),
                    'next_service_miles': round(base_miles + random.uniform(5000, 10000), 1),
    ...
    def generate_vehicle_specs(self):
        ...
        for vehicle in self.vehicles:
            ...
            self.vehicle_specs.append({
                ...
                'has_temperature_control': vehicle['vehicle_type'] == 'refrigerated',
    ...
    def generate_drivers(self):
        ...
        for company in self.companies:
            for d in range(CONFIG['drivers_per_company']):
                ...
                medical_cert_expiry = fake.date_between(start_date='today', end_date='+2y')
    ...
    def generate_routes(self):
        ...
        for i in range(CONFIG['routes']):
            ...
            self.routes.append({
                ...
                'toll_cost': round(distance * 0.05, 2) if random.random() < 0.3 else 0,
                'is_active': True,
                'created_at': datetime.now()
            })
    ...
    def generate_geofences(self):
        ...
        geofence_types = ['depot', 'customer', 'rest_area', 'fuel_station', 'restricted']
        ...
        while current_date <= end_date:
            for vehicle in self.vehicles:
                ...
                for trip_num in range(num_trips):
                    ...
                    start_time = datetime.combine(current_date, time(start_hour, random.randint(0, 59)))
                    duration_hours = route['estimated_hours'] * random.uniform(0.9, 1.2)
                    end_time = start_time + timedelta(hours=duration_hours)

                    # Calculate distance and fuel
                    actual_distance = route['distance_miles'] * random.uniform(0.95, 1.1)
                    spec = next((s for s in self.vehicle_specs if s['vehicle_id'] == vehicle['vehicle_id']), None)
                    fuel_used = actual_distance / spec['mpg_highway'] if spec else actual_distance / 7
                    ...
                    if status in ['completed', 'in_progress']:
                        self.generate_gps_positions_for_trip(trip, route, start_time, end_time)
    ...
    def generate_gps_positions_for_trip(self, trip, route, start_time, end_time):
        ...
        # Get depot locations
        origin_depot = next(d for d in self.depots if d['depot_id'] == route['origin_depot_id'])
        dest_depot = next(d for d in self.depots if d['depot_id'] == route['destination_depot_id'])
        ...
        num_positions = min(total_seconds // CONFIG['gps_interval_seconds'], 10000)  # Limit for performance

        # Calculate movement per position
        lat_diff = (dest_depot['latitude'] - origin_depot['latitude']) / num_positions if num_positions > 0 else 0
        lng_diff = (dest_depot['longitude'] - origin_depot['longitude']) / num_positions if num_positions > 0 else 0

        for i in range(0, num_positions, 20):  # Sample every 100 seconds for demo
            self.position_id += 1

            timestamp = start_time + timedelta(seconds=i * CONFIG['gps_interval_seconds'])
            ...
            self.check_geofence_events(trip['vehicle_id'], current_lat, current_lng, timestamp)
    ...
    def check_geofence_events(self, vehicle_id, lat, lng, timestamp):
        ...
        for geofence in self.geofences[:10]:  # Check first 10 geofences for demo
            ...
            if distance < geofence['radius_meters'] and random.random() < 0.01:  # Reduced frequency
    ...
    def generate_fuel_transactions(self):
        ...
        for trip in self.trips:
            if trip['status'] == 'completed' and random.random() < 0.3:  # 30% of trips have fuel stops
                self.fuel_id += 1

                vehicle = next(v for v in self.vehicles if v['vehicle_id'] == trip['vehicle_id'])
                ...
                self.fuel_transactions.append({
                    ...
                    'gallons': round(random.uniform(20, vehicle['fuel_capacity_gallons'] * 0.8), 2),
                    'price_per_gallon': round(random.uniform(3.50, 4.50), 2),
                    'total_cost': 0,  # Will be calculated
                    'odometer_reading': vehicle['odometer_miles'] + trip['distance_miles'],
                    'payment_method': random.choice(['Company Card', 'Fleet Card', 'Cash']),
                    'created_at': trip['start_time']
                })

                # Calculate total cost
                self.fuel_transactions[-1]['total_cost'] = round(
                    self.fuel_transactions[-1]['gallons'] * self.fuel_transactions[-1]['price_per_gallon'], 2
                )
    ...
    def generate_maintenance_records(self):
        ...
        maintenance_types = ['Oil Change', 'Tire Rotation', 'Brake Service', 'Filter Replacement',
                           'Inspection', 'Transmission Service', 'Cooling System', 'Electrical']
        ...
        for vehicle in self.vehicles:
            ...
            for _ in range(num_records):
                ...
                self.maintenance_records.append({
                    ...
                    'odometer_reading': vehicle['odometer_miles'] - random.uniform(1000, 10000),
                    'performed_by': fake.company(),
                    'cost': round(random.uniform(100, 2000), 2),
                    'next_service_miles': vehicle['odometer_miles'] + random.uniform(5000, 15000),
                    'next_service_date': service_date + timedelta(days=random.randint(60, 180)),
            ...
            if vehicle['status'] == 'active':
                ...
                while current <= datetime.now():
                    ...
                    self.vehicle_diagnostics.append({
                        ...
                        'def_level_percent': random.randint(30, 100) if vehicle['fuel_type'] == 'diesel' else None,
                        ...
                        'fault_codes': None if random.random() > 0.1 else f"P{random.randint(1000, 9999)}",
                        'created_at': current
                    })
            ...
            if driver['status'] == 'active':
                ...
                while current <= date.today():
                    ...
                    while daily_hours < 24:
                        ...
                        if log_type == 'driving':
                            duration = random.uniform(1, 4)  # Max 4 hours continuous driving
                        ...
                        self.driver_logs.append({
                            ...
                            'location': fake.city() + ', ' + fake.state_abbr(),
                            'vehicle_id': random.choice([v['vehicle_id'] for v in self.vehicles
    ...
    def generate_hos_violations(self):
        ...
        violation_types = ['11_hour_driving', '14_hour_on_duty', '30_minute_break',
                         '60_hour_7_day', '70_hour_8_day']
        ...
        for driver in self.drivers:
            if random.random() < 0.2:  # 20% of drivers have violations
                ...
                for _ in range(num_violations):
                    ...
                    violation_date = fake.date_between(start_date=self.start_date, end_date='today')
                    ...
                    self.hos_violations.append({
                        ...
                        'corrective_action': random.choice(['Warning issued', 'Training required', 'Suspension']),
    ...
    def generate_dvir_reports(self):
        ...
        defect_items = ['Brakes', 'Tires', 'Lights', 'Horn', 'Mirrors', 'Windshield',
                       'Coupling Devices', 'Emergency Equipment']
            ...
            if trip['status'] == 'completed':
                ...
                self.dvir_reports.append({
                    ...
                    'defects_json': json.dumps({
                        'defects': random.sample(defect_items, random.randint(1, 3))
                    }) if has_defects else None,
                    'driver_signature': f"DSig_{trip['driver_id']}",
                    'mechanic_signature': f"MSig_{random.randint(1, 10)}" if has_defects else None,
                    'repair_date': trip['start_time'].date() if has_defects else None,
                    'created_at': trip['start_time']
                })
        ...
        for driver in self.drivers:
            if driver['status'] == 'active':
                ...
                while current <= datetime.now():
                    ...
                    driver_events = [e for e in self.driver_events if e['driver_id'] == driver['driver_id']]
                    base_score = 85
                    ...
                    self.driver_scores.append({
                        ...
                        'safety_score': min(100, safety_score + random.randint(-5, 10)),
    ...
    def generate_messages(self):
        ...
        subjects = ['New assignment', 'Route change', 'Weather alert', 'Delivery update',
                   'Break reminder', 'Maintenance required']

        for _ in range(200):  # Generate 200 messages
            self.message_id += 1

            sender_driver = random.choice(self.drivers)
            recipient_driver = random.choice([d for d in self.drivers
                                            if d['company_id'] == sender_driver['company_id']])

            sent_time = fake.date_time_between(start_date=self.start_date, end_date='now')
            ...
            self.messages.append({
                ...
                'sent_time': sent_time,
                'read_time': sent_time + timedelta(minutes=random.randint(1, 60)) if random.random() > 0.2 else None,
    ...
    def generate_vehicle_daily_summary(self):
        ...
        for vehicle in self.vehicles:
            ...
            while current <= date.today():
                ...
                daily_trips = [t for t in self.trips
                ...
                if daily_trips:
                    ...
                    self.vehicle_daily_summary.append({
                        ...
                        'average_mpg': round(total_miles / total_fuel, 2) if total_fuel > 0 else 0,
                        'max_speed_mph': max(t['max_speed_mph'] for t in daily_trips),
    ...
    def generate_summary(self):
        ...
        total_miles = sum(t['distance_miles'] for t in self.trips if t['distance_miles'])
        total_fuel = sum(t['fuel_used_gallons'] for t in self.trips if t['fuel_used_gallons'])
