...
class FoodDeliveryGenerator(BaseGenerator):
    ...
    def generate_delivery_zones(self):
        ...
        for zone_name in self.zones:
            ...
            zone_polygon = json.dumps(
                {"type": "Polygon", "coordinates": [polygon_points]}
            )
            ...
            zone = (
                ...
                (
                    random.uniform(1.0, 2.0) if random.random() > 0.5 else 1.0
                ),  # surge_multiplier
    ...
    def generate_restaurants(self, count: int = 500):
        ...
        for i in range(count):
            ...
            restaurant = (
                ...
                random.sample(
                    self.cuisines, random.randint(1, 3)
                ),  # cuisine_types as JSON
                self.faker.address(),
                location,
        ...
        for restaurant in restaurants:
            ...
            for _ in range(num_items):
                ...
                menu_item = (
                    ...
                    (
                        random.uniform(0, 20) if random.random() > 0.5 else None
                    ),  # discount_price
                    ...
                    (
                        random.randint(0, 1000) if random.random() > 0.3 else 0
                    ),  # times_ordered
                    (
                        random.uniform(3.5, 5.0) if random.random() > 0.3 else None
                    ),  # avg_rating
                    self.faker.date_time_between("-1 year", "now"),
                )
    ...
    def generate_drivers(self, count: int = 1000):
        ...
        for i in range(count):
            ...
            if vehicle_type == "car":
                ...
                vehicle_model = None
                license_plate = (
                    None if vehicle_type == "bicycle" else self.faker.license_plate()
                )
            ...
            driver = (
                ...
                (
                    random.uniform(0.85, 0.99) if random.random() > 0.2 else None
                ),  # acceptance_rate
                (
                    random.uniform(0.90, 0.99) if random.random() > 0.2 else None
                ),  # completion_rate
                random.uniform(15, 45),  # avg_delivery_time
                (
                    self.faker.date_time_between("-30 days", "now")
    ...
    def generate_orders(self, orders_per_day: int = 2000):
        ...
        restaurants = self.fetch_all(
            "SELECT restaurant_id, zone_id, avg_preparation_time FROM restaurants WHERE verification_status = 'approved'"
        )
        drivers = self.fetch_all(
            "SELECT driver_id, current_zone_id FROM drivers WHERE verification_status = 'approved'"
        )
        menu_items = self.fetch_all(
            "SELECT item_id, restaurant_id, price FROM menu_items WHERE is_available = 1"
        )
        ...
        for days_ago in range(30, 0, -1):
            ...
            daily_orders = random.randint(
                int(orders_per_day * 0.7), int(orders_per_day * 1.3)
            )
            ...
            for _ in range(daily_orders):
                ...
                payment_method = random.choice(
                    ["credit_card", "debit_card", "paypal", "cash", "wallet"]
                )
                ...
                order_time = order_date + timedelta(
                    hours=random.randint(11, 22), minutes=random.randint(0, 59)
                )
                ...
                order_record = (
                    ...
                    delivery_location,
                    random.choice(
                        ["Leave at door", "Hand to me", "Meet outside", None]
                    ),
                    random.uniform(0, 60) if order_status == "delivered" else None,
                    order_time,
                    (
                        order_time + timedelta(minutes=random.randint(20, 60))
                ...
                if order_status not in ["pending", "cancelled"]:
                    driver = random.choice(
                        [
                            d
                            for d in drivers
                            if d["current_zone_id"] == restaurant["zone_id"]
                        ]
                    )
                    ...
                    delivery = (
                        ...
                        (
                            order_time + timedelta(minutes=random.randint(25, 35))
                            if order_status == "delivered"
                            else None
                        ),
                        (
                            order_time + timedelta(minutes=random.randint(30, 60))
                            if order_status == "delivered"
                            else None
                        ),
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate test data for Food Delivery Platform"
    )
    ...
    parser.add_argument(
        "--customers", type=int, default=5000, help="Number of customers"
    )
    parser.add_argument(
        "--restaurants", type=int, default=500, help="Number of restaurants"
    )
    parser.add_argument("--drivers", type=int, default=1000, help="Number of drivers")
    parser.add_argument(
        "--orders-per-day", type=int, default=2000, help="Average orders per day"
    )
