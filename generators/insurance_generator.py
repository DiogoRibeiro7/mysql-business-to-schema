...
class InsuranceGenerator(BaseGenerator):
    ...
    def generate_customers(self, count: int = 10000):
        ...
        for i in range(count):
            customer_type = self.faker.random_element(
                [("individual", 0.85), ("corporate", 0.15)]
            )
        ...
        for customer in customers:
            ...
            for i in range(num_beneficiaries):
                ...
                beneficiary = (
                    ...
                    (
                        100.0
                        if is_primary and num_beneficiaries == 1
    ...
    def generate_agencies_and_agents(self, total_agents: int = 500):
        ...
        for i in range(total_agents):
            ...
            manager_id = (
                random.randint(1, int(total_agents * 0.1))
                if not is_manager and random.random() > 0.5
                else None
            )

            agent = (
                (
                    random.choice(agency_ids) if random.random() > 0.2 else None
                ),  # 80% belong to agencies
                ...
                manager_id,
                json.dumps(
                    random.sample(self.product_types, random.randint(1, 4))
                ),  # licensed_products
                json.dumps(
                    random.sample(["CA", "NY", "TX", "FL", "IL"], random.randint(1, 3))
                ),  # licensed_states
    ...
    def generate_brokers(self):
        ...
        for i in range(50):
            ...
            broker = (
                ...
                self.faker.address(),
                json.dumps(
                    random.sample(["CA", "NY", "TX", "FL", "IL"], random.randint(1, 5))
                ),
        ...
        self.bulk_insert(
            ...
            [
                ...
                "commission_rate",
                "status",
                "total_policies_referred",
                "total_premium_referred",
                "created_at",
            ],
        )
            ...
            for product_name, product_code, coverage_range in type_products:
                product = (
                    ...
                    (
                        ...
                        else random.uniform(20, 200)
                    ),  # min_premium
                    (
                        random.uniform(5000, 50000)
                        if product_type != "life"
                        else random.uniform(200, 5000)
                    ),  # max_premium
                    (
                        random.randint(100, 5000)
                        if product_type in ["auto", "home"]
                        else 0
                    ),  # deductible
                    ...
                    (
                        json.dumps(["medical_exam"]) if product_type == "life" else None
        ...
        for customer in customers:
            ...
            for _ in range(num_apps):
                ...
                if product["product_type"] == "life":
                    ...
                    medical_exam_completed = medical_exam_required and random.choice(
                        [0, 1]
                    )
                ...
                if customer["risk_category"] in ["low", "medium"]:
                    decision = self.faker.random_element(
                        [("approved", 0.8), ("pending", 0.15), ("rejected", 0.05)]
                    )
                else:
                    decision = self.faker.random_element(
                        [
                ...
                application = (
                    ...
                    (
                        None
                        if not medical_exam_completed
                        else self.faker.date_time_between("-60 days", "now")
                    ),
                    (
                        random.choice([0, 1])
                        if product["product_type"] == "home"
                        else 0
                    ),  # property_inspection_required
                    (
                        random.choice([0, 1])
                        if product["product_type"] == "home"
                        else 0
                    ...
                    decision,
                    (
                        random.uniform(50, 5000) if decision == "approved" else None
                    ),  # final_premium
                    (
                        None
                        if decision != "rejected"
                            ...
                            [
                                ...
                                "Medical conditions",
                                "Age limit exceeded",
                                "Coverage amount too high",
                                "Incomplete documentation",
                            ]
                        )
                    ),
                    (
                        random.randint(1, 10) if decision != "pending" else None
                    ),  # underwriter_id
    ...
    def generate_policies(self, count: int = 15000):
        ...
        agents = self.fetch_all("SELECT agent_id FROM agents WHERE status = 'active'")
        brokers = self.fetch_all(
            "SELECT broker_id FROM brokers WHERE status = 'active'"
        )
    ...
    def create_policy_from_application(
        ...
        term_months = (
            ...
            if app["product_type"] != "life"
            else random.choice([120, 240, 360])
        )
        ...
        return (
            ...
            app["agent_id"],
            (
                random.choice(brokers)["broker_id"]
                if brokers and random.random() > 0.8
                else None
            ),
            app["application_id"],
            status,
            ...
            (
                ...
                if status == "active"
                    ...
                    self.faker.date_between(effective_date, expiry_date)
                    if status == "cancelled"
                    else None
                )
            ),
            (
                ...
                if status == "active"
                    ...
                    random.choice(
                        ["non_payment", "customer_request", "fraud", "risk_change"]
                    )
                    if status == "cancelled"
                    else None
                )
    ...
    def create_direct_policy(
        ...
        return (
            ...
            (
                random.choice(brokers)["broker_id"]
                if brokers and random.random() > 0.9
                else None
            ),
            ...
            (
                ...
                if status == "active"
                    ...
                    random.choice(["non_payment", "customer_request", "fraud"])
                    if status == "cancelled"
                    else None
                )
            ),
    ...
    def generate_policy_beneficiaries(self):
        ...
        for policy in life_policies:
            customer_beneficiaries = beneficiaries_by_customer.get(
                policy["customer_id"], []
            )
        ...
        for policy in policies:
            ...
            if random.random() > 0.7:
                ...
                for _ in range(num_claims):
                    ...
                    claimed_amount = random.uniform(
                        100, min(float(policy["coverage_amount"]), 50000)
                    )
                    ...
                    claim = (
                        ...
                        paid_amount,
                        (
                            500 if policy["product_type"] in ["auto", "home"] else 0
                        ),  # deductible_applied
                        status,
                        (
                        ...
                        fraud_score,
                        (
                            random.choice([0, 1]) if fraud_suspected else 0
    ...
    def generate_commissions(self):
        ...
        for policy in agent_policies:
            commission_amount = float(policy["premium_amount"]) * float(
            ...
            commission = (
                ...
                (
                    self.faker.date_between("-30 days", "today")
        ...
        for policy in broker_policies:
            commission_amount = (
                float(policy["premium_amount"]) * 0.05
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate test data for Insurance Management Platform"
    )
    ...
    parser.add_argument(
        "--password", default="insurance_pass_2024", help="MySQL password"
    )
    parser.add_argument(
        "--database", default="insurance_platform", help="MySQL database"
    )
    parser.add_argument(
        "--customers", type=int, default=10000, help="Number of customers"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.5,
        help="Average policies per customer",
    )
