...
class LogisticsGenerator:
            ...
            self.warehouses.append(
                {
                    ...
                    "is_climate_controlled": warehouse_types[i % len(warehouse_types)]
                    == "Cold Storage",
                    "is_active": True,
                ...
                self.warehouse_zones.append(
                    {
                        ...
                        "temperature_range": (
                            "-20 to -10"
                            if warehouse["is_climate_controlled"]
                            else "15 to 25"
                        ),
        ...
        for product in self.products:
            ...
            for warehouse in selected_warehouses:
                ...
                if storage_zones:
                    ...
                    zone_bins = [
                        b
                        for b in self.warehouse_bins
                        if b["zone_id"] == zone["zone_id"]
                    ]
                    bin_location = (
                        random.choice(zone_bins)["bin_id"] if zone_bins else None
                    )
                else:
                    bin_location = None
                ...
                self.inventory_levels.append(
                    {
                        ...
                        "last_movement_date": fake.date_between(
            ...
            if inventory["quantity_on_hand"] > 0:
                ...
                for b in range(num_batches):
                    ...
                    product = next(
                        ...
                        if p["product_id"] == inventory["product_id"]
                    )
            ...
            self.customers.append(
                {
                    ...
                    "payment_terms": (
                        random.choice(["COD", "Net 30", "Net 60"])
                        if customer_type in ["B2B", "Wholesale"]
                        else "Immediate"
                    ),
                ...
                self.purchase_orders.append(
                    {
                        ...
                        "actual_delivery": (
                            order_date
                            + timedelta(
                                days=supplier["lead_time_days"] + random.randint(-2, 5)
                            )
                    ...
                    self.purchase_order_items.append(
                        {
                            ...
                            "quantity_received": (
                                quantity if status in ["Received", "Closed"] else 0
                            ),
    ...
    def generate_sales_orders(self):
        ...
        while current <= datetime.now():
            ...
            for _ in range(daily_orders):
                    ...
                    self.sales_order_items.append(
                        {
                            ...
                            "quantity_ordered": quantity,
                            "quantity_shipped": (
                                quantity if status in ["Shipped", "Delivered"] else 0
                            ),
                            "unit_price": unit_price,
                ...
                self.sales_orders[-1]["total_amount"] = round(
                    order_subtotal + tax + self.sales_orders[-1]["shipping_cost"], 2
                )
    ...
    def generate_carriers(self):
        ...
        for i in range(CONFIG["carriers"]):
            ...
            name = carrier_names[i % len(carrier_names)] + (
                f" {i//len(carrier_names) + 1}" if i >= len(carrier_names) else ""
            )
            ...
            self.carriers.append(
                {
                    ...
                    "api_key": hashlib.md5(
                        f"key_{self.carrier_id}".encode()
                    ).hexdigest(),
                    "is_active": True,
                    "created_at": datetime.now()
                    - timedelta(days=random.randint(365, 730)),
                }
            )
    ...
    def generate_shipments(self):
        ...
        shipped_orders = [
            o for o in self.sales_orders if o["status"] in ["Shipped", "Delivered"]
        ]
        ...
        for order in shipped_orders:
            ...
            carrier_services = [
                s
                for s in self.carrier_services
            ...
            order_items = [
                i for i in self.sales_order_items if i["so_id"] == order["so_id"]
            ]
            total_weight = sum(
                next(p for p in self.products if p["product_id"] == item["product_id"])[
                    "weight_kg"
                ]
            ...
            self.shipments.append(
                {
                    ...
                    "estimated_delivery": order["shipped_date"]
                    + timedelta(days=service["transit_time_days"]),
                    "actual_delivery": order["delivered_date"],
                    "status": (
                        "Delivered" if order["status"] == "Delivered" else "In Transit"
                    ),
            ...
            self.vehicles.append(
                {
                    ...
                    "capacity_kg": (
                        ...
                        if vehicle_type != "Semi-Trailer"
                        else random.randint(20000, 30000)
                    ),
                    ...
                    "assigned_warehouse_id": random.choice(self.warehouses)[
                        "warehouse_id"
                    ],
                    "created_at": datetime.now()
            ...
            self.drivers.append(
                {
                    ...
                    "hire_date": hire_date,
                    "assigned_vehicle_id": (
                        random.choice(self.vehicles)["vehicle_id"]
                    ...
                    self.routes.append(
                        {
                            ...
                            "completed_stops": (
                                num_stops if current < date.today() else 0
                            ),
                            ...
                            "status": (
                                "Completed" if current < date.today() else "Planned"
                            ),
                ...
                if available_drivers:
                    ...
                    vehicle = next(
                        (
                            ...
                            if v["vehicle_id"] == driver["assigned_vehicle_id"]
                        ),
                        None,
                    )
                    ...
                    if not vehicle:
                        ...
                        vehicle = (
                            random.choice(available_vehicles)
                        ...
                        self.delivery_runs.append(
                            {
                                ...
                                "end_time": (
                                    start_time
                                    + timedelta(hours=route["actual_duration_hours"])
                                    if route["actual_duration_hours"]
                                    else None
                                ),
                                "start_mileage": vehicle["current_mileage"],
                                "end_mileage": (
                                    vehicle["current_mileage"]
                                    + int(route["total_distance_km"] * 0.621371)
                                    if route["status"] == "Completed"
                                    else None
                                ),
                                "fuel_used_liters": (
                                    route["total_distance_km"] * 0.08
                                    if route["status"] == "Completed"
                                    else None
                                ),
                                "status": route["status"],
    ...
    def generate_inventory_movements(self):
        ...
        for po in self.purchase_orders[-100:]:  # Last 100 POs
            if po["status"] == "Received":
                po_items = [
                    i for i in self.purchase_order_items if i["po_id"] == po["po_id"]
                ]
                ...
                for item in po_items:
                    ...
                    inventory = next(
                        (
                            ...
                            and inv["warehouse_id"] == po["warehouse_id"]
                        ),
                        None,
                    )
    ...
    def generate_kpi_metrics(self):
        ...
        while current <= date.today():
            for warehouse in self.warehouses:
                ...
                fulfillment_rate = (
                    (orders_shipped / orders_received * 100)
                ...
                total_inventory_value = sum(
                    ...
                    * next(
                        p for p in self.products if p["product_id"] == inv["product_id"]
                    )["unit_cost"]
            ...
            self.audit_log.append(
                {
                    ...
                    "old_values": (
                        json.dumps({"field": "old_value"})
                        if random.choice(actions) == "UPDATE"
                        else None
                    ),
                    "new_values": (
                        json.dumps({"field": "new_value"})
                        if random.choice(actions) in ["CREATE", "UPDATE"]
                        else None
                    ),
    ...
    def save_all(self):
        ...
        datasets = [
            ...
            (
                ...
                self.purchase_order_items[:5000],
            ),  # Limit for demo
    ...
    def generate_summary(self):
        ...
        total_inventory_value = sum(
            inv["quantity_on_hand"]
            * next(p for p in self.products if p["product_id"] == inv["product_id"])[
                "unit_cost"
            ]
            for inv in self.inventory_levels
        )
        ...
        delivered_orders = len(
            [o for o in self.sales_orders if o["status"] == "Delivered"]
        )
