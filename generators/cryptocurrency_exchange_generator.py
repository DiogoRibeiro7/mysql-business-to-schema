...
class CryptocurrencyExchangeGenerator(BaseGenerator):
    ...
    def generate_users(self, count: int = 10000):
        ...
        for i in range(count):
            ...
            user_type = self.faker.random_element(
                [("individual", 0.85), ("corporate", 0.10), ("institutional", 0.05)]
            )
            ...
            user = (
                ...
                (
                    random.choice([0, 1])
                    if kyc_level in ["verified", "premium", "institutional"]
                    else 0
                ),  # kyc_verified
                (
                    self.faker.date_time_between("-1 year", "-1 day")
                    if kyc_level != "basic"
                    else None
                ),
    ...
    def generate_trading_pairs(self):
        ...
        for pair in self.trading_pairs[:50]:  # Limit to 50 most popular pairs
            ...
            trading_pair = (
                ...
                random.choice([0, 1]),  # is_active
                (
                    random.uniform(0.0001, 1)
                    if base == "BTC"
                    else random.uniform(0.00001, 0.1)
                ),
                (
                    random.uniform(100, 1000000)
                    if base == "BTC"
                    else random.uniform(10, 100000)
                ),
                (
                    random.uniform(0.0001, 0.001)
                    if is_major
                    else random.uniform(0.001, 0.003)
                ),
                (
                    random.uniform(0.0001, 0.001)
                    if is_major
        ...
        for user in users:
            ...
            for currency in user_currencies:
                ...
                address = (
                    ...
                    if currency["symbol"] in ["ETH", "MATIC"]
        ...
        for pair in pairs[:20]:  # Limit to top 20 pairs for performance
            ...
            for days_ago in range(30, 0, -1):
                for hour in range(24):
                    ...
                    # Apply volatility
                    volatility = self.volatility.get(
                        pair["base_currency"], self.volatility["default"]
                    )
            ...
            for _ in range(num_orders):
                ...
                filled_quantity = (
                    ...
                    if status == "filled"
                        ...
                        quantity * random.uniform(0.1, 0.9)
                ...
                order = (
                    ...
                    order_time,
                    (
                        order_time + timedelta(seconds=random.randint(1, 300))
            ...
            for _ in range(num_transactions):
                ...
                if tx_type == "deposit":
                    amount = (
                        ...
                        if wallet["currency"] in self.fiat_currencies
                        else random.uniform(0.001, 10)
                    )
                    status = self.faker.random_element(
                        [("completed", 0.8), ("pending", 0.15), ("failed", 0.05)]
                    )
                else:
                    # Withdrawal - check balance
                    max_amount = (
                        float(wallet["available_balance"]) * 0.5
                ...
                transaction = (
                    ...
                    (
                        ...
                        if wallet["currency"] not in self.fiat_currencies
                        else None
                    ),
                    (
                        wallet["address"]
                        if tx_type == "deposit"
                        else "0x" + secrets.token_hex(20)
                    ),
    ...
    def generate_staking(self):
        ...
        users = self.fetch_all(
            "SELECT user_id FROM users WHERE status = 'active' LIMIT 200"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate test data for Cryptocurrency Exchange"
    )
    ...
    parser.add_argument(
        "--users", type=int, default=10000, help="Number of users to generate"
    )
    parser.add_argument(
        "--orders-per-user", type=int, default=50, help="Average orders per user"
    )
    ...
    try:
        ...
        generator.generate_all_data(
            users=args.users, orders_per_user=args.orders_per_user
        )
    finally:
