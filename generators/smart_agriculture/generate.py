...
class SmartAgricultureGenerator:
    ...
    def generate_all(self):
        ...
        print(f"[OK] Generated data for {len(self.farms)} farms, "
    ...
    def _generate_farms(self):
        ...
        farm_names = ["Green Valley", "Sunshine", "Harvest Moon", "Golden Fields", "Spring Brook"]
        ...
        for farm in self.farms:
            ...
            for field_num in range(fields_per_farm):
                field = {
                    ...
                    'last_soil_test': datetime.now() - timedelta(days=random.randint(30, 365)),
    ...
    def _generate_crops(self):
        ...
        for crop_name, crop_config in list(self.config['crop_types'].items())[:self.config['counts']['crops']]:
            crop = {
                'crop_id': crop_id,
                'crop_name': crop_name.title(),
                'variety': f"{crop_name.upper()}-{random.randint(100, 999)}",
                'crop_type': 'grain' if crop_name in ['wheat', 'corn', 'rice'] else 'vegetable',
                'growth_days': crop_config['growth_days'],
                'water_requirement_mm_per_day': 5 if crop_config['water_needs'] == 'high' else 3,
    ...
    def _generate_plantings(self):
        ...
        for field in self.fields:
            ...
            planting = {
                ...
                'expected_harvest_date': planting_date + timedelta(days=crop['growth_days']),
    ...
    def _generate_sensors(self):
        ...
        for zone in self.zones:
            ...
            for _ in range(sensors_per_zone):
                sensor_type = random.choice(list(self.config['sensor_types'].keys())[:4])
                ...
                sensor = {
                    ...
                    'manufacturer': random.choice(['AgroSense', 'FarmTech', 'SmartSoil']),
                    'model': f"Model-{random.randint(100, 999)}",
                    'installation_date': datetime.now() - timedelta(days=random.randint(90, 730)),
                    'last_calibration': datetime.now() - timedelta(days=random.randint(7, 90)),
                    'battery_level': random.randint(20, 100),
                    'status': random.choices(['active', 'maintenance', 'faulty'],
        ...
        for sensor in active_sensors:
            ...
            while current_time <= end_date and reading_count < max_readings:
                ...
                reading = {
                    ...
                    'quality': 'good' if sensor['battery_level'] > 20 else 'low_battery'
                }
                self.sensor_readings.append(reading)

                current_time += timedelta(minutes=sensor_config.get('sampling_minutes', 15))
                reading_count += 1
    ...
    def _generate_ndvi_readings(self):
        ...
        for planting in self.plantings[:20]:  # Limit for performance
            field = next(f for f in self.fields if f['field_id'] == planting['field_id'])
            ...
            for day in range(min(7, self.config['counts']['days_of_data'])):
                ...
                ndvi_reading = {
                    ...
                    'ndvi_value': round(random.uniform(ndvi_ranges[0], ndvi_ranges[1]), 3),
                    'growth_stage': growth_stage,
                    'days_after_planting': days_since_planting - day,
                    'satellite_source': random.choice(['Sentinel-2', 'Landsat-8', 'Drone']),
                ...
                if needs_irrigation:
                    ...
                    event = {
                        ...
                        'water_source': random.choices(
                            ...
                            weights=list(self.config['irrigation']['water_sources'].values())
                        )[0]
                    }
        ...
        for field in self.fields:
            ...
            for _ in range(num_detections):
                pest_type = random.choices(
                    ...
                    weights=[self.config['pest_diseases'][p]['probability']
                ...
                detection = {
                    ...
                    'affected_area_pct': random.uniform(5, pest_config['damage_rate'] * 100),
                    'treatment_applied': pest_config['treatment'],
                    'treatment_date': datetime.now() - timedelta(days=random.randint(0, 7)),
                    'treatment_effectiveness': random.uniform(0.7, 0.95),
                    'detection_method': random.choice(['visual', 'drone', 'sensor', 'satellite']),
    ...
    def _generate_fertilizer_applications(self):
        ...
        for planting in self.plantings:
            ...
            for app_num in range(num_applications):
                fertilizer_type = random.choice(list(self.config['fertilizer']['types'].keys()))
                rate_range = self.config['fertilizer']['application_rates'][fertilizer_type]

                application = {
                    'application_id': self.fertilizer_id,
                    'field_id': planting['field_id'],
                    'application_date': planting['planting_date'] + timedelta(days=30 * app_num),
                    'fertilizer_type': fertilizer_type,
                    'application_rate_kg_per_hectare': random.uniform(rate_range[0], rate_range[1]),
                    'application_method': random.choice(['broadcast', 'banding', 'foliar', 'injection']),
    ...
    def _generate_harvest_data(self):
        ...
        for planting in self.plantings:
            ...
            if random.random() < 0.2:  # 20% chance of suboptimal conditions
                yield_factor *= random.choice(list(self.config['yield_factors'].values()))
            ...
            harvest = {
                ...
                'harvest_date': planting['expected_harvest_date'] + timedelta(days=random.randint(-7, 7)),
                ...
                'notes': 'Good harvest' if yield_factor >= 0.9 else 'Below average yield'
            }
                ...
                for animal_type, config in list(self.config['livestock'].items())[:2]:
                    ...
                    for _ in range(min(num_animals, 20)):  # Limit for performance
                        animal = {
                            ...
                            'birth_date': datetime.now() - timedelta(days=random.randint(180, 1825)),
                            'weight_kg': random.uniform(200, 800) if animal_type == 'cattle' else random.uniform(50, 150),
                            'gender': random.choice(['male', 'female']),
                            'health_status': random.choice(['healthy', 'monitoring', 'treatment']),
        ...
        for animal in self.animals[:20]:  # Limit for performance
            ...
            for day in range(min(7, self.config['counts']['days_of_data'])):
                ...
                health_reading = {
                    ...
                    'feed_intake_kg': animal_config.get('feed_per_day', 10) * random.uniform(0.8, 1.2),
                    'water_intake_liters': animal_config.get('water_per_day', 50) * random.uniform(0.8, 1.2),
                    'weight_kg': animal['weight_kg'] + random.uniform(-2, 2),
                    'notes': None if animal['health_status'] == 'healthy' else 'Under observation'
                }
                ...
                for day in range(self.config['counts']['days_of_data']):
                    ...
                    for feeding_num in range(2):  # Morning and evening feeding
                        feeding = {
                            ...
                            'location': location,
                            'feeding_time': feeding_time.replace(hour=6 if feeding_num == 0 else 18),
                            'feed_type': random.choice(['hay', 'grain', 'silage', 'mixed']),
                            'quantity_kg': len(location_animals) * random.uniform(5, 15),
