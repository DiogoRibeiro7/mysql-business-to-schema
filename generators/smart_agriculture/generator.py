...
class SmartAgricultureGenerator:
    ...
    def generate_all(self):
        ...
        print(f"  Total zones: {CONFIG['farms'] * CONFIG['fields_per_farm'] * CONFIG['zones_per_field']}")
    ...
    def generate_operators(self):
        ...
        roles = ['Farm Manager', 'Field Supervisor', 'Equipment Operator',
                'Irrigation Specialist', 'Agronomist', 'Veterinarian']
    ...
    def generate_fields(self):
        ...
        for farm in self.farms:
            ...
            for j in range(CONFIG['fields_per_farm']):
                ...
                lat = farm['latitude']
                lng = farm['longitude']
                field_size = math.sqrt(area_per_field * 10000) / 111000  # Convert hectares to degrees
                ...
                self.fields.append({
                    ...
                    'area_hectares': round(area_per_field * random.uniform(0.8, 1.2), 2),
                    ...
                    'last_soil_test': fake.date_between(start_date='-2y', end_date='today'),
                    'created_at': datetime.now()
                })
        ...
        for field in self.fields:
            ...
            for k in range(CONFIG['zones_per_field']):
                ...
                characteristics = {
                    ...
                    'compaction_level': random.choice(['none', 'slight', 'moderate', 'severe'])
                }
                ...
                self.zones.append({
                    ...
                    'area_hectares': round(area_per_zone * random.uniform(0.9, 1.1), 2),
    ...
    def generate_crops(self):
        ...
        crop_data = [
            ('Corn', 'Zea mays', 'Poaceae', 'cereal', 120, 10, 15, 30, 5.0, 150, 60, 120, 6.0, 7.0),
            ('Wheat', 'Triticum aestivum', 'Poaceae', 'cereal', 110, 0, 12, 25, 4.0, 120, 50, 100, 6.0, 7.5),
            ('Soybeans', 'Glycine max', 'Fabaceae', 'legume', 100, 10, 18, 30, 4.5, 20, 40, 60, 6.0, 7.0),
            ('Rice', 'Oryza sativa', 'Poaceae', 'cereal', 130, 10, 20, 35, 10.0, 100, 40, 80, 6.0, 7.0),
            ('Potato', 'Solanum tuberosum', 'Solanaceae', 'vegetable', 90, 7, 15, 25, 4.0, 120, 60, 150, 5.0, 6.5),
            ('Tomato', 'Solanum lycopersicum', 'Solanaceae', 'vegetable', 80, 10, 18, 28, 5.0, 150, 60, 200, 6.0, 7.0),
            ('Lettuce', 'Lactuca sativa', 'Asteraceae', 'vegetable', 60, 5, 15, 22, 3.0, 100, 40, 120, 6.0, 7.0),
            ('Apple', 'Malus domestica', 'Rosaceae', 'fruit', 150, 7, 15, 25, 4.0, 80, 40, 120, 6.0, 7.5),
            ('Alfalfa', 'Medicago sativa', 'Fabaceae', 'forage', 60, 5, 15, 28, 6.0, 20, 50, 200, 6.5, 7.5),
            ('Cotton', 'Gossypium hirsutum', 'Malvaceae', 'cash_crop', 140, 15, 20, 32, 5.0, 120, 40, 60, 6.0, 7.5),
            ('Barley', 'Hordeum vulgare', 'Poaceae', 'cereal', 95, 0, 12, 24, 3.5, 100, 40, 80, 6.0, 7.8),
            ('Sunflower', 'Helianthus annuus', 'Asteraceae', 'cash_crop', 100, 6, 18, 28, 4.5, 80, 60, 60, 6.0, 7.5),
            ('Canola', 'Brassica napus', 'Brassicaceae', 'cash_crop', 110, 5, 12, 24, 3.5, 150, 60, 100, 5.8, 7.0),
            ('Sugar Beet', 'Beta vulgaris', 'Amaranthaceae', 'cash_crop', 120, 3, 15, 25, 5.0, 120, 40, 120, 6.5, 8.0),
            ('Grapes', 'Vitis vinifera', 'Vitaceae', 'fruit', 150, 10, 18, 30, 3.5, 30, 30, 100, 6.0, 7.0)
        ]
        ...
        for field in self.fields:
            ...
            for _ in range(num_plantings):
                ...
                if harvest_date < today:
                    status = 'harvested'
                    actual_harvest = harvest_date + timedelta(days=random.randint(-5, 5))
                ...
                actual_yield = expected_yield * random.uniform(0.8, 1.2) if status == 'harvested' else None

                self.planting_records.append({
                    'planting_id': self.planting_id,
                    'field_id': field['field_id'],
                    'zone_id': random.choice([z['zone_id'] for z in self.zones if z['field_id'] == field['field_id']]),
                    'crop_id': crop['crop_id'],
                    'variety': random.choice(varieties.get(crop['crop_name'], ['Standard'])),
                    'planting_date': planting_date,
                    'expected_harvest_date': harvest_date,
                    'actual_harvest_date': actual_harvest,
                    'area_planted_hectares': field['area_hectares'] * random.uniform(0.8, 1.0),
        ...
        for planting in self.planting_records:
            if planting['status'] in ['growing', 'harvested']:
                ...
                for stage_name, stage_code, progress in stages:
                    ...
                    observation_date = planting['planting_date'] + timedelta(days=days_to_stage)
    ...
    def generate_sensors(self):
        ...
        sensor_types = ['soil_moisture', 'soil_temperature', 'soil_ph', 'air_temperature',
                       'humidity', 'light', 'rainfall', 'wind_speed']
        manufacturers = ['Davis Instruments', 'METER Group', 'Campbell Scientific',
                        'Sentek', 'AquaCheck']
        ...
        for zone in self.zones:
            ...
            for _ in range(num_sensors):
                ...
                self.sensors.append({
                    ...
                    'depth_cm': random.randint(10, 60) if 'soil' in sensor_type else None,
                    'height_m': random.uniform(1, 3) if 'soil' not in sensor_type else None,
            ...
            for day_of_week in range(7):
                if random.random() < 0.6:  # Not every day
                    ...
                    self.irrigation_schedules.append({
                        ...
                        'flow_rate_liters_per_minute': system['capacity_liters_per_hour'] / 60,
                        'is_active': True,
                        'created_at': datetime.now()
                    })
        ...
        while current <= end:
            for sensor in self.sensors[:50]:  # Limit for demo
                ...
                self.sensor_readings.append({
                    ...
                    'quality_flag': random.choice(['good', 'good', 'good', 'suspect']) if random.random() > 0.95 else 'good',
                    'created_at': current
                })
        ...
        while current <= end:
            for station in self.weather_stations:
                ...
                self.weather_data.append({
                    ...
                    'precipitation_mm': random.uniform(0, 5) if random.random() < 0.1 else 0,
                    'solar_radiation_wm2': max(0, 800 * math.sin((hour - 6) * math.pi / 12)) if 6 <= hour <= 18 else 0,
                    'uv_index': max(0, int(10 * math.sin((hour - 6) * math.pi / 12))) if 6 <= hour <= 18 else 0,
                    'created_at': current
                })
    ...
    def generate_irrigation_events(self):
        ...
        for schedule in self.irrigation_schedules:
            system = next(s for s in self.irrigation_systems if s['system_id'] == schedule['system_id'])
            ...
            while current <= end:
                if current.weekday() == schedule['day_of_week']:
                    ...
                    water_used = schedule['flow_rate_liters_per_minute'] * schedule['duration_minutes']
    ...
    def generate_fertilizer_applications(self):
        ...
        for planting in self.planting_records:
            ...
            for i in range(num_applications):
                ...
                application_date = planting['planting_date'] + timedelta(days=days_after_planting)
    ...
    def generate_pesticide_applications(self):
        ...
        for planting in self.planting_records:
            ...
            if random.random() < 0.7:
                ...
                application_date = planting['planting_date'] + timedelta(days=random.randint(20, 60))
        ...
        for planting in self.planting_records:
            if planting['status'] == 'harvested':
                ...
                self.harvest_records.append({
                    ...
                    'yield_kg': planting['actual_yield_kg_per_hectare'] * planting['area_planted_hectares'],
        ...
        for planting in self.planting_records:
            if planting['status'] == 'growing':
                ...
                while current <= date.today():
                    ...
                    predicted_yield = planting['expected_yield_kg_per_hectare'] * random.uniform(
                        1 - (1 - accuracy_factor) * 0.3,
                        1 + (1 - accuracy_factor) * 0.3
                    )
                    ...
                    self.yield_predictions.append({
                        ...
                        'model_version': '1.0',
                        'factors_considered': json.dumps(['weather', 'soil', 'growth_stage']),
                        'created_at': current
                    })
            ...
            if farm['farm_type'] in ['dairy', 'mixed']:
                ...
                for _ in range(num_animals):
                    ...
                    self.animals.append({
                        ...
                        'gender': random.choice(['F', 'F', 'F', 'M']),  # More females
                        'weight_kg': random.uniform(400, 700),
                        'status': random.choice(['active', 'active', 'active', 'pregnant', 'dry']),
        ...
        for animal in self.animals:
            ...
            for _ in range(num_records):
                ...
                self.health_records.append({
                    ...
                    'medication': random.choice(['Vaccine A', 'Antibiotic B', 'Dewormer C', None]),
                    'dosage': f"{random.randint(5, 20)} ml" if random.random() > 0.3 else None,
                    'next_visit_date': record_date + timedelta(days=random.randint(30, 180)),
            ...
            if animal['gender'] == 'F' and animal['status'] in ['active', 'pregnant']:
                ...
                while current <= end:
                    ...
                    self.milk_production.append({
                        ...
                        'total_yield_liters': round(morning_yield + evening_yield, 1),
    ...
    def generate_summary(self):
        ...
        total_yield = sum(h['yield_kg'] for h in self.harvest_records) if self.harvest_records else 0
