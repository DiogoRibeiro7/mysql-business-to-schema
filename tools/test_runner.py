...
class DatabaseTestSuite:
    ...
    def test_table_structure(self, example_name: str, db_name: str) -> List[TestResult]:
        ...
        if not success:
            ...
            for table_row in tables:
                ...
                success, columns = self.execute_query(
            ...
            if tables_without_pk:
                result.add_warning(
                    f"Tables without primary keys: {', '.join(tables_without_pk)}"
                )
        ...
        if required_tables:
            result = TestResult(
                f"{example_name}/required_tables", "structure_validation"
            )
            ...
            if success:
                ...
                missing_tables = [
                    t for t in required_tables if t not in actual_table_names
                ]
    ...
    def test_constraints(self, example_name: str, db_name: str) -> List[TestResult]:
        ...
        result = TestResult(
            f"{example_name}/unique_constraints", "constraint_validation"
        )
        ...
        try:
            ...
            if success:
                ...
                tables_without_indexes = [
                    t for t in all_table_names if t not in table_indexes
                ]

                if tables_without_indexes:
                    result.add_warning(
                        f"Tables without indexes: {', '.join(tables_without_indexes[:5])}"
                    )

            result.passed(
                f"Found {total_indexes} indexes across {len(table_indexes)} tables"
            )
            ...
            if not generate_script.exists():
                ...
                try:
                    ...
                    if "counts" in config and "distributions" in config:
                        result.passed(
                            f"Generator {generator_name} is properly configured"
                        )
                    else:
    ...
    def generate_report(self, results: List[TestResult], format: str = "text") -> str:
        ...
        for example, example_tests in sorted(example_results.items()):
            ...
            for test in example_tests:
                ...
                report_lines.append(
                    f"  {icon} {test.test_name.split('/')[-1]}: {test.message}"
                )
