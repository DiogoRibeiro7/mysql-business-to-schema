...
def main():
    ...
    for i, gen_path in enumerate(generators, 1):
        print(f"[{i}/{len(generators)}] Testing {gen_path.name}...", end=' ', flush=True)
    ...
    if successful:
        ...
        print("-" * 40)
        sorted_results = sorted(successful, key=lambda x: x['rows_per_sec'], reverse=True)
        for i, r in enumerate(sorted_results[:5], 1):
            print(f"{i}. {r['name']}: {r['rows_per_sec']} rows/sec ({r['rows']} rows in {r['duration']}s)")
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("Results:\n")
        for r in results:
            f.write(f"  {r['name']}: {'SUCCESS' if r['success'] else 'FAIL'} - {r['duration']}s, {r['rows']} rows\n")
