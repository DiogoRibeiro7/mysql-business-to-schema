...
class MySQLToMongoDBConverter:
    ...
    def _analyze_relationships(self):
        ...
        for rel in self.relationships:
            ...
            if self._is_junction_table(from_table):
                ...
                rel['embed_strategy'] = 'reference'  # Usually better to reference in M:N
            else:
                # Check cardinality based on field names and patterns
                if rel['from_field'].endswith('_id'):
                    # Likely a foreign key reference
                    rel['embed_strategy'] = self._determine_embed_strategy(from_table, to_table)
                else:
    ...
    def _create_collection_schema(self, table_name: str, table_info: Dict) -> Dict:
        ...
        for rel in self.relationships:
            ...
            elif rel['from_table'] == table_name and rel.get('embed_strategy') == 'reference':
                reference = {
    ...
    def _create_indexes(self, table_name: str, table_info: Dict) -> List[Dict]:
        ...
        for fk in table_info['foreign_keys']:
            indexes.append({
                'name': f"{table_name}_{fk['field']}_idx",
                'keys': {fk['field']: 1}
            })
        ...
        for idx in table_info.get('indexes', []):
            if 'UNIQUE' in idx.get('definition', '').upper():
                # Parse unique index
                indexes.append({
                    'name': f"{table_name}_unique_idx",
                    'unique': True
                })

        # Add text index for searchable fields
        text_fields = [f['name'] for f in table_info['fields']
                      if f['type'].upper() in ['TEXT', 'VARCHAR'] and f.get('size', 0) != '255']
        if text_fields:
            indexes.append({
                'name': f"{table_name}_text",
                'keys': {field: 'text' for field in text_fields[:3]}  # Limit to 3 fields
            })
    ...
    def _create_aggregations(self, table_name: str) -> List[Dict]:
        ...
        related_tables = [rel['to_table'] for rel in self.relationships
        ...
        numeric_fields = [f['name'] for f in self.tables[table_name]['fields']
                ...
                'pipeline': [
                    {
                        '$group': {
                            '_id': None,
                            'count': {'$sum': 1},
                            **{f"avg_{field}": {'$avg': f"${field}"} for field in numeric_fields[:3]},
                            **{f"sum_{field}": {'$sum': f"${field}"} for field in numeric_fields[:3]}
                        }
                    }
                ]
            })
            ...
            if indexes:
                ...
                for index in indexes:
                    if 'keys' in index:
                        ...
                        script.append(f"db.{collection_name}.createIndex({keys}, {json.dumps(options)});")
                script.append("")
...
def convert_example(example_dir: Path, output_dir: Path = None) -> bool:
    ...
    with open(doc_file, 'w', encoding='utf-8') as f:
        ...
        for collection, schema in mongodb_schema['collections'].items():
            ...
            f.write("\n```\n\n")
            if schema['embedded']:
                f.write(f"**Embedded Documents:** {', '.join([e['field'] for e in schema['embedded']])}\n\n")
            if schema['references']:
                f.write(f"**References:** {', '.join([r['field'] for r in schema['references']])}\n\n")
