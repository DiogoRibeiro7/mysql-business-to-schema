...
class BadgeGenerator:
    ...
    def generate_all(self):
        ...
        generators = len(
            [
                d
                for d in (self.project_root / "generators").iterdir()
        ...
        self.add_badge(
            "Generators",
            f"{generators}/{examples}",
            "green" if generator_coverage >= 90 else "yellow",
        )
        self.add_badge(
            "Coverage",
            f"{generator_coverage:.0f}%",
        ...
        if health_score:
            color = (
                "success"
                if health_score >= 90
                else "green" if health_score >= 75 else "yellow"
            )
    ...
    def add_badge(self, label, message, color):
        ...
        self.badges.append(
            {"label": label, "message": message, "color": color, "url": badge_url}
        )
