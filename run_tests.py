...
def debug(ctx, test_path, debug):
    ...
    if not test_path:
        test_path = click.prompt("Enter test path (e.g., tests/unit/test_generators.py::TestClinicDataGenerator::test_patient_data_generation)")
...
def info():
    ...
    # Test statistics
    result = subprocess.run("pytest --collect-only -q", shell=True, capture_output=True, text=True)
